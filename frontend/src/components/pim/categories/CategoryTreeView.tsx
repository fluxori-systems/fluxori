...
export default function CategoryTreeView({
  ...
  useEffect(() => {
    const loadCategories = async () => {
      if (!isOnline) {
        setError(
          "Cannot load categories while offline. Please check your connection.",
        );
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const categoriesData = await api.pim.getCategories({
    ...
    return (
      <div key={node.id} className="mb-1">
        ...
        {isExpanded && node.children.length > 0 && (
          <div
            className={`pl-5 ${connectionQuality === "critical" ? "max-h-32 overflow-y-auto" : ""}`}
          >
  ...
  return (
    <div className="bg-white rounded shadow">
      ...
      {/* Tree view */}
      <div
        className={`p-4 overflow-auto ${connectionQuality === "critical" ? "max-h-64" : "max-h-[70vh]"}`}
      >
