...
export default function CategoryForm({ 
  ...
  useEffect(() => {
    const initializeForm = async () => {
      ...
      if (categoryId) {
        if (!isOnline) {
          setError('Cannot load category while offline. Please check your connection.');
          setLoading(false);
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isOnline) {
      setError('Cannot save category while offline. Please check your connection.');
      return;
    }
    ...
    try {
      ...
      if (categoryId) {
        ...
        savedCategory = await api.pim.createCategory(formData as Omit<ProductCategory, 'id' | 'createdAt' | 'updatedAt' | 'level' | 'path' | 'productCount'>);
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border rounded"
              />
              <p className="text-sm text-gray-500 mt-1">Used in URLs, auto-generated from name if empty</p>
            </div>
          </div>
