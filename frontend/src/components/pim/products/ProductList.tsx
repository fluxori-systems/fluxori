...
export default function ProductList() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      if (!isOnline) {
        setError('Cannot load products while offline. Please check your connection.');
        setLoading(false);
        return;
      }
    ...
    loadProducts();
  }, [isOnline, connectionQuality, currentPage, pageSize, categoryId, searchQuery, statusFilter]);
  ...
  if (products.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mb-4">Try adjusting your filters or create a new product.</p>
        <button 
  ...
  return (
    <div className="p-4">
      ...
      {loading && products.length > 0 && (
        <div className="mb-4 text-center text-gray-500">Updating results...</div>
      )}
        ...
        <table className="min-w-full bg-white border">
          ...
          <tbody>
            {products.map((product) => (
              <tr key={product.id} onClick={() => handleProductClick(product.id)} className="cursor-pointer hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border">{product.availableQuantity}</td>
                <td className="px-4 py-2 border">
                  <span className={`px-2 py-1 rounded text-xs ${
                    ...
                    product.status === ProductStatus.DRAFT ? 'bg-blue-100 text-blue-800' :
                    product.status === ProductStatus.ARCHIVED ? 'bg-yellow-100 text-yellow-800' :
                    product.status === ProductStatus.OUT_OF_STOCK ? 'bg-red-100 text-red-800' :
        ...
        <div className="mt-6 flex justify-center items-center">
          <button
            ...
            className={`px-3 py-1 rounded ${
              currentPage === 1 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-gray-300 text-gray-700'
            }`}
          >
            Previous
          </button>
          ...
          <button
            ...
            className={`px-3 py-1 rounded ${
              currentPage === totalPages ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-gray-300 text-gray-700'
            }`}
          >
            Next
          </button>
