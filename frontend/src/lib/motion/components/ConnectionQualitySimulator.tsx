...
export function ConnectionQualitySimulator({
  ...
  const [currentProfile, setCurrentProfile] = useState<NetworkProfile>(initialProfile);
  ...
  useEffect(() => {
    ...
    const mockConnection = {
      ...
      removeEventListener: originalConnection?.removeEventListener || (() => {}),
    };
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <span 
                  className={`inline-block w-3 h-3 rounded-full ml-2 ${
                    networkProfiles[currentProfile].quality === 'high' ? 'bg-green-500' :
                    networkProfiles[currentProfile].quality === 'medium' ? 'bg-blue-500' :
                    networkProfiles[currentProfile].quality === 'low' ? 'bg-yellow-500' :
                    'bg-red-500'
                  }`}
                />
              </div>
            ...
            <div className="flex justify-between">
              <label className="inline-flex items-center">
                <input
                  type="radio"
                  checked={mode === 'manual'}
                  onChange={() => mode !== 'manual' && setCurrentProfile(currentProfile)}
