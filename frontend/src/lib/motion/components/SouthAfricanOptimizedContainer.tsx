 ...
 */
export const SouthAfricanOptimizedContainer: React.FC<
  SouthAfricanOptimizedContainerProps
> = ({
  children,
  autoOptimize = true,
  ...
  useEffect(() => {
    ...
    if (animationRecommendations) {
      root.style.setProperty(
        "--sa-animation-duration-multiplier",
    ...
    const observer = new PerformanceObserver((list) => {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.entryType === "resource") {
          ...
          const isAlreadyLoaded = loadedResourcesRef.current.some(
            (r) => r.url === url,
          );
          ...
          const totalUsage =
            loadedResourcesRef.current.reduce((sum, r) => sum + r.size, 0) /
            1024; // KB
          ...
          if (networkRecommendations) {
            const costZAR =
              (totalUsage / 1024) * networkRecommendations.dataCostPerMBZAR;
    ...
    ? React.Children.map(children, (child) => {
        ...
        if (child.type === "img") {
          return React.cloneElement(child as React.ReactElement<any>, {
            ...
            loading: networkRecommendations?.shouldUseLazyLoading
              ? "lazy"
              : undefined,
          });
        }
  ...
  return (
    <div className={`sa-optimized-container ${className}`}>
        ...
        <div className="sa-data-warning">
          <div className="sa-data-warning-content">
            ...
            <div className="sa-data-warning-text">
              ...
              <p>
                You are on an expensive data connection (R
                {networkRecommendations?.dataCostPerMBZAR.toFixed(2)}/MB).
              </p>
              {dataUsage > 0 && (
                <p>
                  Current page: {dataUsage.toFixed(1)} KB (
                  {estimatedCost.toFixed(2)} ZAR)
                </p>
              )}
            </div>
            <button
              className="sa-data-warning-close"
        ...
        <div className="sa-dev-info">
          <details>
            ...
            <div className="sa-dev-info-content">
              <p>
                <strong>Device:</strong> {deviceProfile.deviceName} (
                {deviceProfile.marketSharePercent.toFixed(1)}% market share)
              </p>
              {networkProfile && (
                <p>
                  <strong>Network:</strong> {networkProfile.provider}{" "}
                  {networkProfile.networkType} in {networkProfile.locationType}{" "}
              ...
              {animationRecommendations?.shouldAnimateAtAll && (
                <>
                  ...
                  <p>
                    ...
                    {animationRecommendations?.durationMultiplier.toFixed(1)}x
                  </p>
                </>
              )}
              <p>
                <strong>Images:</strong> Max{" "}
                {networkRecommendations?.maxImageSizeKB}KB
              </p>
              <p>
                <strong>Data usage:</strong> {dataUsage.toFixed(1)} KB (~R
      ...
      {/* Main content */}
      <div className="sa-optimized-content">{optimizedChildren}</div>
      ...
      <style jsx>{`
        ...
        .sa-optimized-content {
          --animation-duration-multiplier: var(
            --sa-animation-duration-multiplier,
            1
          );
