...
export function StreamingText({
  ...
  const shouldStream =
    isStreaming && !shouldReduceMotion && connectionQuality.quality !== "low";
    ...
    if (text !== displayedText) {
      ...
      const streamNextChar = () => {
        if (currentIndex < text.length) {
          ...
          const baseDelay =
            aiAnimations.streaming.characterDuration * 1000 * getSpeedFactor();
          const delay = isPunctuation ? baseDelay * 3 : baseDelay;

          // Schedule next character
          timeoutRef.current = setTimeout(
            streamNextChar,
            delay * complexity.reduceDuration,
          );
        } else {
      ...
      if (char) {
        gsap.fromTo(
          ...
          },
          {
            ...
            y: 0,
            duration:
              aiAnimations.streaming.lineDuration * complexity.reduceDuration,
            ease: complexity.useSimpleEasings
              ? "power1.out"
              : aiAnimations.streaming.ease,
            delay: complexity.disableStaggering
              ? 0
              : index *
                aiAnimations.streaming.stagger *
                complexity.reduceDuration,
          },
        );
      }
    });
