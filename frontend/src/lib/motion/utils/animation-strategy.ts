...
export type { NetworkCondition, AnimationStrategyConfig, AnimationParams } from '../../shared/types/motion-types';
 ...
 */
export function getAnimationStrategy(config: AnimationStrategyConfig): AnimationParams {
  const {
    motionMode,
    networkAware = true,
  ...
  const networkCondition = config.networkCondition || (networkAware ? detectNetworkCondition() : 'fast');
