...
export function useAnimationPerformance(
  ...
  useEffect(() => {
    ...
    try {
      // Get network quality
      const { quality, isDataSaver } = defaultConnectionService.getConnectionQuality();
      
      // Basic adaptations based on network quality
      const networkSettings = {
        reduceComplexity: quality === 'poor' || quality === 'low' || isDataSaver,
        durationMultiplier: 
          quality === 'poor' ? 0.6 :
          quality === 'low' ? 0.8 :
          quality === 'medium' ? 0.9 : 1.0,
        useSimpleEasings: quality === 'poor' || quality === 'low' || isDataSaver,
        disableStaggering: quality === 'poor' || isDataSaver
      };
      
      // Combine with performance-based adaptation
      setAdaptedSettings({
        reduceComplexity: hasPerformanceIssues || networkSettings.reduceComplexity,
        durationMultiplier: hasPerformanceIssues ? 0.7 * networkSettings.durationMultiplier : networkSettings.durationMultiplier,
        useSimpleEasings: hasPerformanceIssues || networkSettings.useSimpleEasings,
        disableStaggering: hasPerformanceIssues || networkSettings.disableStaggering
      });
    } catch (e) {
  ...
  const stopMonitoring = (animationId: string): void => {
    if (!enabled || !activeAnimation.current || activeAnimation.current.id !== animationId) return;
    ...
    if (animation.frameCount > 0 && animation.frameTimes.length > 0) {
      ...
      avgFrameTime = animation.frameTimes.reduce((sum, time) => sum + time, 0) / animation.frameTimes.length;
      ...
      droppedFrameCount = animation.frameTimes.filter(time => time > droppedFrameThreshold).length;
