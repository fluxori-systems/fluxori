  ...
  implements IPerformanceMonitoringService
{
  ...
  public recordMetric(metric: Omit<PerformanceMetric, "timestamp">): void {
    ...
    if (this.config.consoleLogging) {
      console.log(
        `[Performance] ${fullMetric.label}: ${fullMetric.value}ms`,
        fullMetric,
      );
    }
    ...
    if (
      ...
      Math.random() < (this.config.sampleRate || 0.1)
    ) {
    ...
    if (this.metrics.length > (this.config.bufferSize || 100)) {
      ...
      this.metrics = this.metrics.slice(
        -Math.floor((this.config.bufferSize || 100) / 2),
      );
    }
  }
  ...
  public getMetrics(options?: {
    ...
    if (options.priority) {
      filteredMetrics = filteredMetrics.filter(
        (m) =>
          this.getPriorityLevel(m.priority) >=
          this.getPriorityLevel(options.priority || "low"),
      );
    }

    if (options.component) {
      filteredMetrics = filteredMetrics.filter(
        (m) => m.component === options.component,
      );
    }
   ...
   */
  private getPriorityForAnimation(
  ...
  private shouldSampleMetric(
    ...
    if (
      ...
      this.getPriorityLevel(this.config.priorityThreshold || "medium")
    ) {
      return true;
    }
    ...
    switch (this.samplingStrategy) {
      ...
      case "adaptive":
        ...
        if (
          type === "animation" &&
          this.deviceCapabilities.cpuPerformance === "high"
        ) {
          return (
            Math.random() < Math.min(1.0, (this.config.sampleRate || 0.1) * 2)
          );
        }
  ...
  private setSamplingStrategy(): void {
    ...
    if (this.deviceCapabilities.cpuPerformance === "low") {
      ...
      this.config.sampleRate = Math.min(
        (this.config.sampleRate || 0.1) * 2,
        0.5,
      );
    }
  }
 ...
 */
export const defaultPerformanceMonitoringService =
