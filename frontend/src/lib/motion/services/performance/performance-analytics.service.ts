...
export class SouthAfricanPerformanceAnalyticsService {
  ...
  getInsightsBySeverity(
    ...
    return this.getAllInsights().filter(
  ...
  generateInsights(): PerformanceInsight[] {
    ...
    const deviceProfile =
      getDeviceProfile(deviceCapabilities) || southAfricanDeviceProfiles[1]; // Mid-range as default
    ...
    const networkProfile =
      getNetworkProfile(networkConditions) || southAfricanNetworkProfiles[0]; // Vodacom urban as default
  ...
  private generateDeviceInsights(
    ...
    if (deviceProfile.memory <= 2048) {
      insights.push({
        ...
        severity: "high",
        recommendation:
          "Reduce memory usage by implementing memory-efficient components, lazy loading and careful DOM management.",
    ...
    if (
      deviceProfile.processorTier === "entry" ||
      deviceProfile.processorTier === "low"
    ) {
      insights.push({
        ...
        severity: "medium",
        recommendation:
          "Optimize JavaScript execution, reduce animation complexity, and consider using lightweight animations for this device profile.",
        relatedMetrics: metrics
    ...
    if (!deviceProfile.hasGPUAcceleration) {
      insights.push({
        ...
        severity: "medium",
        recommendation:
          "Reduce use of complex CSS animations, shadows, blurs, and 3D transforms. Consider using simpler visual effects for this device profile.",
        relatedMetrics: metrics
  ...
  private generateNetworkInsights(
    ...
    if (networkProfile.dataCostPerMBZAR > 0.04) {
      ...
      if (recommendedOptimizations.length > 0) {
        insights.push({
          ...
          relatedMetrics: [],
          targetNetworkProfiles: [
            `${networkProfile.provider}_${networkProfile.locationType}`,
          ],
          estimatedImpact: 8,
          timestamp: Date.now(),
        });
    ...
    if (networkProfile.latencyMs > 200) {
      insights.push({
        ...
        severity: "high",
        recommendation:
          "Implement optimistic UI updates, reduce round-trips, and use background sync for non-critical operations.",
        relatedMetrics: [],
        targetNetworkProfiles: [
          `${networkProfile.provider}_${networkProfile.locationType}`,
        ],
    ...
    if (networkProfile.packetLossPercent > 5.0) {
      insights.push({
        ...
        severity: "high",
        recommendation:
          "Implement offline-first approach, robust retry mechanisms, and local caching strategies.",
        relatedMetrics: [],
        targetNetworkProfiles: [
          `${networkProfile.provider}_${networkProfile.locationType}`,
        ],
  ...
  private generateMetricInsights(
    ...
    if (hasLowFPS) {
      insights.push({
        ...
        description:
          "Frame rate is below 30 FPS, which results in visibly choppy animations.",
        severity: "high",
        recommendation:
          "Optimize animations, reduce visual complexity, and prioritize smooth motion.",
    ...
    const memoryMetrics = metrics.filter(
      (m) => m.label.includes("memory") || m.label.includes("heap"),
    );
    const highMemoryUsage = memoryMetrics.some(
      (m) =>
    ...
    if (highMemoryUsage) {
      insights.push({
        ...
        description:
          "Memory usage is excessive, which may lead to poor performance or crashes on lower-end devices.",
        severity: "critical",
        recommendation:
          "Check for memory leaks, reduce DOM size, and optimize asset loading.",
    ...
    const clsMetrics = metrics.filter(
      (m) => m.label.includes("cls") || m.label.includes("layout_shift"),
    );
    ...
    if (highCLS) {
      insights.push({
        ...
        description:
          "Cumulative Layout Shift (CLS) is high, causing a poor user experience as elements move unexpectedly.",
        severity: "high",
        recommendation:
          "Set explicit dimensions for images and embeds, avoid inserting content above existing content, and use transform animations instead of layout-triggering properties.",
    ...
    const loadingMetrics = metrics.filter(
      (m) =>
    ...
    const slowLoading = loadingMetrics.some(
      (m) =>
    ...
    if (slowLoading) {
      ...
      let loadingRecommendation =
        "Optimize critical rendering path, reduce initial bundle size, and implement code splitting.";

      if (networkProfile && networkProfile.downloadSpeedMbps < 1.0) {
        // Specific recommendations for low-bandwidth networks
        loadingRecommendation =
          "Drastically reduce initial payload size, implement aggressive code splitting, and defer non-critical resources. Consider a text-only mode for extremely slow connections.";
      }
      ...
      insights.push({
        ...
        description:
          "Page loading metrics indicate the site loads too slowly, especially on slower connections common in South Africa.",
        severity: "critical",
    ...
    const interactivityMetrics = metrics.filter(
      (m) =>
    ...
    const poorInteractivity = interactivityMetrics.some(
      (m) =>
    ...
    if (poorInteractivity) {
      insights.push({
        ...
        description:
          "The page takes too long to become fully interactive, causing user frustration, especially on lower-end devices common in South Africa.",
        severity: "high",
        recommendation:
          "Break up long tasks, optimize JavaScript execution, reduce main thread work, and implement web workers for heavy operations.",
...
export const defaultPerformanceAnalyticsService =
  new SouthAfricanPerformanceAnalyticsService();
