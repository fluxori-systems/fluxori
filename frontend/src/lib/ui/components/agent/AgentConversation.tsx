...
import React, { createContext as reactCreateContext, useContext, useState, useCallback, useEffect, ReactNode } from 'react';
...
const AgentConversationContextValue = createContext<AgentConversationContext | undefined>(undefined);
...
export function useAgentConversation() {
  ...
  if (!context) {
    throw new Error('useAgentConversation must be used within an AgentConversationProvider');
  }
  return context;
}
...
export function AgentConversationProvider({
  ...
  const [title, setTitle] = useState<string>(initialConversation?.title || 'New Conversation');
  const [messages, setMessages] = useState<AgentMessage[]>(initialConversation?.messages || []);
  const [agentState, setAgentState] = useState<'idle' | 'thinking' | 'processing' | 'streaming' | 'complete' | 'error'>(
    initialConversation?.agentState || 'idle'
  );
  const [activeTools, setActiveTools] = useState<AgentToolUsage[]>(initialConversation?.activeTools || []);
  const [lastUpdated, setLastUpdated] = useState<Date>(initialLastUpdated);
  const [metadata, setMetadata] = useState<Record<string, any>>(initialConversation?.metadata || {});
  ...
  const sendMessage = useCallback(async (content: string, attachments?: any[]) => {
    try {
      ...
      setActiveTools(prev => 
        prev.map(tool => 
          tool.id === mockTool.id 
            ? { ...tool, status: 'success', result: 'Found relevant information' } 
            : tool
        )
      );
    ...
    try {
      ...
      const mockConversation = {
        ...
        messages: [
          ...
          {
            id: uuidv4(),
            content: 'I understand you want to access a previous conversation. Here it is!',
        ...
        <Stack gap="md">
          {messages.map(message => (
            <AgentMessageComponent
              ...
              tools={message.tools?.map(tool => ({ name: tool.name, count: tool.count }))}
            />
          ))}
          ...
          {agentState !== 'idle' && agentState !== 'complete' && (
            <AgentMessageComponent
              ...
              tools={activeTools.map(tool => ({ name: tool.name, count: tool.count }))}
            />
          )}
      ...
      {showInput && (
        ...
        >
          <AgentInput
            ...
            disabled={agentState === 'thinking' || agentState === 'processing' || agentState === 'streaming'}
            loading={agentState !== 'idle' && agentState !== 'complete' && agentState !== 'error'}
            onSend={sendMessage}
