...
export const Container = forwardRef<HTMLDivElement, ContainerProps>(
  (
    ...
    const shouldOptimize =
      networkAware && (isDataSaver || quality === "poor" || quality === "low");
    ...
    const getContainerSize = (): number | string => {
      ...
      if (isRuralSouthAfrican && saMarket.shouldReduceDataUsage) {
        ...
        switch (size) {
          ...
          case "lg":
          case "xl":
            return "var(--container-size-xs, 576px)"; // Use smallest size for all
          case "fluid":
            return "100%";
          default:
            return "var(--container-size-xs, 576px)";
        }
      }
      ...
      switch (size) {
        case "xs":
          return "var(--container-size-xs, 576px)"; // Mobile focused
        case "sm":
          return "var(--container-size-sm, 768px)"; // Small tablets
        case "md":
        ...
        case "fluid":
          return "100%";
        default:
          return "var(--container-size-md, 992px)";
      }
    };
    ...
    const combinedClassName =
      `flx-container flx-container-${size} container-${intent} ${
        saSensitive ? "sa-optimized" : ""
      } ${priority ? `priority-${priority}` : ""} ${className}`.trim();
    ...
    if ((animatePresence || animated) && !shouldOptimize) {
      ...
      animationStyle = {
        opacity: isVisible ? 1 : 0,
        animation: isVisible
          ? `saOptimizedFade ${networkAnimationDuration}s ease-out forwards`
          : undefined,
        animationDelay: `${networkAnimationDelay}s`,
      };
    }
