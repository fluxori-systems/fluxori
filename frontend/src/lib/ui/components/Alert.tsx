 ...
 * Alert colors
 */
export type AlertColor = 'info' | 'success' | 'warning' | 'error' | 'primary' | 'secondary' | 'neutral';

/**
 * Semantic intent/purpose for the alert
 */
export type AlertIntent = 'default' | 'notification' | 'system' | 'feedback' | 'action-required';
...
export const Alert = forwardRef<HTMLDivElement, AlertProps>(
  ({ 
    ...
    const [showShake, setShowShake] = useState(color === 'error' || color === 'warning');
    ...
    useEffect(() => {
      if (showShake) {
        const timer = setTimeout(() => {
          setShowShake(false);
        }, shouldDisableAnimations ? 250 : 500); // Shorter time on poor connections
    ...
    const mapVariant = (alertVariant: AlertVariant): 'light' | 'filled' | 'default' => {
    ...
    const backgroundColorValue = getIntentBackgroundColor(styleIntent, mapVariant(variant));
    ...
    useEffect(() => {
      if (autoClose && autoClose > 0) {
        ...
        const adjustedAutoClose = shouldDisableAnimations && networkAware ? 
          autoClose * 1.5 : autoClose;
    ...
    const alertStyles: React.CSSProperties = {
      ...
      boxShadow: networkAware && (isDataSaver || quality === 'poor') ? 'none' : undefined,
      ...props.style
    };
    
    // Build class name with intent and network quality
    const combinedClassName = `flx-alert flx-alert-${color} flx-alert-${variant} alert-intent-${intent} ${className}`.trim();
