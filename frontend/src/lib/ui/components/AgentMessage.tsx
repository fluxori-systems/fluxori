 ...
 */
export type AgentMessageType = 'user' | 'agent' | 'system' | 'error' | 'warning' | 'info';
 ...
 */
export type AgentState = 'idle' | 'thinking' | 'processing' | 'streaming' | 'complete' | 'error';
...
export function AgentMessage({ 
  ...
  const getMessageVariant = (): 'default' | 'filled' | 'outlined' => {
    switch (type) {
      case 'user': return 'outlined';
      case 'system': 
      case 'info': 
      case 'warning': 
      case 'error': return 'filled';
  ...
  const getBorderColor = (): string => {
    switch (type) {
      case 'user': return 'var(--color-primary-300)';
      case 'system': return 'var(--color-neutral-300)';
      case 'error': return 'var(--color-error-base)';
      case 'warning': return 'var(--color-warning-base)';
      case 'info': return 'var(--color-info-base)';
  ...
  const getConfidenceColor = (): string => {
    switch (confidence) {
      ...
      case 'medium': return 'var(--color-warning-base)';
      case 'low': return 'var(--color-error-base)';
      ...
      >
        ...
        <Text size="xs" c="var(--text-secondary)">
          {confidence === 'high' ? 'High confidence' : 
            ...
            confidence === 'low' ? 'Low confidence' : 'Unknown confidence'}
        </Text>
      </div>
    );
  };
  ...
  const renderToolsUsed = () => {
    ...
    return (
      ...
      >
        <Text size="xs" c="var(--text-secondary)">Tools used:</Text>
    ...
    return (
      <div style={{ marginBottom: 'var(--spacing-sm)' }}>
        <AIProcessingIndicator 
          state={state === 'error' ? 'idle' : state === 'streaming' ? 'processing' : state}
          size={24}
    ...
    >
      ...
      >
        ...
        {streaming && content ? (
          ...
          <div className="flx-agent-message-content">
            {children || content}
          </div>
        )}
        ...
        <div
          style={{
            marginTop: showConfidence || (showTools && tools.length) ? 'var(--spacing-sm)' : 0,
            display: 'flex',
            flexDirection: 'column',
            gap: 'var(--spacing-xs)'
          }}
