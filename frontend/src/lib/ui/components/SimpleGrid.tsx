...
export const SimpleGrid = forwardRef<HTMLDivElement, SimpleGridProps>(
  (
    ...
    if (typeof spacing === "string" && spacing !== "auto") {
      ...
      const baseSpacing = getSpacingFromMantine(
        theme,
        spacing as keyof SpacingScale,
        undefined,
      );
    ...
    if (typeof verticalSpacing === "string" && verticalSpacing !== "auto") {
      const baseVerticalSpacing = getSpacingFromMantine(
        theme,
        verticalSpacing as keyof SpacingScale,
        undefined,
      );

      if (shouldSimplify && isSaSensitive) {
        verticalSpacingValue =
          typeof baseVerticalSpacing === "number"
    ...
    if (props.breakpoints) {
      processedBreakpoints = props.breakpoints.map(
        (breakpoint: { maxWidth: number; cols: number; spacing?: Spacing }) => {
          if (
            typeof breakpoint.spacing === "string" &&
            breakpoint.spacing !== "auto"
          ) {
            const baseBreakpointSpacing = getSpacingFromMantine(
              theme,
              breakpoint.spacing as keyof SpacingScale,
              undefined,
            );
            ...
            if (shouldSimplify && isSaSensitive && saMarket.isRural) {
              ...
              const optimizedSpacing =
                typeof baseBreakpointSpacing === "number"
    ...
    return <MantineSimpleGrid {...mantineProps}>{children}</MantineSimpleGrid>;
  },
);
