...
export function useObservability(customConfig?: Partial<ObservabilityConfig>) {
  ...
  const unhandledErrorListenerRef = useRef<(event: ErrorEvent) => void | null>(null);
  const unhandledRejectionListenerRef = useRef<(event: PromiseRejectionEvent) => void | null>(null);
  ...
  useEffect(() => {
    if (!config.enabled || !config.performanceSamplingRate || Math.random() > config.performanceSamplingRate) {
      return;
    }
...
export function useComponentPerformance(componentName: string) {
  ...
  const endMeasure = useCallback((operation: string) => {
