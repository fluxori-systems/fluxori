...
export function useFirestore<T extends BaseEntity>(
  ...
  const getById = useCallback(async (id: string) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error : new Error('Failed to get document'));
      return null;
    }
  }, [service, setLoading, setError]);
  
  // Get all documents
  const getAll = useCallback(async (options?: QueryOptions) => {
    setLoading(true);
    
    try {
      // If this is a tenant collection and we have a current user with an organization
      if (isTenantCollection && user?.organizationId) {
        const tenantService = service as TenantFirestoreService<T & TenantEntity>;
        const results = await tenantService.getAllForOrganization(user.organizationId, options);
        setState(prevState => ({ ...prevState, items: results, isLoading: false }));
        return results;
      } else {
      ...
      setError(error instanceof Error ? error : new Error('Failed to get documents'));
      return [];
    }
  }, [service, user, isTenantCollection, setLoading, setError]);
  
  // Get paginated documents
  const getPaginated = useCallback(async (options?: QueryOptions) => {
    setLoading(true);
    ...
    try {
      ...
      if (isTenantCollection && user?.organizationId) {
        const tenantService = service as TenantFirestoreService<T & TenantEntity>;
        const results = await tenantService.getPaginatedForOrganization(user.organizationId, options);
        setState(prevState => ({ ...prevState, paginatedData: results, items: results.data, isLoading: false }));
        return results;
      } else {
        // For non-tenant collections or when no user is logged in
      ...
      setError(error instanceof Error ? error : new Error('Failed to get paginated documents'));
      return {
        data: [],
        meta: {
          currentPage: 1,
  ...
  // Create a document
  const create = useCallback(async (data: Omit<T, 'id'>, customId?: string) => {
    setLoading(true);
    ...
    try {
      ...
      setError(error instanceof Error ? error : new Error('Failed to create document'));
      return null;
    }
  }, [service, user, isTenantCollection, setLoading, setError]);
  ...
  const update = useCallback(async (id: string, data: Partial<T>) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error : new Error('Failed to update document'));
      return null;
    }
  }, [service, setLoading, setError]);
  
  // Delete a document
  const remove = useCallback(async (id: string, softDelete = true) => {
    setLoading(true);
    ...
    try {
      ...
      setError(error instanceof Error ? error : new Error('Failed to delete document'));
      return false;
    }
  }, [service, setLoading, setError]);
