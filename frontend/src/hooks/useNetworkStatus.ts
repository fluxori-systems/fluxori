...
export function useNetworkStatus(): NetworkStatus {
  ...
  const checkForLoadShedding = useCallback(() => {
    ...
    const now = Date.now();
    const disconnectionEvents = sessionStorage.getItem(
      "network_disconnections",
    );
  ...
  const logDisconnection = useCallback(() => {
    ...
    const events = JSON.parse(
      sessionStorage.getItem("network_disconnections") || "[]",
    );
    events.push(now);
    ...
    if (connection) {
      ...
      if (connection.effectiveType) {
        switch (connection.effectiveType) {
          case "4g":
            return connection.rtt < SA_NETWORK_SETTINGS.qualityThresholds.good
              ? "excellent"
              : "good";
          case "3g":
            return "fair";
      ...
      if (connection.rtt) {
        if (connection.rtt < SA_NETWORK_SETTINGS.qualityThresholds.excellent)
          return "excellent";
        if (connection.rtt < SA_NETWORK_SETTINGS.qualityThresholds.good)
          return "good";
        if (connection.rtt < SA_NETWORK_SETTINGS.qualityThresholds.fair)
          return "fair";
        if (connection.rtt < SA_NETWORK_SETTINGS.qualityThresholds.poor)
          return "poor";
        if (connection.rtt < SA_NETWORK_SETTINGS.qualityThresholds.critical)
          return "critical";
        return "critical";
      }
