...
export default function ChartsShowcasePage() {
  ...
  const { networkProfile, shouldReduceDataUsage } =
    useSouthAfricanMarketOptimizations();

  // State to control the connection quality simulation
  const [simulatedQuality, setSimulatedQuality] = useState<
    ChartConnectionQuality | undefined
  >(undefined);

  // Generate larger dataset for complex charts
  const [lineData, setLineData] = useState<ReturnType<typeof generateLineData>>(
    generateLineData(30),
  );
    ...
    >
      <div style={{ marginBottom: "var(--spacing-xl)" }}>
        ...
        <Text style={{ marginBottom: "var(--spacing-md)" }}>
          These charts are built with Chart.js and integrate with our design
          system and South African market optimizations. They adapt to network
          conditions to provide the best performance across different connection
          speeds.
        </Text>
        <div
          style={{
        ...
        >
          <div>
            ...
            <Text>
              {networkProfile}{" "}
              {shouldReduceDataUsage ? "(Data saving enabled)" : ""}
            </Text>
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "var(--spacing-sm)" }}>
              {(
                ["high", "medium", "low", "poor"] as ChartConnectionQuality[]
              ).map((quality) => (
                <button
                  key={quality}
                  onClick={() =>
      ...
      <div style={{ marginBottom: "var(--spacing-xl)" }}>
        ...
        <Text size="sm" c="dimmed" style={{ marginTop: "var(--spacing-xs)" }}>
          This chart dynamically adapts to connection quality. Try simulating
          different connection qualities above.
        </Text>
      </div>
      ...
      <div style={{ marginBottom: "var(--spacing-xl)" }}>
        ...
        <Text size="sm" c="dimmed" style={{ marginTop: "var(--spacing-xs)" }}>
          Notice how grid lines, animations, and data labels adapt based on
          connection quality.
        </Text>
      </div>
      ...
      <div style={{ marginBottom: "var(--spacing-xl)" }}>
        ...
        <Text size="sm" c="dimmed" style={{ marginTop: "var(--spacing-xs)" }}>
          On poor connections, this shows a simplified table view to conserve
      ...
      >
        ...
        <Text size="sm" style={{ marginTop: "var(--spacing-xs)" }}>
          These charts integrate with our design system tokens for colors,
          typography, and motion, and automatically adapt to network conditions
          through the South African market optimizations. They implement the
          Agent Appropriateness Framework to determine when to show interactive
