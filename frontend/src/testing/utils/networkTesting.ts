...
export function setupNetworkConditions(config: NetworkTestConfig = {}) {
  ...
  const updateNetworkConditions = (newConfig: NetworkTestConfig = {}) => {
    const preset = newConfig.preset
      ? NetworkQuality[newConfig.preset]
      : undefined;

    // Create a new mock with updated values
    const newConnectionMock = createConnectionMock({
      effectiveType:
        newConfig.effectiveType ||
        preset?.effectiveType ||
        connectionMock.effectiveType,
      downlink:
        newConfig.downlink ?? preset?.downlink ?? connectionMock.downlink,
      rtt: newConfig.rtt ?? preset?.rtt ?? connectionMock.rtt,
      saveData:
        newConfig.saveData ?? preset?.saveData ?? connectionMock.saveData,
    });
...
export function withNetworkConditions<T>(
  ...
  return async (...args: any[]) => {
    ...
    try {
      ...
      return await testFn(
        { updateNetworkConditions: networkUtils.updateNetworkConditions },
        ...args,
      );
    } finally {
      // Clean up
...
export function describeWithNetworkConditions(
  name: string,
  testFn: (utils: {
    network: ReturnType<typeof setupNetworkConditions>;
  }) => void,
) {
  describe(name, () => {
