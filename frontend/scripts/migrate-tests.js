...
const replacements = [
  ...
  // Update render imports
  {
    ...
    replace:
      "import { $1renderWithProviders as render$2 } from '../../../testing/utils/render'",
  },
  ...
  // Fix problematic React hook mocks
  {
    ...
    replace:
      "// REMOVED problematic React hooks mock that causes 'Invalid hook call' errors",
  },
  // Fix mock
  {
    find: /jest\.mock\(/g,
...
function addVitestImports(content) {
  if (needsVitestImports(content)) {
    return (
      `// Make sure to add 'use client' for components that use hooks
'use client';
...
if (customPath) {
  ...
  const files = glob.sync(`${customPath}/**/*.{ts,tsx}`, {
    cwd: process.cwd(),
  });
...
testFiles.forEach((file) => {
  ...
  try {
    ...
    if (oldPath !== newPath) {
      ...
      console.log(
        `Migrated: ${file} -> ${path.relative(process.cwd(), newPath)}`,
      );
    } else {
