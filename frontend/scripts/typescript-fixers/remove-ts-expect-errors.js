  ...
  if (content.includes('@ts-expect-error')) {
    ...
    if (!content.includes("import '@testing-library/jest-dom'")) {
      ...
      if (lastImport) {
        ...
        content = content.slice(0, position) + "\nimport '@testing-library/jest-dom';" + content.slice(position);
      } else {
        // No imports found, add at the top after 'use client' if present
        if (content.startsWith("'use client'")) {
          const clientDirectiveEndPosition = content.indexOf("'use client'") + "'use client'".length;
          content = content.slice(0, clientDirectiveEndPosition) + "\n\nimport '@testing-library/jest-dom';" + content.slice(clientDirectiveEndPosition);
        } else {
