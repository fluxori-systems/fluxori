...
const expectMatcherRegex = new RegExp(
  `expect\\([^)]+\\)\\.(?:not\\.)?(?:${jestDomMatchers.join("|")})`,
  "g",
);
...
${content}`;
  ...
  jestDomMatchers.forEach((matcher) => {
    ...
    content = content.replace(
      matcherRegex,
      (match, expPart, notPart, matcherPart) => {
...
console.log(
  "All test files processed with properly documented TypeScript expect-error comments",
);
