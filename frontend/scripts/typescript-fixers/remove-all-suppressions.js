...
const findCommand = "grep -r '@ts-expect-error\\|@ts-ignore\\|@ts-nocheck' --include='*.ts' --include='*.tsx' ./src";
let filePaths = [];
...
filePaths.forEach(filePath => {
  ...
  if (filePath.includes('__tests__') && 
    ...
    if (importMatch) {
      ...
      content = content.slice(0, pos) + "import '@testing-library/jest-dom';\n" + content.slice(pos);
    }
  }
  
  // Remove all TypeScript suppressions
  const beforeCount = (content.match(/@ts-(expect-error|ignore|nocheck)/g) || []).length;
  content = content.replace(/\/\/\s*@ts-(expect-error|ignore|nocheck).*\n/g, '');
  content = content.replace(/\/\*\s*@ts-(expect-error|ignore|nocheck).*?\*\//g, '');
