...
const testPatterns = ["src/**/*.test.{ts,tsx}", "src/**/*.spec.{ts,tsx}"];

// Get all test files
const getTestFiles = () => {
  try {
    const output = execSync(
      `find src -name "*.test.tsx" -o -name "*.spec.tsx" -o -name "*.test.ts" -o -name "*.spec.ts"`,
      { encoding: "utf8" },
    );
    return output.split("\n").filter(Boolean);
  } catch (error) {
...
const main = () => {
  ...
  console.log(
    `- Files unchanged: ${testFiles.length - updatedCount - errorCount}`,
  );
