"""
Loot scraper implementation for marketplace data collection.

This module provides the Loot-specific implementation of the marketplace scraper,
with specialized methods for extracting data from Loot's website.
"""
...
class LootScraper(MarketplaceScraper):
    """Loot-specific implementation of the marketplace scraper.
    
    This class provides specialized methods for scraping Loot's website,
    ...
    def __init__(self, 
        ...
        """
        # List of generic templates that might be compatible with Loot
        ...
        # Loot-specific constants
        self.category_base_url = "https://www.loot.co.za/category"
        self.search_base_url = "https://www.loot.co.za/search"
        self.product_base_url = "https://www.loot.co.za/product"
        self.daily_deals_url = "https://www.loot.co.za/deals"
    ...
    async def discover_products(self, 
        ...
        try:
            ...
            if category:
                ...
                url = f"{self.base_url}/newreleases?offset={(page-1)*24}"
                
            # Fetch page with JavaScript rendering
            response = await self.fetch_page(
                url=url,
                use_js=True,
                selector_to_wait=".product-list-item"
            )
            ...
            if "content" in response:
                ...
                product_links = re.findall(r'<a\s+[^>]*href="(/product/[^"]+)"[^>]*>', html_content)
                
                # Build product URLs
                for link in product_links:
                    product_urls.append(f"{self.base_url}{link}")
    ...
    def _build_category_url(self, category: str, page: int = 1) -> str:
        ...
        if category.startswith("http"):
            ...
            params["offset"] = [(page-1) * 24]  # Loot uses offset with 24 items per page
        ...
        if category.startswith("/"):
            ...
            base_path = f"/category/{category}"
            
        # Add pagination
        return f"{self.base_url}{base_path}?offset={(page-1)*24}"
            
    async def extract_product_details(self, product_id_or_url: str) -> Dict[str, Any]:
        """Extract detailed product information from Loot.
        ...
        try:
            ...
            session_id = f"loot_product_{int(time.time())}"
    ...
    async def search_products(self, 
        ...
        """Search for products on Loot using a keyword.
        ...
        try:
            # Build search URL
            search_url = f"{self.search_base_url}?q={quote(keyword)}&offset={(page-1)*24}"
            
            # Create a session ID for consistent IP usage
            session_id = f"loot_search_{int(time.time())}"
            ...
            if not template_success:
                ...
                response = await self.fetch_page(
                    url=search_url,
                    use_js=True,
                    selector_to_wait=".product-list-item, .no-results-message",
                    session_id=session_id
                )
    ...
    async def extract_search_suggestions(self, keyword_prefix: str) -> Dict[str, Any]:
        """Extract search suggestions for a keyword prefix from Loot.
        ...
        try:
            ...
            suggest_url = f"{self.base_url}/search/autocomplete?q={quote(keyword_prefix)}"
    ...
    async def extract_category(self, category_id_or_url: str) -> Dict[str, Any]:
        """Extract category details from Loot.
        ...
        try:
            ...
            if category_id_or_url.startswith("http"):
                ...
                # Assume it's a category path slug
            ...
            response = await self.fetch_page(
                url=category_url,
                use_js=True,
                selector_to_wait=".category-page"
            )
    ...
    async def extract_daily_deals(self) -> List[Dict[str, Any]]:
        ...
        try:
            # Create a session ID for consistent IP usage
            session_id = f"loot_deals_{int(time.time())}"
            ...
            if not template_success:
                ...
                if "content" in response:
                    ...
                    for link in unique_links[:20]:  # Limit to first 20 deals to avoid too many requests
    ...
    async def test_template_compatibility(self, test_url: Optional[str] = None) -> Dict[str, Any]:
        """Test compatibility of various templates with Loot.
        
        Runs a comprehensive test of available templates against Loot pages
        ...
        """
        self.logger.info("Starting template compatibility testing for Loot")
        
        # Use provided URL or default to popular category page
        if not test_url:
            test_url = f"{self.base_url}/category/books-1"
            
        # Create test URLs for different page types
        product_url = f"{self.base_url}/product/harry-potter-and-the-philosopher-s-stone/qbfr-930-g010"
        search_url = f"{self.search_base_url}?q=smartphone&offset=0"
        category_url = f"{self.category_base_url}/electronics-5"
    ...
    async def discover_products_by_category(self, 
                                          category_slug: str, 
                                          max_pages: int = 5,
                                          products_per_page: int = 24) -> List[str]:
        ...
        try:
            ...
            session_id = f"loot_category_{int(time.time())}"
            ...
            for page in range(1, max_pages + 1):
                ...
                if "content" in response:
                    ...
                    # Build product URLs and remove duplicates
                    page_products = []
                    for link in list(dict.fromkeys(product_links)):
                        page_products.append(f"{self.base_url}{link}")
            ...
            # Log final count and remove duplicates
            unique_products = list(dict.fromkeys(discovered_products))
            self.logger.info(f"Total discovered products from category {category_slug}: {len(unique_products)}")
    ...
    async def discover_products_by_search(self, 
        ...
        try:
            ...
            unique_products = list(dict.fromkeys(discovered_products)) 
            self.logger.info(f"Total discovered products from search '{keyword}': {len(unique_products)}")
            return unique_products
    ...
    async def extract_product_pricing_history(self, 
        ...
        This method retrieves historical pricing data from the storage repository.
        ...
        try:
            ...
            # Get historical pricing from repository
