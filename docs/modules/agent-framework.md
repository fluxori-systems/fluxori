...
export class ProductAnalysisService {
  ...
  async analyzeProduct(userId: string, organizationId: string, productData: any): Promise<string> {
    ...
    const conversation = await this.agentService.createConversation(createRequest);
    
    // Send a message to the agent
    const response = await this.agentService.sendMessage({
      userId,
      conversationId: conversation.id,
      message: 'Analyze this product for potential market opportunities and pricing strategies.'
    });
