...
export class WarehouseService {
  ...
  async createWarehouse(
    ...
    this.logger.log(
      `Creating new warehouse: ${createWarehouseDto.name} (${createWarehouseDto.code})`,
    );
  ...
  async getWarehouseByCode(
    ...
    const warehouse = await this.warehouseRepository.findByCode(
      organizationId,
      code,
    );
  ...
  async getDefaultWarehouse(organizationId: string): Promise<Warehouse> {
    const warehouse =
      await this.warehouseRepository.findDefaultWarehouse(organizationId);

    if (!warehouse) {
      throw new NotFoundException(
        `No default warehouse found for organization ${organizationId}`,
      );
    }
  ...
  async setAsDefaultWarehouse(id: string): Promise<Warehouse> {
    ...
    const success = await this.warehouseRepository.setAsDefault(
      id,
  ...
  async updateCapacity(id: string, usedCapacity: number): Promise<Warehouse> {
    ...
    const updated = await this.warehouseRepository.updateCapacity(
      id,
      usedCapacity,
    );
   ...
   * @returns Success indicator
   */
  private async clearDefaultWarehouses(
    organizationId: string,
  ): Promise<boolean> {
    const warehouses =
      await this.warehouseRepository.findByOrganization(organizationId);

    const defaultWarehouses = warehouses.filter(
