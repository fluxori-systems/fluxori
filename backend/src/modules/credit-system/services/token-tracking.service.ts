...
export class TokenTrackingService {
  ...
  async checkCreditsForModelRequest(
    ...
    try {
      ...
      this.logger.error(`Error checking credits for model request: ${error.message}`, error.stack);
  ...
  async recordAgentResponseUsage(
    ...
    try {
      ...
      this.logger.error(`Error recording agent response usage: ${error.message}`, error.stack);
      return false;
    }
  }
  ...
  async checkCreditsForEmbedding(
    ...
    try {
      ...
      this.logger.error(`Error checking credits for embedding: ${error.message}`, error.stack);
  ...
  async checkCreditsForRagQuery(
    ...
    try {
      ...
      const documentTokens = Math.ceil(documentCount * averageDocumentLength / 4);
      ...
      this.logger.error(`Error checking credits for RAG query: ${error.message}`, error.stack);
    ...
    try {
      ...
      if (taskComplexity === "simple") {
        eligibleModels = activeModels.filter(m => 
          m.complexity === "simple" || m.complexity === "standard" || m.complexity === "complex"
        );
      ...
      const estimatedTokens = this.tokenEstimator.estimateTokensForString(userPrompt);
      
      // Sort models by cost
      const sortedModels = [...eligibleModels].sort((a, b) => {
        // Calculate total cost
        const costA = (estimatedTokens * a.costPer1kInputTokens / 1000) + 
                      (estimatedTokens * 0.7 * a.costPer1kOutputTokens / 1000);
        const costB = (estimatedTokens * b.costPer1kInputTokens / 1000) + 
      ...
      this.logger.error(`Error optimizing model selection: ${error.message}`, error.stack);
