...
export class OrderMapperRegistry implements IOrderMapperRegistry {
  ...
  registerMapper(marketplaceId: string, mapper: IOrderMapper): void {
    this.logger.log(
      `Registering order mapper for marketplace: ${marketplaceId}`,
    );

    if (this.mappers.has(marketplaceId)) {
      this.logger.warn(
        `Overriding existing order mapper for marketplace: ${marketplaceId}`,
      );
    }
  ...
  getMapper(marketplaceId: string): IOrderMapper {
    ...
    if (!mapper) {
      this.logger.error(
        `No order mapper registered for marketplace: ${marketplaceId}`,
      );
      throw new Error(
        `No order mapper registered for marketplace: ${marketplaceId}`,
      );
