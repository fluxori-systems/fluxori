...
export class EmbeddingService {
   ...
   */
  async createProvider(createDto: CreateEmbeddingProviderDto): Promise<EmbeddingProvider> {
    ...
    const data: any = {
      ...
      costPerToken: createDto.costPerToken || this.getDefaultCostPerToken(createDto.type),
      usageCount: 0,
    };
   ...
   */
  async updateProvider(id: string, updateDto: UpdateEmbeddingProviderDto): Promise<EmbeddingProvider> {
    ...
    const updated = await this.embeddingProviderRepository.update(id, updateDto);
  ...
  async setAsDefault(id: string): Promise<EmbeddingProvider> {
    ...
    const success = await this.embeddingProviderRepository.setAsDefault(id, provider.organizationId);
  ...
  async setEnabled(id: string, enabled: boolean): Promise<EmbeddingProvider> {
    this.logger.log(`${enabled ? 'Enabling' : 'Disabling'} embedding provider ${id}`);
    
    const updated = await this.embeddingProviderRepository.setEnabled(id, enabled);
  ...
  async deleteProvider(id: string): Promise<boolean> {
    ...
    const result = await this.embeddingProviderRepository.delete(id);
  ...
  async generateEmbeddings(
    ...
    if (providerId) {
      ...
      if (!defaultProvider) {
        throw new Error(`No default embedding provider found for organization ${organizationId}`);
      }
  ...
  private async callEmbeddingProvider(
    ...
    this.logger.log(`Generating embeddings using ${provider.type}/${provider.modelName}`);
   ...
   * @returns Success indicator
   */
  private async clearDefaultProviders(organizationId: string): Promise<boolean> {
    const providers = await this.embeddingProviderRepository.findByOrganization(organizationId);
    
    const defaultProviders = providers.filter(provider => provider.isDefault);
    
    for (const provider of defaultProviders) {
      await this.embeddingProviderRepository.update(provider.id, { isDefault: false });
    }
