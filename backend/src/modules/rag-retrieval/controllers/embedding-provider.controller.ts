...
export class EmbeddingProviderController {
   ...
   */
  @Post()
  async create(@Body() createEmbeddingProviderDto: CreateEmbeddingProviderDto): Promise<EmbeddingProvider> {
  ...
  async findDefaultProvider(
    ...
    const provider = await this.embeddingService.findDefaultProvider(organizationId);
    
    if (!provider) {
      throw new ForbiddenException(`No default embedding provider found for organization ${organizationId}`);
    }
  ...
  async generateEmbeddings(
    ...
    return this.embeddingService.generateEmbeddings(texts, organizationId, providerId);
  }
}