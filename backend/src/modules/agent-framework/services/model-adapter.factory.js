...
var ModelAdapterFactory = function () {
    ...
    var ModelAdapterFactory = _classThis = /** @class */ (function () {
         ...
         */
        ModelAdapterFactory_1.prototype.registerAdapter = function (providerName, adapter) {
            this.adapters.set(providerName, adapter);
            this.logger.log("Registered model adapter for provider: ".concat(providerName));
        };
        /**
        ...
        ModelAdapterFactory_1.prototype.getAdapter = function (model) {
            ...
            if (!adapter) {
                throw new Error("No adapter found for provider: ".concat(model.provider));
            }
            // Verify that the adapter supports this model
            if (!adapter.supportsModel(model.model)) {
                throw new Error("Adapter for provider ".concat(model.provider, " does not support model: ").concat(model.model));
            }
            return adapter;
        };
        /**
                ...
                return __generator(this, function (_c) {
                    switch (_c.label) {
                        case 0:
                            ...
                            if (config["vertex-ai"]) {
                                initPromises.push(this.vertexAdapter.initialize(config["vertex-ai"]).catch(function (error) {
                                    _this.logger.error("Failed to initialize Vertex AI adapter: ".concat(error.message), error.stack);
                            ...
                            _loop_1 = function (provider, adapter) {
                                ...
                                if (config[provider]) {
                                    initPromises.push(adapter.initialize(config[provider]).catch(function (error) {
                                        _this.logger.error("Failed to initialize ".concat(provider, " adapter: ").concat(error.message), error.stack);
                                        // Don't throw - we want to continue initializing other adapters
                                    }));
                                }
                            };
         ...
         * @throws Error if no adapter is found
         */
        ModelAdapterFactory_1.prototype.getAdapterByProvider = function (providerName) {
