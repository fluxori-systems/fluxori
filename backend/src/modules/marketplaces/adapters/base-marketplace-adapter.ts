...
export abstract class BaseMarketplaceAdapter implements IMarketplaceAdapter {
  ...
  async initialize(credentials: MarketplaceCredentials): Promise<void> {
    ...
    if (!credentials.organizationId) {
      throw new Error(`Organization ID is required for ${this.marketplaceName} adapter`);
    }
    ...
    try {
      ...
      this.logger.log(`${this.marketplaceName} adapter initialized successfully`);
    } catch (error) {
      this.logger.error(`Failed to initialize ${this.marketplaceName} adapter: ${error.message}`, error.stack);
      throw new Error(`Failed to initialize ${this.marketplaceName} adapter: ${error.message}`);
    }
  }
   ...
   */
  protected abstract initializeInternal(credentials: MarketplaceCredentials): Promise<void>;
  ...
  async testConnection(): Promise<ConnectionStatus> {
    ...
    try {
      ...
      this.logger.error(`Connection test to ${this.marketplaceName} failed: ${error.message}`, error.stack);
  ...
  async getMarketplaceHealth(): Promise<ConnectionStatus> {
    try {
      ...
      this.logger.error(`${this.marketplaceName} health check failed: ${error.message}`, error.stack);
  ...
  async close(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error closing ${this.marketplaceName} adapter: ${error.message}`, error.stack);
    }
  }
  ...
  protected checkInitialized(): void {
    if (!this.isInitialized) {
      throw new Error(`${this.marketplaceName} adapter is not initialized. Call initialize() first.`);
    }
  }
   ...
   */
  protected createErrorResult<T>(code: string, message: string, details?: any): OperationResult<T> {
    return {
      success: false,
      error: {
        code,
  ...
  protected handleError<T>(operation: string, error: any): OperationResult<T> {
    ...
    this.logger.error(`${operation} failed for ${this.marketplaceName}: ${errorMessage}`, error.stack);
