...
export class MarketplaceController {
   ...
   */
  @Get('connections')
  @ApiOperation({ summary: 'Get all marketplace connections for the authenticated organization' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
  ...
  async getConnections(@GetUser() user: DecodedFirebaseToken) {
    ...
    const credentials = await this.credentialsRepository.findByOrganization(organizationId);
    const adapters = this.adapterFactory.getAllAdapters();
    
    // Format the response to include marketplace details and connection status
    const connections = credentials.map(credential => {
      const adapter = adapters.find(a => a.marketplaceId === credential.marketplaceId);
  ...
  @ApiParam({ name: 'marketplaceId', description: 'ID of the marketplace to connect to' })
  ...
  async createConnection(
    ...
    try {
      ...
      const updatedCredential = await this.credentialsRepository.updateConnectionStatus(
        savedCredential.id,
      ...
      if (!updatedCredential) {
        this.logger.warn(`Failed to update connection status for credential ${savedCredential.id}`);
      }
      ...
      this.logger.error(`Error creating marketplace connection: ${error.message}`, error.stack);
  ...
  @ApiParam({ name: 'marketplaceId', description: 'ID of the marketplace to test' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
  ...
  async testConnection(
    ...
    try {
      const connectionStatus = await this.adapterFactory.testConnection(
        marketplaceId,
        organizationId,
        credentials ? { ...credentials, organizationId } as MarketplaceCredentials : undefined,
      );
      ...
      this.logger.error(`Error testing marketplace connection: ${error.message}`, error.stack);
  ...
  @ApiOperation({ summary: 'Sync product inventory to all connected marketplaces' })
  ...
  async getMarketplaceProducts(
    ...
    try {
      ...
      const response = await adapter.getProducts(pageNum, pageSizeNum, productFilters);
      
      return response;
    } catch (error) {
      this.logger.error(`Error getting products from marketplace: ${error.message}`, error.stack);
  ...
  @ApiParam({ name: 'marketplaceId', description: 'ID of the marketplace' })
  @ApiParam({ name: 'productId', description: 'ID of the product in the marketplace' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
  ...
  async importProduct(
    ...
    return this.syncService.importProduct(marketplaceId, organizationId, productId);
  }
}