...
export class PimStorageController {
  ...
  constructor(
    ...
    this.cdnDomain = this.configService.get<string>('CDN_DOMAIN') || 'cdn.fluxori.com';
  }
  ...
  @ApiOperation({ summary: 'Generate a signed URL for direct product image upload' })
  @ApiResponse({ status: 201, description: 'Signed URL created', type: ProductImageSignedUrlResponseDto })
  async generateProductImageSignedUploadUrl(
    ...
    if (!validImageTypes.includes(dto.contentType)) {
      throw new BadRequestException(`Invalid image type: ${dto.contentType}. Allowed types: ${validImageTypes.join(', ')}`);
    }

    // Set default values
    const imageType = dto.imageType || 'gallery';
    const position = dto.position || 0;
    const compressionQuality = dto.compressionQuality || CompressionQuality.ADAPTIVE;
    const generateThumbnails = dto.generateThumbnails !== undefined ? dto.generateThumbnails : true;
    const resizeOption = dto.resizeOption || ResizeOption.NONE;
    const optimizeForLowBandwidth = dto.optimizeForLowBandwidth !== undefined ? dto.optimizeForLowBandwidth : true;
    ...
    try {
      ...
      const networkOptimization = {
        ...
        estimatedSizeBytes: this.estimateImageSize(dto.contentType, resizeOption, compressionQuality),
      ...
      const apiDomain = this.configService.get<string>('API_DOMAIN') || 'api.fluxori.com';
  ...
  @ApiQuery({ name: 'type', required: false, description: 'Filter by image type (main, gallery, variant)' })
  async getProductImages(
    ...
    try {
      ...
      this.logger.error(`Failed to get product images: ${error.message}`, error.stack);
      throw new BadRequestException(`Failed to get product images: ${error.message}`);
    }
  }
  ...
  async deleteProductImage(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(`Some thumbnails could not be deleted: ${thumbnailError.message}`);
      }
      
      return { success: true, message: 'Image deleted successfully' };
    } catch (error) {
      this.logger.error(`Failed to delete product image: ${error.message}`, error.stack);
      if (error instanceof NotFoundException) {
        throw error;
      }
      throw new BadRequestException(`Failed to delete product image: ${error.message}`);
    }
  }
   ...
   */
  private generateThumbnailUrls(fileName: string, cdnDomain: string): Record<string, string> {
    const fileNameWithoutExt = fileName.substring(0, fileName.lastIndexOf('.'));
    const fileExt = fileName.substring(fileName.lastIndexOf('.'));
