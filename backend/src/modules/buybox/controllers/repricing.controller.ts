...
export class RepricingController {
  ...
  async getRules(@Param('organizationId') organizationId: string): Promise<RepricingRule[]> {
  ...
  async getActiveRules(@Param('organizationId') organizationId: string): Promise<RepricingRule[]> {
  ...
  async runRepricing(
    ...
    const count = await this.repricingSchedulerService.runRepricingForOrganization(organizationId);
