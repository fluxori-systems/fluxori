...
export class BuyBoxMonitoringService {
  ...
  async updateBuyBoxStatus(
    ...
    this.logger.log(`Updating BuyBox status for product ${productId} on ${marketplaceName}`);
    ...
    // Find BuyBox winner
    const buyBoxWinner = competitorListings.find(listing => listing.isInBuyBox);
    ...
    const statusData: Partial<BuyBoxStatus> = {
      ...
      currency: competitorListings.length > 0 ? competitorListings[0].currency : 'USD',
    ...
    if (status) {
      ...
      status = await this.buyBoxStatusRepository.update(status.id, statusData) as BuyBoxStatus;
    } else {
      // Create new status
      status = await this.buyBoxStatusRepository.create(statusData as any as BuyBoxStatus);
    }
  ...
  private calculateMarketPosition(
    ...
    const priceDifferencePercentage = cheapestPrice > 0 
      ? (priceDifference / cheapestPrice) * 100 
      : 0;
