...
export class RepricingSchedulerService {
  ...
  async runScheduledRepricing() {
    ...
    try {
      ...
      this.logger.error(
        `Error running scheduled repricing: ${error.message}`,
        error.stack,
      );
    } finally {
  ...
  async runRepricingForOrganization(organizationId: string): Promise<number> {
    ...
    const statuses =
      await this.buyBoxStatusRepository.findMonitored(organizationId);

    this.logger.log(
      `Found ${statuses.length} monitored products for organization ${organizationId}`,
    );
    ...
    this.logger.log(
      `Processed ${processedCount} products for organization ${organizationId}`,
    );
  ...
  private async cleanupOldHistory(organizationId: string): Promise<number> {
    ...
    // Delete old history
    return this.buyBoxHistoryRepository.deleteOlderThan(
      organizationId,
      cutoffDate,
    );
  }
