...
export class RepricingEngineService {
  ...
  async getActiveRules(organizationId: string): Promise<RepricingRule[]> {
    ...
    const rules =
      await this.repricingRuleRepository.findByOrganization(organizationId);
  ...
  async updateRule(
    id: string,
    ruleData: Partial<RepricingRule>,
  ...
  async deleteRule(id: string): Promise<boolean> {
    ...
    try {
      await this.repricingRuleRepository.delete(id);
      return true;
    } catch (error) {
  ...
  async applyRules(
    ...
    this.logger.log(
      `Applying repricing rules for product ${productId} on marketplace ${marketplaceId}`,
    );
    ...
    if (rules.length === 0) {
      this.logger.log(
        `No active repricing rules found for organization ${organizationId}`,
      );
      return [];
    }

    // Get BuyBox status for the product
    const buyBoxStatus = await this.buyBoxMonitoringService.getBuyBoxStatus(
      productId,
      marketplaceId,
    );
      ...
      try {
        ...
        switch (rule.operation) {
          case PricingRuleOperation.MATCH:
            if (
              buyBoxStatus.competitors &&
              buyBoxStatus.competitors.length > 0
            ) {
              // Find the lowest competitor price
              const lowestCompetitorPrice = Math.min(
                ...buyBoxStatus.competitors.map(
                  (cp: CompetitorPrice) => cp.totalPrice,
                ),
              );
