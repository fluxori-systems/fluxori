...
export class WantitallConnector extends BaseMarketplaceConnector {
  ...
  protected async initializeInternal(credentials: ConnectorCredentials): Promise<void> {
    ...
    this.apiClient = new NetworkAwareClient(
      this.baseUrl,
      {
        timeout: 30000,
        headers: {
  ...
  protected async testConnectionInternal(): Promise<ConnectionStatus> {
    try {
      ...
      this.logger.error(`Wantitall connection test failed: ${error.message}`, error.stack);
  ...
  protected async getProductByIdInternal(productId: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      if (response.status === 200 && response.data) {
        return this.createSuccessResult(this.mapWantitallProduct(response.data));
      } else {
  ...
  protected async getProductBySkuInternal(sku: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      if (response.status === 200 && response.data?.items?.length > 0) {
        return this.createSuccessResult(this.mapWantitallProduct(response.data.items[0]));
      } else {
        return this.createErrorResult(
          'PRODUCT_NOT_FOUND',
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        const products = Array.isArray(response.data.items)
          ? response.data.items.map((item: any) => this.mapWantitallProduct(item))
          : [];
  ...
  protected async getProductsInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceProduct>> {
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        return {
          ...
          pagination: {
            ...
            totalPages: Math.ceil((response.data.total_items || products.length) / (options.pageSize || 20)),
            hasNextPage: ((options.page || 0) + 1) * (options.pageSize || 20) < (response.data.total_items || products.length)
          }
        };
      } else {
        return {
      ...
      this.logger.error(`Failed to fetch products from Wantitall: ${error.message}`, error.stack);
  ...
  protected async getRecentOrdersInternal(
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        return {
          ...
          pagination: {
            ...
            totalPages: Math.ceil((response.data.total_items || orders.length) / (options.pageSize || 20)),
            hasNextPage: ((options.page || 0) + 1) * (options.pageSize || 20) < (response.data.total_items || orders.length)
          }
        };
      } else {
        return {
      ...
      this.logger.error(`Failed to fetch recent orders from Wantitall: ${error.message}`, error.stack);
  ...
  protected async getOrdersInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    try {
      ...
      this.logger.error(`Failed to fetch orders from Wantitall: ${error.message}`, error.stack);
    ...
    try {
      ...
      if (response.status === 200) {
        return this.createSuccessResult({
          ...
          success: true,
          timestamp: new Date(),
          marketplaceReference: response.data?.reference || `WIA-ACK-${orderId}`
        });
      } else {
        return this.createErrorResult(
  ...
  protected async updateStockInternal(updates: Array<{
    ...
    try {
      ...
      const response = await this.apiClient.post('/inventory/update', updateData);
      ...
      if (error.response?.data?.results && Array.isArray(error.response.data.results)) {
        const results = error.response.data.results;
        const successful = results.filter((r: any) => r.success).map((r: any) => r.sku);
        const failed = results.filter((r: any) => !r.success).map((r: any) => ({
          sku: r.sku,
  ...
  private getErrorCodeForType(type: ConnectorErrorType): string {
    switch (type) {
      case ConnectorErrorType.AUTHENTICATION: return 'AUTH_ERROR';
      case ConnectorErrorType.AUTHORIZATION: return 'PERMISSION_DENIED';
      case ConnectorErrorType.RATE_LIMIT: return 'RATE_LIMIT_EXCEEDED';
      case ConnectorErrorType.SERVER_ERROR: return 'SERVER_ERROR';
      case ConnectorErrorType.NETWORK: return 'NETWORK_ERROR';
      case ConnectorErrorType.TIMEOUT: return 'TIMEOUT_ERROR';
      case ConnectorErrorType.VALIDATION: return 'VALIDATION_ERROR';
      case ConnectorErrorType.NOT_FOUND: return 'NOT_FOUND';
      case ConnectorErrorType.UNSUPPORTED: return 'UNSUPPORTED_OPERATION';
      case ConnectorErrorType.LOAD_SHEDDING: return 'LOAD_SHEDDING_ERROR';
  ...
  private mapWantitallProduct(wantitallProduct: any): MarketplaceProduct {
    return {
      ...
      createdAt: new Date(wantitallProduct.created_at || wantitallProduct.date_created || Date.now()),
      updatedAt: new Date(wantitallProduct.updated_at || wantitallProduct.date_modified || Date.now())
    };
  }
  ...
  private mapWantitallOrder(wantitallOrder: any): MarketplaceOrder {
    ...
    const shippingAddress = wantitallOrder.shipping_address || wantitallOrder.shipping;
    const billingAddress = wantitallOrder.billing_address || wantitallOrder.billing;
    const orderItems = wantitallOrder.items || wantitallOrder.line_items || [];
    
    return {
      id: wantitallOrder.id.toString(),
      marketplaceOrderId: wantitallOrder.reference || wantitallOrder.order_number || wantitallOrder.id.toString(),
      status: this.mapOrderStatus(wantitallOrder.status),
      createdAt: new Date(wantitallOrder.created_at || wantitallOrder.date_created || Date.now()),
      updatedAt: new Date(wantitallOrder.updated_at || wantitallOrder.date_modified || Date.now()),
      totalPrice: parseFloat(wantitallOrder.total || wantitallOrder.total_price || 0),
      subtotalPrice: parseFloat(wantitallOrder.subtotal || wantitallOrder.subtotal_price || 0),
      totalShipping: parseFloat(wantitallOrder.shipping_cost || wantitallOrder.shipping_price || 0),
      totalTax: parseFloat(wantitallOrder.tax_amount || wantitallOrder.tax || 0),
      currency: wantitallOrder.currency || wantitallOrder.currency_code || 'ZAR',
      customerName: wantitallOrder.customer_name || 
        `${wantitallOrder.customer?.name || ''} ${wantitallOrder.customer?.surname || ''}`.trim(),
      customerEmail: wantitallOrder.customer_email || wantitallOrder.customer?.email,
      items: Array.isArray(orderItems) 
        ? orderItems.map((item: any) => ({
            ...
            totalPrice: parseFloat(item.total || (item.quantity * item.price)) || 0
          })) 
        : [],
   ...
   */
  private mapProductStatus(status: string): 'active' | 'inactive' | 'pending' | 'rejected' {
