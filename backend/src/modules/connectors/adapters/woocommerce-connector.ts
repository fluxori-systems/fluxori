...
export class WooCommerceConnector extends BaseMarketplaceConnector {
  ...
  protected async initializeInternal(credentials: ConnectorCredentials): Promise<void> {
    ...
    if (!credentials.apiKey || !credentials.apiSecret) {
      throw new Error('WooCommerce connector requires consumerKey and consumerSecret');
    }
    ...
    this.client.interceptors.response.use(response => {
      ...
      if (response.headers['x-ratelimit-limit']) {
        this.rateLimitTotal = parseInt(response.headers['x-ratelimit-limit'], 10);
      }
  ...
  private convertToMarketplaceProduct(product: any): MarketplaceProduct {
    ...
    if (product.status === 'publish') status = 'active' as 'active';
    else if (product.status === 'draft') status = 'pending' as 'pending';
    ...
    return {
      ...
      stockLevel: product.stock_quantity !== null ? product.stock_quantity : (product.in_stock ? 999 : 0),
      status: status,
      categories,
      images,
  ...
  private convertToMarketplaceOrder(order: any): MarketplaceOrder {
    ...
    return {
      ...
      dateCompleted: order.date_completed ? new Date(order.date_completed) : undefined,
  ...
  protected async getProductByIdInternal(productId: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      return this.createSuccessResult(this.convertToMarketplaceProduct(product));
    } catch (error) {
  ...
  protected async getProductBySkuInternal(sku: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      if (response && response.length > 0) {
        return this.createSuccessResult(this.convertToMarketplaceProduct(response[0]));
      }
  ...
  protected async getProductsBySkusInternal(skus: string[]): Promise<OperationResult<MarketplaceProduct[]>> {
    ...
    try {
      ...
      for (let i = 0; i < skus.length; i += batchSize) {
        ...
        const batchPromises = batch.map(sku => this.getProductBySkuInternal(sku));
  ...
  protected async getProductsInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceProduct>> {
    try {
      ...
      const response = await this.request<any[]>('get', 'products', undefined, params);
      
      // Extract total pages and items from headers
      const totalItems = parseInt(this.client.defaults.headers['X-WP-Total'] as string, 10) || 0;
      const totalPages = parseInt(this.client.defaults.headers['X-WP-TotalPages'] as string, 10) || 1;
      
      // Convert to marketplace format
      const products = response.map(product => this.convertToMarketplaceProduct(product));
  ...
  protected async getRecentOrdersInternal(sinceDate: Date, options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    try {
      ...
      // Get orders from API
      const response = await this.request<any[]>('get', 'orders', undefined, params);
      ...
      // Convert to marketplace format
      const orders = response.map(order => this.convertToMarketplaceOrder(order));
      ...
      this.logger.error(`Error fetching recent orders: ${error.message}`, error.stack);
  ...
  protected async updateStockInternal(updates: Array<{
    ...
    try {
      ...
      for (let i = 0; i < updates.length; i += batchSize) {
        ...
        const productPromises = batch.map(update => this.getProductBySkuInternal(update.sku));
      ...
      const remainingSkus = updates
        .map(u => u.sku)
        .filter(sku => !successful.includes(sku) && !failed.some(f => f.sku === sku));
   ...
   */
  async getLeadTime(productId: string): Promise<OperationResult<{ leadTimeDays: number }>> {
    try {
      const product = await this.request<any>('get', `products/${productId}`);
      ...
      if (product.meta_data) {
        const leadTimeMeta = product.meta_data.find((meta: any) => 
   ...
   */
  async updateLeadTime(productId: string, leadTimeDays: number): Promise<OperationResult<{ updated: boolean }>> {
    try {
   ...
   */
  async getSouthAfricanShippingRates(): Promise<OperationResult<Array<{
    name: string;
    price: number;
    ...
    try {
      ...
      const saZone = shippingZones.find(zone => 
      ...
      const methods = await this.request<any[]>('get', `shipping/zones/${saZone.id}/methods`);
      ...
      this.logger.warn(`Error fetching shipping rates: ${error.message}. Using defaults.`);
   ...
   */
  async getLowBandwidthProducts(options?: PaginationOptions): Promise<PaginatedResponse<MarketplaceProduct>> {
    ...
    try {
      ...
      const params = {
        ...
        order: options?.sortDirection === 'asc' ? 'asc' : 'desc',
        orderby: this.mapSortField(options?.sortBy),
        _fields: 'id,name,sku,price,stock_quantity,status,date_created,date_modified'
      };
      ...
      const products = response.map(product => ({
        ...
        price: parseFloat(product.price || 0),
        currency: 'ZAR',
        stockLevel: product.stock_quantity !== null ? product.stock_quantity : 0,
        status: product.status === 'publish' ? 'active' as 'active' : 'inactive' as 'inactive',
        createdAt: new Date(product.date_created || Date.now()),
        updatedAt: new Date(product.date_modified || Date.now())
      }));
      ...
      this.logger.error(`Error fetching low bandwidth products: ${error.message}`, error.stack);
