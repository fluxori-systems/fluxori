...
export class MakroConnector extends BaseMarketplaceConnector {
  ...
  protected async testConnectionInternal(): Promise<ConnectionStatus> {
    try {
      ...
      this.logger.error(`Makro connection test failed: ${error.message}`, error.stack);
  ...
  protected async getProductsInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceProduct>> {
    try {
      ...
      this.logger.error(`Failed to fetch products from Makro: ${error.message}`, error.stack);
  ...
  protected async getRecentOrdersInternal(
    ...
    try {
      ...
      this.logger.error(`Failed to fetch recent orders from Makro: ${error.message}`, error.stack);
  ...
  protected async getOrdersInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    try {
      ...
      this.logger.error(`Failed to fetch orders from Makro: ${error.message}`, error.stack);
   ...
   */
  async addToPromotion(productSku: string, promotionData: {
    promotionId: string;
   ...
   */
  async checkStorePickupEligibility(productSku: string, storeId?: string): Promise<OperationResult<{
    eligible: boolean;
    ...
    try {
      ...
      return this.handleError(error, `checkStorePickupEligibility(${productSku})`);
    }
  }
   ...
   */
  async getStoreLocations(): Promise<OperationResult<Array<{
    storeId: string;
    name: string;
    region: string;
    address: string;
