...
export class SuperbalistConnector extends BaseMarketplaceConnector {
  ...
  protected async testConnectionInternal(): Promise<ConnectionStatus> {
    try {
      ...
      this.logger.error(`Superbalist connection test failed: ${error.message}`, error.stack);
  ...
  protected async getProductByIdInternal(productId: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      if (response.status === 200 && response.data) {
        return this.createSuccessResult(this.mapSuperbalistProduct(response.data));
      } else {
  ...
  protected async getProductBySkuInternal(sku: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      if (response.status === 200 && response.data?.data?.length > 0) {
        return this.createSuccessResult(this.mapSuperbalistProduct(response.data.data[0]));
      } else {
        return this.createErrorResult(
          'PRODUCT_NOT_FOUND',
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        const products = Array.isArray(response.data.data) 
          ? response.data.data.map((product: any) => this.mapSuperbalistProduct(product))
          : [];
        ...
        return {
          ...
          pagination: {
            ...
            hasNextPage: (response.data.meta?.current_page || 1) < (response.data.meta?.last_page || 1)
          }
        };
      } else {
        return {
      ...
      this.logger.error(`Failed to fetch products from Superbalist: ${error.message}`, error.stack);
  ...
  protected async getOrderByIdInternal(id: string): Promise<OperationResult<MarketplaceOrder>> {
    try {
      ...
      if (response.status === 200 && response.data) {
        return this.createSuccessResult(this.mapSuperbalistOrder(response.data));
      } else {
        return this.createErrorResult(
          'ORDER_NOT_FOUND',
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        const orders = Array.isArray(response.data.data)
          ? response.data.data.map((order: any) => this.mapSuperbalistOrder(order))
          : [];
      ...
      this.logger.error(`Failed to fetch recent orders from Superbalist: ${error.message}`, error.stack);
  ...
  protected async getOrdersInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    try {
      ...
      this.logger.error(`Failed to fetch orders from Superbalist: ${error.message}`, error.stack);
    ...
    try {
      ...
      if (response.status === 200) {
        return this.createSuccessResult({
          ...
          success: true,
          timestamp: new Date(),
          marketplaceReference: response.data?.reference || `SPBL-ACK-${orderId}`
        });
      } else {
        return this.createErrorResult(
  ...
  protected async updateStockInternal(updates: Array<{
    ...
    try {
      ...
      const response = await this.apiClient.post('/products/stock', { updates: updateData });
      ...
      if (error.response?.data?.failed && Array.isArray(error.response.data.failed)) {
        ...
        const successfulSkus = allSkus.filter(sku => 
          !failedSkus.some((f: { sku: string }) => f.sku === sku)
        );
  ...
  protected async updatePricesInternal(updates: Array<{
    ...
    try {
      ...
      const response = await this.apiClient.post('/products/prices', { updates: updateData });
  ...
  private mapSuperbalistOrder(superbalistOrder: any): MarketplaceOrder {
    return {
      ...
      marketplaceOrderId: superbalistOrder.reference || superbalistOrder.id.toString(),
      ...
      customerName: `${superbalistOrder.customer?.first_name || ''} ${superbalistOrder.customer?.last_name || ''}`.trim(),
