...
export class BobShopConnector extends BaseMarketplaceConnector {
  ...
  protected async testConnectionInternal(): Promise<ConnectionStatus> {
    try {
      ...
      this.logger.error(`Bob Shop connection test failed: ${error.message}`, error.stack);
  ...
  protected async getProductsInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceProduct>> {
    try {
      ...
      this.logger.error(`Failed to fetch products from Bob Shop: ${error.message}`, error.stack);
  ...
  protected async getRecentOrdersInternal(
    ...
    try {
      ...
      this.logger.error(`Failed to fetch recent orders from Bob Shop: ${error.message}`, error.stack);
  ...
  protected async getOrdersInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    try {
      ...
      this.logger.error(`Failed to fetch orders from Bob Shop: ${error.message}`, error.stack);
      ...
      const categories: MarketplaceCategory[] = [
        {
          id: 'electronics',
          name: 'Electronics',
          path: parentId ? ['BobShop', parentId, 'Electronics'] : ['BobShop', 'Electronics'],
        ...
        },
        {
          id: 'fashion',
          name: 'Fashion',
          path: parentId ? ['BobShop', parentId, 'Fashion'] : ['BobShop', 'Fashion'],
          parentId: parentId || undefined,
          level: parentId ? 1 : 0,
          isActive: true
        },
        {
          id: 'home',
          name: 'Home & Garden',
          path: parentId ? ['BobShop', parentId, 'Home & Garden'] : ['BobShop', 'Home & Garden'],
          parentId: parentId || undefined,
          level: parentId ? 1 : 0,
          isActive: true
        }
