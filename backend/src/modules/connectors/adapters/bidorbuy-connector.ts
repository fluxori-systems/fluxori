/**
 * Bidorbuy Marketplace Connector
 * 
 * This connector integrates with the Bidorbuy Seller API to provide
...
/**
 * Bidorbuy API Connector
 * 
 * This class implements the marketplace connector interface for Bidorbuy Seller API
 */
@Injectable()
export class BidorbuyConnector extends BaseMarketplaceConnector {
  readonly connectorId: string = 'bidorbuy';
  readonly connectorName: string = 'Bidorbuy Seller API';
  
  private apiClient: NetworkAwareClient;
  private baseUrl: string = 'https://api.bidorbuy.co.za/v2';
  private apiKey: string;
  private apiSecret: string;
  private organizationId: string;
  
  constructor() {
    super('BidorbuyConnector');
  }
  ...
  protected async initializeInternal(credentials: ConnectorCredentials): Promise<void> {
    if (!credentials.apiKey || !credentials.apiSecret) {
      throw new Error('Bidorbuy API requires both apiKey and apiSecret');
    }
    ...
    this.logger.log('Bidorbuy connector initialized');
  }
  ...
  protected async testConnectionInternal(): Promise<ConnectionStatus> {
    try {
      ...
      return {
        ...
        message: 'Connection to Bidorbuy API successful',
        lastChecked: new Date()
      };
    } catch (error) {
      this.logger.error(`Bidorbuy connection test failed: ${error.message}`, error.stack);
  ...
  /**
   * Get rate limit status from Bidorbuy API
   */
  ...
  protected async getProductByIdInternal(productId: string): Promise<OperationResult<MarketplaceProduct>> {
    try {
      ...
      return this.createSuccessResult({
        id: productId,
        sku: `BOB-${productId}`,
        name: 'Bidorbuy Product',
        description: 'This is a placeholder for the Bidorbuy API integration',
        price: 89.99,
        currency: 'ZAR',
        stockLevel: 50,
  ...
  protected async getProductsInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceProduct>> {
    try {
      ...
      this.logger.error(`Failed to fetch products from Bidorbuy: ${error.message}`, error.stack);
  ...
  protected async getRecentOrdersInternal(
    ...
    try {
      ...
      const demoOrders = Array(options.pageSize || 10).fill(0).map((_, i) => ({
        ...
        status: 'processing',
        createdAt: new Date(Math.max(sinceDate.getTime(), Date.now() - i * 86400000)), // Each order 1 day apart, but not before sinceDate
        updatedAt: new Date(Math.max(sinceDate.getTime(), Date.now() - i * 86400000 + 3600000)), // Updated 1 hour after creation
        totalPrice: 149.99,
        subtotalPrice: 129.99,
        currency: 'ZAR',
        customerName: 'Jane Smith',
      ...
      this.logger.error(`Failed to fetch recent orders from Bidorbuy: ${error.message}`, error.stack);
  ...
  protected async getOrdersInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    try {
      ...
      this.logger.error(`Failed to fetch orders from Bidorbuy: ${error.message}`, error.stack);
  ...
  // Bidorbuy specific methods for auction functionality
   ...
   */
  async createAuction(productSku: string, auctionData: {
    startPrice: number;
    reservePrice?: number;
   ...
   */
  async getAuctionStatus(auctionId: string): Promise<OperationResult<{
    auctionId: string;
    productSku: string;
    currentBid: number;
    bidCount: number;
      ...
      const categories: MarketplaceCategory[] = [
        {
          id: 'electronics',
          name: 'Electronics',
          path: parentId ? ['Bidorbuy', parentId, 'Electronics'] : ['Bidorbuy', 'Electronics'],
        ...
        },
        {
          id: 'fashion',
          name: 'Fashion',
          path: parentId ? ['Bidorbuy', parentId, 'Fashion'] : ['Bidorbuy', 'Fashion'],
          parentId: parentId || undefined,
          level: parentId ? 1 : 0,
          isActive: true
        },
        {
          id: 'home',
          name: 'Home & Garden',
          path: parentId ? ['Bidorbuy', parentId, 'Home & Garden'] : ['Bidorbuy', 'Home & Garden'],
          parentId: parentId || undefined,
          level: parentId ? 1 : 0,
          isActive: true
        }
