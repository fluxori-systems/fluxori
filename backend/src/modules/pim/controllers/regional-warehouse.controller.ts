...
export class RegionalWarehouseController {
  ...
  async getWarehouses(@GetUser() user: any): Promise<any> {
    try {
      const warehouses =
        await this.regionalWarehouseService.getRegionalWarehouses(
          user.organizationId,
        );

      // Group warehouses by region for better organization
      const groupedByRegion = warehouses.reduce(
        (groups, warehouse) => {
      ...
      if (error.message.includes('not enabled')) {
        throw new BadRequestException(
          'Multi-warehouse support is not enabled for your organization',
        );
      }
      throw new BadRequestException(
  ...
  async getWarehousesByRegion(
    ...
    try {
      const warehouses =
        await this.regionalWarehouseService.getWarehousesByRegion(
          user.organizationId,
          region,
        );
      ...
      this.logger.error(
        `Error getting warehouses for region ${region}: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to retrieve warehouses: ${error.message}`,
      );
  ...
  async findWarehouses(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to search warehouses: ${error.message}`,
      );
    }
  }
  ...
  async getDefaultWarehouseForRegion(
    ...
    try {
      const warehouse =
        await this.regionalWarehouseService.getDefaultWarehouseForRegion(
          user.organizationId,
          region,
        );
      ...
      this.logger.error(
        `Error getting default warehouse for region ${region}: ${error.message}`,
      );
      if (error instanceof NotFoundException) {
        throw error;
      }
      throw new BadRequestException(
        `Failed to get default warehouse: ${error.message}`,
      );
    }
  }
  ...
  async setDefaultWarehouseForRegion(
    ...
    try {
      const warehouse =
        await this.regionalWarehouseService.setDefaultWarehouseForRegion(
          user.organizationId,
          warehouseId,
          region,
        );
      ...
      throw new BadRequestException(
        `Failed to set default warehouse: ${error.message}`,
      );
    }
  }
  ...
  async getOptimalWarehouse(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to find optimal warehouse: ${error.message}`,
      );
    }
  }
  ...
  async canWarehouseFulfillProduct(
    ...
    try {
      const result =
        await this.regionalWarehouseService.canWarehouseFulfillProduct(
          user.organizationId,
          warehouseId,
          productId,
          destinationCountry,
      ...
      this.logger.error(
        `Error checking fulfillment capability: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to check fulfillment capability: ${error.message}`,
      );
    }
  }
  ...
  async getSupportedCountries(@GetUser() user: any): Promise<any> {
    try {
      const countries =
        await this.regionalWarehouseService.getSupportedCountries(
          user.organizationId,
        );
      ...
      const countryNames = await Promise.all(
        countries.map(async (country) => {
          try {
            ...
            const context =
              await this.marketContextService.getMarketContextByRegion(
                'africa',
                country,
              );
            return {
  ...
  async getWarehouseCurrencies(@GetUser() user: any): Promise<any> {
    try {
      const currencies =
        await this.regionalWarehouseService.getWarehouseCurrencies(
          user.organizationId,
        );
      ...
      throw new BadRequestException(
        `Failed to get warehouse currencies: ${error.message}`,
      );
    }
  }
  ...
  async updateWarehouseRegionalSettings(
    ...
    try {
      const warehouse =
        await this.regionalWarehouseService.updateWarehouseRegionalSettings(
          user.organizationId,
          warehouseId,
          updates,
        );
      ...
      this.logger.error(
        `Error updating warehouse regional settings: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to update warehouse regional settings: ${error.message}`,
      );
    }
  }
