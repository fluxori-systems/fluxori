...
import { BulkOperationOptions, BulkOperationStats } from '../services/bulk-operations/bulk-operations.service';
...
export class BulkOperationsController {
  ...
  private async getBulkOptions(
    ...
    const enableLoadSheddingResilience = await this.featureFlagService.isEnabled(
  ...
  async bulkUpdateProducts(
    ...
    @GetUser() user: User
  ): Promise<{ results: Array<OperationResult<Product>>, stats: BulkOperationStats }> {
    ...
    return await this.productBulkService.bulkUpdateProducts(operations, options);
  }
  ...
  async bulkDeleteProducts(
    ...
    this.logger.log(`Bulk delete request for ${body.productIds.length} products`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.productBulkService.bulkDeleteProducts(user.organizationId, body.productIds, options);
  }
  ...
  async bulkUpdateProductStatus(
    ...
    this.logger.log(`Bulk status update request for ${operations.length} products`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.productBulkService.bulkUpdateProductStatus(user.organizationId, operations, options);
  }
  ...
  async bulkUpdateProductPrices(
    ...
    this.logger.log(`Bulk price update request for ${operations.length} products`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.productBulkService.bulkUpdateProductPrices(user.organizationId, operations, options);
  }
  ...
  async bulkUpdateProductCategories(
    ...
    this.logger.log(`Bulk category update request for ${operations.length} products`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.productBulkService.bulkUpdateProductCategories(user.organizationId, operations, options);
  }
  ...
  async bulkUpdateProductInventory(
    ...
    this.logger.log(`Bulk inventory update request for ${operations.length} products`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.productBulkService.bulkUpdateProductInventory(user.organizationId, operations, options);
  }
  ...
  async bulkDuplicateProducts(
    ...
    this.logger.log(`Bulk duplication request for ${operations.length} products`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.productBulkService.bulkDuplicateProducts(user.organizationId, operations, options);
  }
  ...
  async bulkUpdateCategories(
    ...
    return await this.categoryBulkService.bulkUpdateCategories(operations, options);
  }
  ...
  async bulkDeleteCategories(
    ...
    this.logger.log(`Bulk delete request for ${body.categoryIds.length} categories`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.categoryBulkService.bulkDeleteCategories(user.organizationId, body.categoryIds, options);
  }
  ...
  async bulkMoveCategories(
    ...
    return await this.categoryBulkService.bulkMoveCategories(user.organizationId, operations, options);
  }
  ...
  async bulkUpdateCategoryMarketplaceMappings(
    ...
    this.logger.log(`Bulk marketplace mapping update request for ${operations.length} categories`);
    
    const options = await this.getBulkOptions(user, queryParams);
    
    return await this.categoryBulkService.bulkUpdateCategoryMarketplaceMappings(user.organizationId, operations, options);
  }
}