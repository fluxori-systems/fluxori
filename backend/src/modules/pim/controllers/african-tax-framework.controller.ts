...
import { AfricanTaxFrameworkService, AfricanTaxRule } from '../services/african-tax-framework.service';
...
export class AfricanTaxFrameworkController {
  ...
  async getSupportedCountries(): Promise<string[]> {
    try {
      ...
      this.logger.error(`Error getting supported countries: ${error.message}`, error.stack);
  ...
  async getCountryTaxRules(@Param('countryCode') countryCode: string): Promise<AfricanTaxRule> {
    try {
      const rules = this.africanTaxFrameworkService.getCountryTaxRules(countryCode);
      ...
      this.logger.error(`Error getting country tax rules: ${error.message}`, error.stack);
   ...
   */
  @Post('calculate')
  async calculateAfricanTax(@Body() request: TaxRateRequest): Promise<TaxRateResult> {
    try {
      return await this.africanTaxFrameworkService.calculateAfricanTax(request);
    } catch (error) {
      this.logger.error(`Error calculating African tax: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  async getSpecialProductTaxCategories(
    ...
    try {
      return await this.africanTaxFrameworkService.getSpecialProductTaxCategories(countryCode);
    } catch (error) {
      this.logger.error(`Error getting special product tax categories: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  async isProductTaxExempt(
    ...
    try {
      ...
      this.logger.error(`Error checking product tax exemption: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  async getUpcomingRateChanges(
    ...
    try {
      return await this.africanTaxFrameworkService.getUpcomingRateChanges(countryCode);
    } catch (error) {
      this.logger.error(`Error getting upcoming rate changes: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  async isCountrySupported(
    ...
    try {
      const supported = this.africanTaxFrameworkService.isCountrySupported(countryCode);
      return { supported };
    } catch (error) {
      this.logger.error(`Error checking country support: ${error.message}`, error.stack);
      throw new HttpException(
