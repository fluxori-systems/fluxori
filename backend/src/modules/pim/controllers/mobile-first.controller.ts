...
} from '../services/mobile-first-detection.service';
...
export class MobileFirstController {
  ...
  async getProducts(
    ...
    const deviceProfile =
      this.mobileDetectionService.detectDeviceProfile(request);

    // Get optimization options based on profile
    const optimizationOptions =
      this.mobileDetectionService.getOptimizationOptions(deviceProfile);
    ...
    try {
      ...
      const mobileResponse: MobileResponse<any> = {
        ...
        meta: {
          optimized: true,
          clientHints:
            this.mobileDetectionService.generateViewportHints(deviceProfile),
          loadShedding,
      ...
      this.logger.error(
        `Error fetching products for mobile: ${error.message}`,
        error.stack,
      );
  ...
  async getProduct(
    ...
    try {
      ...
      this.logger.error(
        `Error fetching product for mobile: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
  ...
  async getCategories(
    ...
    try {
      ...
      this.logger.error(
        `Error fetching categories for mobile: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
  ...
  async getOptimizedImage(
    ...
    const imageWidth = width
      ? parseInt(width, 10)
      : optimizationOptions.imageTransform.maxWidth;
    const imageHeight = height
      ? parseInt(height, 10)
      : optimizationOptions.imageTransform.maxHeight;
   ...
   */
  @Get('device-profile')
  async getDeviceProfile(@Req() request: Request, @Res() response: Response) {
    // Detect device profile
    const deviceProfile =
      this.mobileDetectionService.detectDeviceProfile(request);
  ...
  async getMarketContext(
    ...
    const marketContext = await this.marketContextService.getMarketContext(
      user.tenantId,
    );
  ...
  private setCacheHeaders(
    ...
    response.header(
      'Vary',
      'User-Agent, Accept, Save-Data, Viewport-Width, DPR',
    );
  }
   ...
   */
  private getSouthAfricanUssdCodes(
