...
import { ProductReview, ProductReviewStatus, ReviewSource } from '../models/product-review.model';
...
export class ProductReviewController {
  ...
  async createReview(@Body() dto: CreateReviewDto, @Req() req: any): Promise<ProductReview> {
    try {
      const organizationId = req.user.organizationId;
      const userId = req.user.uid;
      
      // Map DTO to review model
      const reviewData: Omit<ProductReview, 'id' | 'createdAt' | 'updatedAt' | 'organizationId' | 'status'> = {
      ...
      return await this.reviewService.createReview(reviewData, organizationId, userId);
    } catch (error) {
  ...
  async updateReview(
    ...
    try {
      ...
      return await this.reviewService.updateReview(id, dto, organizationId, userId);
    } catch (error) {
  ...
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Review not found' })
  async getReviewById(@Param('id') id: string, @Req() req: any): Promise<ProductReview> {
    const organizationId = req.user.organizationId;
  ...
  @ApiQuery({ name: 'limit', description: 'Maximum number of reviews to return', required: false })
  @ApiQuery({ name: 'offset', description: 'Number of reviews to skip', required: false })
  ...
  async getPublicReviewsForProduct(
    ...
    try {
      ...
      if (loadSheddingStatus.currentStage > 2 || networkQuality.quality === 'low') {
  ...
  @ApiParam({ name: 'id', description: 'Review ID' })
  @ApiQuery({ name: 'helpful', description: 'Whether the review was helpful', required: true })
  @ApiResponse({ status: HttpStatus.NO_CONTENT, description: 'Vote recorded successfully' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Review not found' })
  async recordHelpfulnessVote(
    ...
    try {
      ...
      await this.reviewService.recordHelpfulnessVote(id, isHelpful, organizationId);
    } catch (error) {
  ...
  @ApiParam({ name: 'id', description: 'Review ID' })
  @ApiResponse({ status: HttpStatus.NO_CONTENT, description: 'Report recorded successfully' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Review not found' })
  ...
  async getReviewStatistics(
    ...
    try {
      ...
      return await this.reviewService.getReviewStatistics(productId, organizationId);
    } catch (error) {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Reviews imported successfully' })
  async importReviews(
    ...
    try {
      ...
      return await this.reviewService.importReviews(dto.reviews, organizationId, userId);
    } catch (error) {
  ...
  async getReviewDashboardStatistics(
    ...
    return await this.reviewService.getReviewDashboardStatistics(organizationId);
  }
  ...
  @ApiParam({ name: 'productId', description: 'Product ID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Sentiment insights found' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found' })
  async getReviewSentimentInsights(
    ...
    try {
      ...
      return await this.reviewService.getReviewSentimentInsights(productId, organizationId);
    } catch (error) {
  ...
  @ApiQuery({ name: 'batchSize', description: 'Number of reviews to process', required: false })
  ...
  async processSentimentAnalysisBatch(
    ...
    try {
      ...
      const processedCount = await this.reviewService.processPendingSentimentAnalysisBatch(
        organizationId,
