...
export class ProductController {
  ...
  async findProducts(
    ...
    // Parse arrays
    if (filterParams.categoryIds && typeof filterParams.categoryIds === 'string') {
      filter.categoryIds = filterParams.categoryIds.split(',');
    }
    
    if (filterParams.marketplaceIds && typeof filterParams.marketplaceIds === 'string') {
    ...
    if (filterParams.minPrice || filterParams.maxPrice) {
      filter.priceRange = {};
      if (filterParams.minPrice) filter.priceRange.min = parseFloat(filterParams.minPrice);
      if (filterParams.maxPrice) filter.priceRange.max = parseFloat(filterParams.maxPrice);
    }
    
    // Parse dates
    if (filterParams.createdFrom || filterParams.createdTo) {
      filter.createdAt = {};
      if (filterParams.createdFrom) filter.createdAt.from = new Date(filterParams.createdFrom);
      if (filterParams.createdTo) filter.createdAt.to = new Date(filterParams.createdTo);
    }
    
    if (filterParams.updatedFrom || filterParams.updatedTo) {
      filter.updatedAt = {};
      if (filterParams.updatedFrom) filter.updatedAt.from = new Date(filterParams.updatedFrom);
      if (filterParams.updatedTo) filter.updatedAt.to = new Date(filterParams.updatedTo);
    }
    
    // Parse South African compliance filters
    if (filterParams.icasa !== undefined || filterParams.sabs !== undefined || filterParams.nrcs !== undefined) {
      filter.complianceStatus = {};
      if (filterParams.icasa !== undefined) filter.complianceStatus.icasa = filterParams.icasa === 'true';
      if (filterParams.sabs !== undefined) filter.complianceStatus.sabs = filterParams.sabs === 'true';
      if (filterParams.nrcs !== undefined) filter.complianceStatus.nrcs = filterParams.nrcs === 'true';
    }
  ...
  async getRecentlyUpdatedProducts(
    ...
    const organizationId = user.organizationId;
    return await this.productService.getRecentlyUpdatedProducts(organizationId, limit);
  }
  ...
  async getProductsBySkus(
    ...
    const organizationId = user.organizationId;
    return await this.productService.getProductsBySkus(organizationId, body.skus);
  }
  ...
  async bulkDeleteProducts(
    ...
    const organizationId = user.organizationId;
    return await this.productService.bulkDeleteProducts(organizationId, body.productIds);
  }
}