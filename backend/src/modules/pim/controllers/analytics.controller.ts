...
export class AnalyticsController {
  ...
  async getCatalogCompleteness(
    ...
    try {
      ...
      const descriptionScore =
        totalProducts > 0 ? (productsWithDescription / totalProducts) * 100 : 0;
      const imageScore =
        totalProducts > 0 ? (productsWithImages / totalProducts) * 100 : 0;
      const categoryScore =
        totalProducts > 0 ? (productsWithCategory / totalProducts) * 100 : 0;
      const attributeScore =
        totalProducts > 0 ? (productsWithAttributes / totalProducts) * 100 : 0;
      const priceScore =
        totalProducts > 0 ? (productsWithPrice / totalProducts) * 100 : 0;
      ...
      throw new BadRequestException(
        `Failed to get catalog completeness: ${error.message}`,
      );
    }
  }
  ...
  async getCategoryDistribution(@GetUser() user: any): Promise<any> {
    try {
      // Get network and power conditions
      const networkQuality =
        await this.networkAwareStorageService.getNetworkQuality();
      ...
      const categorizedProductsCount = Array.from(
      ...
      throw new BadRequestException(
        `Failed to get category distribution: ${error.message}`,
      );
    }
  }
  ...
  async getAttributeUsage(@GetUser() user: any): Promise<any> {
    try {
      // Get network and power conditions
      const networkQuality =
        await this.networkAwareStorageService.getNetworkQuality();
      ...
      const attributeCounts = new Map<
        string,
        { count: number; categoryIds: Set<string> }
      >();
      ...
      for (const product of products) {
        if (product.attributes && product.attributes.length > 0) {
          for (const attr of product.attributes) {
            ...
            const current = attributeCounts.get(attrName) || {
              count: 0,
              categoryIds: new Set(),
            };
            current.count++;
      ...
      // Format results
      const attributeUsage = Array.from(attributeCounts.entries()).map(
        ([name, data]) => ({
          name,
      ...
      return {
        attributeUsage: isLightweightMode
          ? attributeUsage.slice(0, 20)
          : attributeUsage,
        summary: {
          totalProducts: products.length,
      ...
      throw new BadRequestException(
        `Failed to get attribute usage: ${error.message}`,
      );
    }
  }
  ...
  @ApiQuery({
    name: 'marketplace',
    required: false,
    description: 'Target marketplace (default: takealot)',
  })
    ...
    try {
      ...
      if (marketplace.toLowerCase() === 'takealot') {
        ...
        const descriptionCheck = {
          ...
          description:
            'Product should have a detailed description of at least 100 characters',
        ...
        const priceCheck = {
          ...
          recommendation:
            product.pricing && product.pricing.basePrice > 0
              ? 'Good'
      ...
      const overallScore =
        possibleScore > 0 ? (readinessScore / possibleScore) * 100 : 0;
      ...
      return {
        ...
        summary: {
          ...
          criticalIssues: checks.filter(
            (c) =>
              !c.passed && ['Product Name', 'Product Price'].includes(c.name),
          ).length,
          majorIssues: checks.filter(
            (c) =>
              !c.passed && !['Product Name', 'Product Price'].includes(c.name),
          ).length,
        },
      ...
      throw new BadRequestException(
        `Failed to get marketplace readiness: ${error.message}`,
      );
    }
  }
  ...
  private getOptimizationRecommendations(scores: {
    ...
    if (scores.descriptionScore < 50) {
      recommendations.push({
        ...
        recommendation:
          'Most products are missing descriptions. Use AI-powered description generation to quickly add quality content.',
      });
      ...
      recommendations.push({
        ...
        area: 'Product Descriptions',
        recommendation:
          'Some products need descriptions. Focus on high-value products first.',
      });
    }
    ...
    if (scores.imageScore < 50) {
      recommendations.push({
        ...
        area: 'Product Images',
        recommendation:
          'Most products are missing images. Add at least one image per product, prioritizing bestsellers.',
      });
      ...
      recommendations.push({
        ...
        area: 'Product Images',
        recommendation:
          'Some products need images. Ensure all visible products have quality images.',
      });
    }
    ...
    if (scores.categoryScore < 70) {
      recommendations.push({
        ...
        recommendation:
          'Many products are uncategorized. Use AI classification to assign categories quickly.',
      });
      ...
      recommendations.push({
        ...
        area: 'Product Categories',
        recommendation:
          'Some products need categorization. Review and assign appropriate categories.',
      });
    }
    ...
    if (scores.attributeScore < 60) {
      recommendations.push({
        ...
        recommendation:
          'Many products have insufficient attributes. Add key attributes to improve search visibility.',
      });
      ...
      recommendations.push({
        ...
        area: 'Product Attributes',
        recommendation:
          'Some products need more detailed attributes. Use attribute extraction to improve data quality.',
      });
    }
    ...
    if (scores.priceScore < 90) {
      recommendations.push({
        ...
        recommendation:
          'Some products are missing prices. Add pricing information to enable sales.',
      });
    }
