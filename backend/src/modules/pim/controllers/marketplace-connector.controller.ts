...
import { MarketplaceValidationService, MarketplaceValidationResult } from '../services/marketplace-validation.service';
...
export class MarketplaceConnectorController {
  ...
  async getMapping(
    ...
    const mapping = await this.mappingRepository.findById(mappingId, user.tenantId);
  ...
  async createMapping(
    ...
    const product = await this.productService.findById(dto.productId, user.tenantId);
    ...
    if (dto.variantId) {
      const variant = await this.variantService.findById(dto.variantId, user.tenantId);
      ...
      if (variant.parentId !== dto.productId) {
        throw new HttpException('Variant does not belong to the specified product', HttpStatus.BAD_REQUEST);
      }
    }
    
    // Check if mapping already exists
    const existingMapping = await this.mappingRepository.findByProductAndMarketplace(
      dto.productId,
      dto.marketplaceId,
      user.tenantId
    );
    ...
    if (existingMapping) {
      throw new HttpException('Mapping already exists for this product and marketplace', HttpStatus.CONFLICT);
    }
  ...
  async updateMapping(
    ...
    if (dto.productId && dto.productId !== mapping.productId) {
      throw new HttpException('Cannot change productId', HttpStatus.BAD_REQUEST);
    }
    
    if (dto.marketplaceId && dto.marketplaceId !== mapping.marketplaceId) {
      throw new HttpException('Cannot change marketplaceId', HttpStatus.BAD_REQUEST);
    }
  ...
  async validateProduct(
    ...
    if (dto.includeVariants) {
      variants = await this.variantService.findByParentId(dto.productId, user.tenantId);
    }
  ...
  async syncStock(
    ...
    // Check if product exists
    const product = await this.productService.findById(productId, user.tenantId);
