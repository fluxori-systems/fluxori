...
export class AttributeTemplateService {
  ...
  async createAttributeTemplate(
    createAttributeTemplateDto: CreateAttributeTemplateDto,
  ): Promise<AttributeTemplate> {
    try {
      this.logger.log(
        `Creating attribute template ${createAttributeTemplateDto.name}`,
      );
      ...
      if (createAttributeTemplateDto.position === undefined) {
        ...
        const templates =
          await this.attributeTemplateRepository.findByOrganization(
      ...
      if (
        createAttributeTemplateDto.scope === AttributeScope.REGIONAL &&
        !createAttributeTemplateDto.region
      ) {
        throw new Error(
      ...
      if (
        createAttributeTemplateDto.scope === AttributeScope.MARKETPLACE &&
        !createAttributeTemplateDto.marketplaceId
      ) {
        throw new Error(
          'Marketplace ID must be provided for marketplace attribute templates',
        );
      }

      // Create the attribute template
      const attributeTemplate = await this.attributeTemplateRepository.create(
        createAttributeTemplateDto,
      );

      this.logger.log(
        `Attribute template created with ID ${attributeTemplate.id}`,
      );

      return attributeTemplate;
    } catch (error) {
      this.logger.error(
        `Error creating attribute template: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async updateAttributeTemplate(
    id: string,
    updateAttributeTemplateDto: UpdateAttributeTemplateDto,
  ): Promise<AttributeTemplate> {
    try {
      ...
      const currentTemplate =
        await this.attributeTemplateRepository.findById(id);

      if (!currentTemplate) {
        throw new NotFoundException(
          `Attribute template with ID ${id} not found`,
        );
      }

      // Validate template data based on scope
      if (
        updateAttributeTemplateDto.scope === AttributeScope.REGIONAL &&
        !updateAttributeTemplateDto.region
      ) {
        // If scope is changing to REGIONAL, region must be provided
        if (
          currentTemplate.scope !== AttributeScope.REGIONAL ||
      ...
      if (
        updateAttributeTemplateDto.scope === AttributeScope.MARKETPLACE &&
        !updateAttributeTemplateDto.marketplaceId
      ) {
        // If scope is changing to MARKETPLACE, marketplaceId must be provided
        if (
          currentTemplate.scope !== AttributeScope.MARKETPLACE ||
          !currentTemplate.marketplaceId
        ) {
          throw new Error(
            'Marketplace ID must be provided for marketplace attribute templates',
          );
      ...
      const attributeTemplate = await this.attributeTemplateRepository.update(
        id,
        updateAttributeTemplateDto,
      );
      ...
      this.logger.error(
        `Error updating attribute template: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAttributeTemplateById(
    ...
    try {
      ...
      if (
        !attributeTemplate ||
        attributeTemplate.organizationId !== organizationId
      ) {
        return null;
      }
      ...
      this.logger.error(
        `Error getting attribute template: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async deleteAttributeTemplate(id: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error deleting attribute template: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAllAttributeTemplates(
    ...
    try {
      this.logger.log(
        `Getting all attribute templates for organization ${organizationId}`,
      );
      ...
      this.logger.error(
        `Error getting all attribute templates: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAttributeTemplatesByCategory(
    ...
    try {
      ...
      return await this.attributeTemplateRepository.findByCategory(
        organizationId,
        categoryId,
      );
    } catch (error) {
      this.logger.error(
        `Error getting attribute templates by category: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getGlobalAttributeTemplates(
    ...
    try {
      this.logger.log(
        `Getting global attribute templates for organization ${organizationId}`,
      );

      return await this.attributeTemplateRepository.findGlobalTemplates(
        organizationId,
      );
    } catch (error) {
      this.logger.error(
        `Error getting global attribute templates: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAttributeTemplatesByRegion(
    ...
    try {
      ...
      return await this.attributeTemplateRepository.findByRegion(
        organizationId,
        region,
      );
    } catch (error) {
      this.logger.error(
        `Error getting attribute templates by region: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAttributeTemplatesByMarketplace(
    ...
    try {
      this.logger.log(
        `Getting attribute templates for marketplace ${marketplaceId}`,
      );

      return await this.attributeTemplateRepository.findByMarketplace(
        organizationId,
        marketplaceId,
      );
    } catch (error) {
      this.logger.error(
        `Error getting attribute templates by marketplace: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAttributeTemplatesByScope(
    ...
    try {
      ...
      return await this.attributeTemplateRepository.findByScope(
        organizationId,
        scope,
        options,
      );
      ...
      this.logger.error(
        `Error getting attribute templates by scope: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getApplicableAttributeTemplates(
    ...
    try {
      this.logger.log(
        `Getting applicable attribute templates for product in categories ${categoryIds.join(', ')}`,
      );

      // Get templates that apply to all products
      const globalApplicableTemplates =
        await this.attributeTemplateRepository.findGlobalApplicable(
          organizationId,
        );

      // Get templates for the product's categories
      const categoryTemplatesPromises = categoryIds.map((categoryId) =>
        this.attributeTemplateRepository.findByCategory(
          organizationId,
          categoryId,
        ),
      );

      const categoryTemplatesArrays = await Promise.all(
        categoryTemplatesPromises,
      );
      ...
      if (region) {
        regionalTemplates = await this.attributeTemplateRepository.findByRegion(
          organizationId,
          region,
        );
      }
      ...
      if (marketplaceIds.length > 0) {
        const marketplaceTemplatesPromises = marketplaceIds.map(
          (marketplaceId) =>
            this.attributeTemplateRepository.findByMarketplace(
              organizationId,
              marketplaceId,
            ),
        );

        const marketplaceTemplatesArrays = await Promise.all(
          marketplaceTemplatesPromises,
        );
      ...
      this.logger.error(
        `Error getting applicable attribute templates: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async bulkUpdateAttributeTemplates(
    ...
    try {
      ...
      const operationPromises = operations.map(async ({ id, updates }) => {
        try {
          const attributeTemplate = await this.updateAttributeTemplate(
            id,
            updates,
          );
          return {
  ...
  async bulkDeleteAttributeTemplates(
    ...
    try {
      this.logger.log(
        `Bulk deleting ${templateIds.length} attribute templates`,
      );
      ...
      const deletePromises = templateIds.map(async (id) => {
        try {
          ...
          const template = await this.getAttributeTemplateById(
            id,
            organizationId,
          );
