...
import { PriceInfo, ProductAttribute, CategoryReference } from '../interfaces/types';
...
export class ImportExportService {
  ...
  async importProducts(
    ...
    try {
      ...
      this.logger.error(`Error during product import: ${error.message}`, error.stack);
  ...
  async exportProducts(options: ExportOptions): Promise<ExportResult> {
    ...
    try {
      ...
      this.logger.error(`Error during product export: ${error.message}`, error.stack);
      ...
      readableStream
        ...
        .on('data', (row) => {
          ...
          try {
            ...
            const product = this.mapRowToProduct(row, options.fieldMappings, options.defaultValues);
          ...
          for (const product of products) {
            try {
              ...
              if (existingProduct && options.updateExisting) {
                ...
                result.warnings.push(`Skipped product with SKU ${product.sku} as it already exists`);
              }
            } catch (error) {
              result.failureCount++;
              result.errors.push({
  ...
  private async importFromJson(
    ...
    try {
      ...
      if (!Array.isArray(jsonData)) {
        throw new BadRequestException('JSON import must be an array of products');
      }
  ...
  private async generateExport(
    ...
    // Calculate file size
    originalSize = typeof result.data === 'string' 
    ...
    if (networkInfo.connectionQuality !== 'high' && originalSize > 50000) { // 50KB threshold
  ...
  private async getProductsForExport(
    ...
    const products = await this.productService.findAll(productFilters, tenantId);
  ...
  private async includeVariantsForProducts(
    ...
    for (const product of products) {
      if (product.type === ProductType.SIMPLE) {
        const variants = await this.variantService.findByParentId(product.id, tenantId);
  ...
  private mapRowToProduct(
    ...
    Object.entries(row).forEach(([columnName, value]) => {
      ...
      if (fieldName.includes('.')) {
        ...
        product[parentField][childField] = this.parseFieldValue(value, childField);
      } 
  ...
  private parseFieldValue(value: string, fieldName: string): any {
    ...
    // Parse numbers
    if (fieldName === 'basePrice' || fieldName === 'specialPrice' || 
        fieldName === 'costPrice' || fieldName === 'rrp' || 
        fieldName === 'weight' || fieldName.includes('price')) {
      return parseFloat(value);
    }
    
    // Parse booleans
    if (fieldName === 'featured' || fieldName === 'inStock' || 
        fieldName === 'manageStock' || fieldName === 'vatIncluded' || 
        fieldName.includes('Approved') || fieldName.includes('Compliant')) {
  ...
  private validateRequiredFields(product: Partial<Product>): void {
    ...
    if (product.type && !Object.values(ProductType).includes(product.type as ProductType)) {
