...
export class AttributeTemplateBulkOperationsService {
  private readonly logger = new Logger(AttributeTemplateBulkOperationsService.name);
  ...
  ): Promise<{ results: Array<OperationResult<AttributeTemplate>>, stats: BulkOperationStats }> {
    this.logger.log(`Starting bulk attribute template update for ${operations.length} templates`);
    
    return this.bulkOperationsService.executeBulk<AttributeTemplateBulkUpdateOperation, AttributeTemplate>(
      operations,
      async (operation) => {
        return await this.attributeTemplateService.updateAttributeTemplate(operation.id, operation.updates);
      },
      options
    );
  }
  ...
  async bulkDeleteAttributeTemplates(
    ...
    this.logger.log(`Starting bulk attribute template deletion for ${templateIds.length} templates`);
    ...
    return this.bulkOperationsService.executeBulk<{ id: string, organizationId: string }, { id: string }>(
      operations,
      async (operation) => {
        await this.attributeTemplateService.deleteAttributeTemplate(operation.id, operation.organizationId);
  ...
  async bulkModifyTemplateAttributes(
    ...
    this.logger.log(`Starting bulk attribute modification for ${operations.length} templates`);
    
    return this.bulkOperationsService.executeBulk<AttributeTemplateBulkAttributeOperation, AttributeTemplate>(
      operations,
      async (operation) => {
        // Get current template
        const template = await this.attributeTemplateService.getAttributeTemplateById(operation.id, organizationId);
        
        if (!template) {
          throw new Error(`Attribute template with ID ${operation.id} not found`);
        }
        ...
        if (operation.attributesToRemove && operation.attributesToRemove.length > 0) {
          const attributeKeysToRemove = new Set(operation.attributesToRemove);
          const filteredAttributes = attributes.filter(attr => !attributeKeysToRemove.has(attr.key));
          attributes.length = 0;
        ...
        if (operation.attributesToAdd && operation.attributesToAdd.length > 0) {
          ...
          for (const newAttr of operation.attributesToAdd) {
            if (existingKeys.has(newAttr.key)) {
              ...
              const index = attributes.findIndex(attr => attr.key === newAttr.key);
        ...
        return await this.attributeTemplateService.updateAttributeTemplate(operation.id, updates);
      },
      options
    );
  }
  ...
  async bulkUpdateMarketplaceMappings(
    ...
    this.logger.log(`Starting bulk marketplace mapping update for ${operations.length} templates`);
    
    return this.bulkOperationsService.executeBulk<AttributeTemplateBulkMarketplaceMappingOperation, AttributeTemplate>(
      operations,
      async (operation) => {
        // Get current template
        const template = await this.attributeTemplateService.getAttributeTemplateById(operation.id, organizationId);
