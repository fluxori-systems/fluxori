...
export class ProductBulkOperationsService {
  ...
  async bulkUpdateProducts(
    ...
    this.logger.log(`Starting bulk product update for ${operations.length} products`);
    
    return this.bulkOperationsService.executeBulk<ProductBulkUpdateOperation, Product>(
      operations,
      async (operation) => {
        return await this.productService.updateProduct(operation.id, operation.updates);
      },
      options
    );
  }
  ...
  async bulkDeleteProducts(
    ...
    this.logger.log(`Starting bulk product deletion for ${productIds.length} products`);
  ...
  async bulkUpdateProductStatus(
    ...
    this.logger.log(`Starting bulk product status update for ${operations.length} products`);
    
    return this.bulkOperationsService.executeBulk<ProductBulkStatusOperation, Product>(
      operations,
      async (operation) => {
  ...
  async bulkUpdateProductPrices(
    ...
    this.logger.log(`Starting bulk product price update for ${operations.length} products`);
    
    return this.bulkOperationsService.executeBulk<ProductBulkPriceOperation, Product>(
      operations,
      async (operation) => {
        const updateDto: UpdateProductDto = {
          organizationId,
  ...
  async bulkUpdateProductCategories(
    ...
    this.logger.log(`Starting bulk product category update for ${operations.length} products`);
    
    return this.bulkOperationsService.executeBulk<ProductBulkCategoryOperation, Product>(
      operations,
      async (operation) => {
        // First fetch the current product to get existing categories if needed
        const product = await this.productService.getProductById(operation.id, organizationId);
        ...
        if (!operation.replaceExisting && product.categoryIds) {
          ...
          categoryIds = [...new Set([...product.categoryIds, ...operation.categoryIds])];
        }
  ...
  async bulkUpdateProductInventory(
    ...
    this.logger.log(`Starting bulk product inventory update for ${operations.length} products`);
    
    return this.bulkOperationsService.executeBulk<ProductBulkInventoryOperation, Product>(
      operations,
      async (operation) => {
        const updateDto: UpdateProductDto = {
          organizationId,
  ...
  async bulkDuplicateProducts(
    ...
    this.logger.log(`Starting bulk product duplication for ${operations.length} products`);
    
    return this.bulkOperationsService.executeBulk<ProductBulkDuplicateOperation, Product>(
      operations,
      async (operation) => {
        // Fetch the source product
        const sourceProduct = await this.productService.getProductById(operation.id, organizationId);
