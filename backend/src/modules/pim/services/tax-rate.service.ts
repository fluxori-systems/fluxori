...
export class TaxRateService implements ITaxRateService {
  ...
  async getRateAtDate(params: TaxRateRequest): Promise<TaxRateResult> {
    ...
    if (params.productType || (params.productCategories && params.productCategories.length > 0)) {
      try {
        ...
        if (specialRate) {
          return this.convertScheduleToResult(specialRate, true, params.includeHistory);
        }
      } catch (error) {
    ...
    if (!standardRate) {
      throw new Error(`No tax rate found for jurisdiction ${params.country}${params.region ? '/' + params.region : ''} and tax type ${params.taxType} at date ${transactionDate.toISOString()}`);
    }
    
    return this.convertScheduleToResult(standardRate, false, params.includeHistory);
  }
  ...
  private async convertScheduleToResult(
    ...
    if (includeHistory) {
      const allRates = await this.getRateChanges(schedule.jurisdiction, schedule.taxType);
      const now = new Date();
      
      result.history = {
        past: allRates.filter(rate => rate.validTo ? rate.validTo < now : false),
