...
export class AfricanTaxFrameworkService {
   ...
   */
  public async calculateAfricanTax(params: TaxRateRequest): Promise<TaxRateResult> {
   ...
   */
  private async applyCountrySpecificRules(baseRate: TaxRateResult, params: TaxRateRequest): Promise<TaxRateResult> {
    ...
    switch (countryCode) {
      case 'NG': // Nigeria
        ...
        if (params.region && countryRules.hasRegionalRates) {
          ...
          try {
            ...
            const regionalRates = await this.taxRateRepository.findAllRatesForJurisdiction(
            ...
            if (regionalRates.length > 0) {
              ...
              const applicableRegionalRate = regionalRates.find(rate => 
                rate.taxType === params.taxType && 
                (!rate.validTo || rate.validTo >= (params.transactionDate || new Date()))
              );
            ...
            console.error(`Error getting regional tax rates for Nigeria/${params.region}:`, error);
          }
        }
        break;
      ...
      case 'GH': // Ghana
        ...
        if (params.taxType === TaxType.VAT) {
          ...
          result.description = 'Standard VAT (12.5%) plus NHIL (2.5%) and GETFund (2.5%) levies';
          result.isSpecialRate = true;
        }
        break;
      ...
      case 'ZA': // South Africa
        ...
        if (params.transactionDate) {
          ...
          if (transactionDate >= april2026) {
            ...
            result.description = 'South Africa VAT at 16% (effective April 2026)';
            ...
            result.description = 'South Africa VAT at 15.5% (effective May 2025)';
          }
        }
        break;
   ...
   */
  public async getSpecialProductTaxCategories(countryCode: string): Promise<Array<{
    productType: string;
    category: string;
    rate: number;
    description: string;
    ...
    try {
      ...
      const specialRates = await this.taxRateRepository.findSpecialRatesByJurisdiction(jurisdiction);
      ...
      specialRates.forEach(rate => {
        if (rate.metadata?.productType || rate.metadata?.productTypes) {
          ...
          specialProductTypes.push({
            ...
            category: rate.name,
            rate: rate.rate,
            description: rate.description || `${rate.name} at ${rate.rate * 100}%`
          });
        }
      });
        ...
        specialProductTypes.push(
          {
            ...
            category: 'Zero-Rated',
            rate: 0,
            description: 'Basic food items like brown bread, maize meal, rice, vegetables, fruits, etc.'
          },
          {
      ...
      console.error(`Error getting special product tax categories for ${countryCode}:`, error);
    }
  ...
  public async isProductTaxExempt(params: TaxRateRequest): Promise<{
    ...
    switch (params.country) {
      case 'ZA': // South Africa
        ...
        if (params.productType === 'BasicFoodstuffs' || 
            (params.productCategories && params.productCategories.includes('BasicFoodstuffs'))) {
          return { 
            exempt: true, 
      ...
      case 'NG': // Nigeria
        ...
        if (params.productType === 'BasicFood' || 
   ...
   */
  public async getUpcomingRateChanges(countryCode: string): Promise<Array<{
    effectiveDate: Date;
    oldRate: number;
    newRate: number;
    taxType: string;
    ...
    try {
      ...
      futureRates.forEach(rate => {
        upcomingChanges.push({
          ...
          description: rate.description || `${rate.taxType} rate change from ${previousRate * 100}% to ${rate.rate * 100}%`,
      ...
      console.error(`Error getting upcoming rate changes for ${countryCode}:`, error);
    }
