...
export class B2BService {
  ...
  constructor(
    ...
    @Inject('MARKET_CONTEXT_OPTIONS')
    private readonly marketContextOptions: any,
  ) {
  ...
  async createCustomer(customer: B2BCustomer): Promise<B2BCustomer> {
    ...
    if (existing) {
      throw new Error(
        `Customer with number ${customer.customerNumber} already exists`,
      );
    }
    ...
    const newCustomer: B2BCustomer = {
      ...
      paymentInfo: {
        ...
        creditStatus:
          customer.paymentInfo?.creditStatus || CreditStatus.NOT_ELIGIBLE,
        requiresPurchaseOrder:
          customer.paymentInfo?.requiresPurchaseOrder !== undefined
      ...
      defaultCurrencyCode:
        ...
        this.getDefaultCurrencyForRegion(customer.marketRegion),
    };
  ...
  async updateCustomer(
    ...
    if (
      ...
      updates.customerNumber !== existing.customerNumber
    ) {
  ...
  async createCustomerTier(tier: CustomerTier): Promise<CustomerTier> {
    ...
    const newTier: CustomerTier = {
      ...
      allowsPaymentTerms:
        tier.allowsPaymentTerms !== undefined ? tier.allowsPaymentTerms : false,
      enableCreditLimits:
        tier.enableCreditLimits !== undefined ? tier.enableCreditLimits : false,
      hasSpecialPaymentMethods:
        tier.hasSpecialPaymentMethods !== undefined
          ? tier.hasSpecialPaymentMethods
          : false,
  ...
  async createPriceList(priceList: B2BPriceList): Promise<B2BPriceList> {
    ...
    const newPriceList: B2BPriceList = {
      ...
      enableVolumePricing:
        priceList.enableVolumePricing !== undefined
          ? priceList.enableVolumePricing
          : false,
      enableRegionalPricing:
        priceList.enableRegionalPricing !== undefined
          ? priceList.enableRegionalPricing
          : false,
      createdAt: now,
      updatedAt: now,
    };
  ...
  async addProductToPriceList(
    ...
    const existingIndex = priceList.prices.findIndex(
      (p) => p.productId === priceEntry.productId,
    );
    ...
    if (existingIndex >= 0) {
      ...
      priceList.prices.push({
        ...priceEntry,
        isActive:
          priceEntry.isActive !== undefined ? priceEntry.isActive : true,
      });
    }
  ...
  async findPriceListsForCustomer(
    ...
    const now = new Date();
    const validPriceLists = allPriceLists
      .flat()
      .filter(
        (priceList) =>
          priceList.isActive &&
  ...
  async createContract(contract: CustomerContract): Promise<CustomerContract> {
    ...
    if (existing) {
      throw new Error(
        `Contract with number ${contract.contractNumber} already exists`,
      );
    }
  ...
  async calculateB2BPrice(
    ...
    if (options?.contractId) {
      ...
      for (const contract of activeContracts) {
        ...
        if (
          contractPrice &&
          (bestContractPrice === null ||
            contractPrice.finalPrice < bestContractPrice)
        ) {
    ...
    if (priceSource === 'standard') {
      const priceLists = await this.findPriceListsForCustomer(
        customerId,
        organizationId,
      );
      ...
      for (const priceList of priceLists) {
        ...
        if (
          ...
          priceList.currencyCode !== options.currencyCode
        ) {
          continue;
        }

        // Check if this price list has an entry for this product
        const productEntry = priceList.prices.find(
        ...
        if (productEntry) {
          ...
          if (
            productEntry.minQuantity &&
            (!options?.quantity || options.quantity < productEntry.minQuantity)
          ) {
            continue;
          }
          ...
          if (
            productEntry.maxQuantity &&
            options?.quantity &&
            options.quantity > productEntry.maxQuantity
          ) {
            continue;
          }
          ...
          if (
            bestPriceListPrice === null ||
            productEntry.price < bestPriceListPrice
          ) {
            ...
            if (
              priceList.type === PriceListType.GROUP_SPECIFIC &&
              priceList.customerGroupIds?.length
            ) {
              bestPriceGroupId = priceList.customerGroupIds[0];
            } else if (
              priceList.type === PriceListType.TIER_SPECIFIC &&
              priceList.customerTierIds?.length
            ) {
        ...
        if (
          priceList.enableVolumePricing &&
          priceList.volumePrices &&
          options?.quantity
        ) {
          const volumePrice = priceList.volumePrices.find(
            (vp) => vp.productId === productId && vp.isActive,
          );

          if (volumePrice) {
            // Find the applicable tier based on quantity
            const applicableTier = volumePrice.tiers.find(
              (tier) =>
            ...
            if (
              applicableTier &&
              (bestPriceListPrice === null ||
                applicableTier.price! < bestPriceListPrice)
            ) {
    ...
    if (priceSource === 'standard' && customer.customerGroupIds?.length) {
      ...
      for (const group of customerGroups) {
        if (
          group &&
          group.isActive &&
          group.customDiscountPercentage &&
          group.customDiscountPercentage > bestDiscount
        ) {
  ...
  private calculateContractPrice(
    ...
    if (pricing.specificProductPrices) {
      const productPricing = pricing.specificProductPrices.find(
        (p) => p.productId === product.id || p.sku === product.sku,
      );
      ...
      if (productPricing) {
        if (productPricing.fixedPrice !== undefined) {
          return {
            ...
            discountPercentage:
              ((standardPrice - productPricing.fixedPrice) / standardPrice) *
              100,
          };
        }
        ...
        if (productPricing.discountPercentage !== undefined) {
          const discountAmount =
            standardPrice * (productPricing.discountPercentage / 100);
          return {
            finalPrice: standardPrice - discountAmount,
    ...
    if (pricing.volumeDiscounts && pricing.volumeDiscounts.length > 0) {
      ...
      const sortedTiers = [...pricing.volumeDiscounts].sort(
      ...
      const applicableTier = sortedTiers.find(
        (tier) => orderValue >= tier.minOrderValue,
      );

      if (applicableTier) {
        const discountAmount =
          standardPrice * (applicableTier.discountPercentage / 100);
        return {
          finalPrice: standardPrice - discountAmount,
    ...
    if (
      pricing.globalDiscountPercentage !== undefined &&
      pricing.globalDiscountPercentage > 0
    ) {
      const discountAmount =
        standardPrice * (pricing.globalDiscountPercentage / 100);
      return {
        finalPrice: standardPrice - discountAmount,
  ...
  async createPurchaseOrder(
    ...
    if (purchaseOrder.purchaseOrderNumber) {
      ...
      if (existing) {
        throw new Error(
          `Purchase order with number ${purchaseOrder.purchaseOrderNumber} already exists`,
        );
      }
    }
    ...
    if (
      !newPurchaseOrder.approvalWorkflowId &&
      customer.defaultApprovalWorkflowId
    ) {
  ...
  async submitPurchaseOrderForApproval(
    ...
    if (order.status !== PurchaseOrderStatus.DRAFT) {
      throw new Error(
        `Purchase order with ID ${orderId} cannot be submitted (current status: ${order.status})`,
      );
    }

    // Get the approval workflow
    const workflowId = order.approvalWorkflowId;
    const nextApprover: { id?: string; name?: string } = {};
    ...
    if (workflowId) {
      ...
      if (workflow && workflow.steps.length > 0) {
        ...
        if (
          firstStep &&
          firstStep.approverUserIds &&
          firstStep.approverUserIds.length > 0
        ) {
  ...
  async approvePurchaseOrder(
    ...
    if (order.status !== PurchaseOrderStatus.PENDING_APPROVAL) {
      throw new Error(
        `Purchase order with ID ${orderId} cannot be approved (current status: ${order.status})`,
      );
    }
    ...
    if (order.approvalWorkflowId && order.currentApprovalStep !== undefined) {
      ...
      if (workflow) {
        ...
        const nextStep = workflow.steps.find(
          (step) => step.stepNumber === order.currentApprovalStep! + 1,
        );
    ...
    const updatedOrder: PurchaseOrder = {
      ...
      currentApprovalStep: finalApproval
        ? undefined
        : order.currentApprovalStep! + 1,
      approvalActions: [...(order.approvalActions || []), approvalAction],
      updatedAt: new Date(),
