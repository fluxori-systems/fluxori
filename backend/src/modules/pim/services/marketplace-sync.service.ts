...
export class MarketplaceSyncService {
  ...
  async syncProductToMarketplace(
    ...
    try {
      ...
      this.logger.error(`Error syncing product ${productId} to ${marketplaceId}: ${error.message}`, error.stack);
  ...
  private async performProductSync(
    ...
    const variants = await this.variantRepository.findByParentId(productId, tenantId);
    
    // 2. Get existing marketplace mapping if any
    let mapping = await this.mappingRepository.findByProductAndMarketplace(
      productId,
      marketplaceId,
      tenantId
    );
    ...
    const connector = await this.getMarketplaceConnector(marketplaceId, tenantId);
    
    // 4. Transform product to marketplace format
    const { marketplaceProduct, metadata } = this.transformProductToMarketplaceFormat(
      product,
      variants,
      marketplaceId,
      mapping
    ...
    if (marketplaceId === 'takealot') {
      ...
      if (variants.length > 0) {
        marketplaceProduct.variants = variants.map(variant => ({
          ...
          compareAtPrice: variant.pricing?.compareAtPrice || product.pricing.rrp,
  ...
  private transformProductForTakealot(
    ...
    const takealotProduct = {
      ...
      short_description: product.shortDescription || product.description.substring(0, 255),
      long_description: product.description,
      rrp: mapping?.southAfrica?.takealot?.rrp || product.pricing.rrp || product.pricing.basePrice * 1.15,
   ...
   */
  private extractSouthAfricanBarcode(productOrVariant: Product | ProductVariant): string {
   ...
   */
  private extractVariantAttributes(variant: ProductVariant): Record<string, any> {
    const attributes: Record<string, any> = {};
   ...
   */
  private extractTakealotVariantAttributes(variant: ProductVariant): Record<string, any> {
  ...
  private async updateExistingProduct(
    ...
    try {
      ...
      const statusResult = await connector.updateStatus([{
        sku: mapping.externalSku || product.sku,
        status: marketplaceProduct.product_status === 'active' ? 'active' : 'inactive'
      }]);
  ...
  private async createNewProduct(
    ...
    try {
      ...
      // Create mapping
      const newMapping = await this.mappingRepository.create(mappingDto, tenantId);
      ...
      this.logger.error(`Error creating new product in ${marketplaceId}: ${error.message}`, error.stack);
   ...
   */
  private async getMarketplaceConnector(marketplaceId: string, tenantId: string): Promise<any> {
    try {
      ...
      this.logger.error(`Error getting connector for ${marketplaceId}: ${error.message}`, error.stack);
      throw new Error(`Could not initialize marketplace connector: ${error.message}`);
    }
  }
  ...
  async syncStockToAllMarketplaces(
    ...
    try {
      ...
      const mappings = await this.mappingRepository.findByProductId(productId, tenantId);
      ...
      const results = await Promise.all(
        mappings.map(async mapping => {
          try {
            const connector = await this.getMarketplaceConnector(mapping.marketplaceId, tenantId);
      ...
      this.logger.error(`Error syncing stock for product ${productId}: ${error.message}`, error.stack);
  ...
  async syncPriceToAllMarketplaces(
    ...
    try {
      ...
      this.logger.error(`Error syncing price for product ${productId}: ${error.message}`, error.stack);
