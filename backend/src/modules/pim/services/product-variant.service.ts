...
import { ProductVariant, VariantGroup, CreateProductVariantDto, UpdateProductVariantDto } from '../models/product-variant.model';
...
export class ProductVariantService {
  ...
  async create(tenantId: string, dto: CreateProductVariantDto): Promise<ProductVariant> {
    const parent = await this.productRepository.findById(dto.parentId, tenantId);
    
    if (!parent) {
      throw new NotFoundException(`Parent product with ID ${dto.parentId} not found`);
    }
    
    // Handle South African regional specifics
    if (this.options.enableSouthAfricanOptimizations) {
      const context = await this.marketContextService.getMarketContext(tenantId);
   ...
   */
  async findById(variantId: string, tenantId: string): Promise<ProductVariant | null> {
   ...
   */
  async findByParentId(parentId: string, tenantId: string): Promise<ProductVariant[]> {
   ...
   */
  async getVariantGroup(productId: string, tenantId: string): Promise<VariantGroup | null> {
    ...
    const variants = await this.productVariantRepository.findByParentId(productId, tenantId);
   ...
   */
  async update(variantId: string, tenantId: string, dto: UpdateProductVariantDto): Promise<ProductVariant> {
    const variant = await this.productVariantRepository.findById(variantId, tenantId);
    ...
    if (dto.images && this.options.enableSouthAfricanOptimizations) {
      const networkInfo: NetworkQualityInfo = await this.networkAwareStorageService.getNetworkQuality();
      
      if (networkInfo.connectionQuality !== 'high') {
        this.logger.log(`Processing images with optimized settings for ${networkInfo.connectionQuality} connection`);
        
        if (dto.images.main) {
          dto.images.main = await this.networkAwareStorageService.optimizeImageUrl(
            dto.images.main,
            networkInfo
          );
        }
        ...
        if (dto.images.gallery && dto.images.gallery.length > 0) {
          dto.images.gallery = await Promise.all(
            dto.images.gallery.map(url => 
              this.networkAwareStorageService.optimizeImageUrl(url, networkInfo)
            )
          );
        }
      }
    ...
    try {
      await Promise.all(
        positions.map(({ variantId, position }) =>
          this.productVariantRepository.updatePosition(variantId, position, tenantId)
        )
      );
      
      return { success: true };
    } catch (error) {
      this.logger.error(`Failed to update variant positions: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  async delete(variantId: string, tenantId: string): Promise<OperationResult> {
    try {
      ...
      this.logger.error(`Failed to delete variant ${variantId}: ${error.message}`, error.stack);
      return {
        success: false,
   ...
   */
  async deleteByParentId(parentId: string, tenantId: string): Promise<OperationResult<{ count: number }>> {
    try {
      const count = await this.productVariantRepository.deleteByParentId(parentId, tenantId);
      ...
      this.logger.error(`Failed to delete variants for product ${parentId}: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  async generateVariants(
    ...
    try {
      ...
      for (const combination of combinations) {
        ...
        const variant = await this.productVariantRepository.create(variantDto, tenantId);
      ...
      await this.productRepository.update(productId, { attributes: updatedAttributes }, tenantId);
      ...
      this.logger.error(`Failed to generate variants: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  private generateAttributeCombinations(
    attributes: Array<{ code: string; value: any; label: string; type: string }>
    ...
    if (attributes.length === 1) {
      ...
      // For boolean attributes
        ...
        return [
          [{ code: attr.code, value: true, label: `${attr.label}: Yes`, type: attr.type }],
          [{ code: attr.code, value: false, label: `${attr.label}: No`, type: attr.type }]
        ];
      }
    ...
    // For boolean attributes
      ...
      firstOptions = [
        { code: firstAttr.code, value: true, label: `${firstAttr.label}: Yes`, type: firstAttr.type },
        { code: firstAttr.code, value: false, label: `${firstAttr.label}: No`, type: firstAttr.type }
      ];
    }
    // Default to using the current value
    else {
