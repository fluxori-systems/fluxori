...
export class LoadSheddingResilienceService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const isLoadSheddingEnabled = await this.featureFlagService.isEnabled('pim.loadSheddingResilience', { 
    ...
    if (isLoadSheddingEnabled) {
      ...
      const queueIntervalMs = this.configService.get<number>('LOAD_SHEDDING_QUEUE_INTERVAL_MS') || 30000; // 30 seconds
      this.processingInterval = setInterval(() => this.processQueue(), queueIntervalMs);
      
      // Start periodic checking of load shedding status
      const checkIntervalMs = this.configService.get<number>('LOAD_SHEDDING_CHECK_INTERVAL_MS') || 300000; // 5 minutes
      this.checkInterval = setInterval(() => this.updateLoadSheddingStatus(), checkIntervalMs);
  ...
  private async updateLoadSheddingStatus(): Promise<void> {
    try {
      ...
      // Log current status
      this.logger.debug(`Load shedding status: Stage ${this.loadSheddingInfo.stage}, Active: ${this.loadSheddingInfo.active}`);
      
      // If load shedding has ended, process the queue
      if (!this.loadSheddingInfo.active && this.operationQueue.length > 0 && !this.isProcessingQueue) {
        await this.processQueue();
      }
    } catch (error) {
      this.logger.error(`Error updating load shedding status: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async enqueueOperation<T>(
    ...
    this.logger.log(`Enqueued operation ${id} of type ${type} with priority ${options.priority || 5}`);
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      const sortedQueue = [...this.operationQueue].sort((a, b) => a.priority - b.priority);
      ...
      this.logger.log(`Processing ${batch.length} operations from queue (${this.operationQueue.length} total)`);
      ...
      const completedIds = results
        .map((result, index) => result.status === 'fulfilled' && result.value ? batch[index].id : null)
        .filter((id): id is string => id !== null);
      
      this.operationQueue = this.operationQueue.filter(op => !completedIds.includes(op.id));
      
      this.logger.log(`Completed ${completedIds.length} operations, ${this.operationQueue.length} remaining in queue`);
    } catch (error) {
      this.logger.error(`Error processing operation queue: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
   ...
   */
  private async executeOperation(operation: QueuedOperation<any>): Promise<boolean> {
    try {
      ...
      this.logger.log(`Successfully executed operation ${operation.id} of type ${operation.type}`);
      return true;
    } catch (error) {
      // Log the error
      this.logger.error(`Error executing operation ${operation.id} of type ${operation.type}: ${error instanceof Error ? error.message : String(error)}`);
      
      // Check if we should retry
      if (operation.retries < operation.maxRetries) {
        this.logger.log(`Operation ${operation.id} will be retried (${operation.retries}/${operation.maxRetries})`);
        return false; // Keep in queue for retry
      } else {
        this.logger.warn(`Operation ${operation.id} failed after ${operation.retries} retries and will be removed from queue`);
  ...
  async executeWithResilience<T>(
    ...
    const executeImmediately = options.executeImmediatelyIfPossible !== false && 
    ...
    if (executeImmediately) {
      try {
        ...
        this.logger.warn(`Operation ${type} failed with error: ${error instanceof Error ? error.message : String(error)}. Queueing for retry.`);
      ...
      await this.enqueueOperation(type, params, async (p) => {
  ...
  async executeBatchWithResilience<T, R>(
    ...
    try {
      ...
      for (let i = 0; i < items.length; i += batchSize) {
        ...
        this.logger.log(`Processing batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(items.length / batchSize)}`);
        ...
        if (loadSheddingStatus.active && loadSheddingStatus.stage > 4) {
          ...
          this.logger.warn(`Pausing batch processing due to load shedding stage ${loadSheddingStatus.stage}`);
        ...
        const batchPromises = batch.map(async (item) => {
          ...
          while (retries <= retryCount && !success) {
            try {
              ...
              this.logger.warn(`Error processing item (retry ${retries}/${retryCount}): ${lastError.message}`);
              retries++;
            }
          }
      ...
      this.logger.error(`Error in batch execution: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
