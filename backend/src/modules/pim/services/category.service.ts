...
export class CategoryService {
   ...
   */
  async findCategories(filter: CategoryFilter): Promise<{
    items: Category[];
    total: number;
    page: number;
    limit: number;
    ...
    try {
      this.logger.log(
        `Finding categories with filter: ${JSON.stringify(filter)}`,
      );
  ...
  async getCategoryTree(
    ...
    try {
      this.logger.log(
        `Getting category tree for organization ${organizationId}`,
      );
  ...
  async getRootCategories(organizationId: string): Promise<Category[]> {
    try {
      this.logger.log(
        `Getting root categories for organization ${organizationId}`,
      );

      return await this.categoryRepository.findRootCategories(organizationId);
    } catch (error) {
      this.logger.error(
        `Error getting root categories: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getChildCategories(
    ...
    try {
      ...
      if (parentId === null) {
        ...
        return await this.categoryRepository.findChildCategories(
          organizationId,
          parentId,
        );
      }
      ...
      this.logger.error(
        `Error getting child categories: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getCategoriesByMarketplace(
    ...
    try {
      ...
      return await this.categoryRepository.findByMarketplace(
        organizationId,
        marketplaceId,
      );
    } catch (error) {
      this.logger.error(
        `Error getting categories by marketplace: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async bulkUpdateCategories(
    operations: Array<{ id: string; updates: UpdateCategoryDto }>,
    ...
    try {
      ...
      for (const id of categoryOrder) {
        if (!childIds.includes(id)) {
          throw new Error(
            `Category with ID ${id} is not a child of parent ${parentId}`,
          );
        }
      }
      ...
      const updatedCategories = await this.getChildCategories(
        organizationId,
        parentId,
      );
