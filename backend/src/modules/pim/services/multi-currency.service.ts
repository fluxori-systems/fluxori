...
export class MultiCurrencyService {
   ...
   */
  async getMultiCurrencyConfig(organizationId: string): Promise<MultiCurrencyConfig> {
  ...
  async getSupportedCurrencies(): Promise<CurrencyInfo[]> {
    return [
      { code: 'ZAR', name: 'South African Rand', symbol: 'R', decimalDigits: 2, regions: ['south-africa'] },
      { code: 'NAD', name: 'Namibian Dollar', symbol: 'N$', decimalDigits: 2, regions: ['south-africa'] },
      { code: 'BWP', name: 'Botswana Pula', symbol: 'P', decimalDigits: 2, regions: ['south-africa'] },
      { code: 'LSL', name: 'Lesotho Loti', symbol: 'L', decimalDigits: 2, regions: ['south-africa'] },
      { code: 'SZL', name: 'Swazi Lilangeni', symbol: 'E', decimalDigits: 2, regions: ['south-africa'] },
      { code: 'KES', name: 'Kenyan Shilling', symbol: 'KSh', decimalDigits: 2, regions: ['east-africa'] },
      { code: 'UGX', name: 'Ugandan Shilling', symbol: 'USh', decimalDigits: 0, regions: ['east-africa'] },
      { code: 'TZS', name: 'Tanzanian Shilling', symbol: 'TSh', decimalDigits: 0, regions: ['east-africa'] },
      { code: 'RWF', name: 'Rwandan Franc', symbol: 'FRw', decimalDigits: 0, regions: ['east-africa'] },
      { code: 'NGN', name: 'Nigerian Naira', symbol: '₦', decimalDigits: 2, regions: ['west-africa'] },
      { code: 'GHS', name: 'Ghanaian Cedi', symbol: '₵', decimalDigits: 2, regions: ['west-africa'] },
      { code: 'XOF', name: 'West African CFA Franc', symbol: 'CFA', decimalDigits: 0, regions: ['west-africa'] },
      { code: 'EGP', name: 'Egyptian Pound', symbol: 'E£', decimalDigits: 2, regions: ['north-africa'] },
      { code: 'MAD', name: 'Moroccan Dirham', symbol: 'DH', decimalDigits: 2, regions: ['north-africa'] },
      { code: 'TND', name: 'Tunisian Dinar', symbol: 'DT', decimalDigits: 3, regions: ['north-africa'] },
      { code: 'USD', name: 'US Dollar', symbol: '$', decimalDigits: 2, regions: ['global'] },
      { code: 'EUR', name: 'Euro', symbol: '€', decimalDigits: 2, regions: ['global'] },
      { code: 'GBP', name: 'British Pound', symbol: '£', decimalDigits: 2, regions: ['global'] }
    ];
  }
  ...
  async getEnabledCurrencies(organizationId: string): Promise<CurrencyInfo[]> {
    ...
    const enabled = allCurrencies.filter(c => config.enabledCurrencies.includes(c.code));
   ...
   */
  async getCurrencyInfo(currencyCode: string): Promise<CurrencyInfo | undefined> {
  ...
  async getExchangeRate(fromCurrency: string, toCurrency: string): Promise<ExchangeRate> {
    ...
    if (cachedRate && Date.now() - cachedRate.lastUpdated.getTime() < this.CACHE_TTL) {
      return cachedRate;
    }
  ...
  async convertPrice(
    ...
    if (options.usePsychologicalPricing) {
      convertedPrice = this.applyPsychologicalPricing(convertedPrice, precision);
    }
  ...
  async convertMultiplePrices(
    ...
    const results = await Promise.all(items.map(async item => {
      ...
      calculatedPrice = this.roundPrice(calculatedPrice, roundingMethod, precision);
      
      // Apply psychological pricing if enabled
      if (options.usePsychologicalPricing) {
        calculatedPrice = this.applyPsychologicalPricing(calculatedPrice, precision);
      }
  ...
  async getCurrenciesForRegion(region: string): Promise<CurrencyInfo[]> {
    ...
    return allCurrencies.filter(currency => 
      currency.regions?.includes(region) || currency.regions?.includes('global')
    );
  }
  ...
  async getDefaultCurrencyForRegion(region: string): Promise<CurrencyInfo> {
    ...
    const currencyInfo = regionCurrencies.find(c => c.code === defaultCurrency);
    
    if (!currencyInfo) {
      // Fallback to the first currency in the region
      return regionCurrencies[0] || {
        code: 'USD',
        name: 'US Dollar',
        symbol: '$',
        decimalDigits: 2,
  ...
  private initializeExchangeRates(): void {
    ...
    const baseRates: Record<string, number> = {
      ...
      'GBP': 0.78,
      'ZAR': 18.20,
      'NAD': 18.20, // Pegged to ZAR
      'BWP': 13.45,
      'LSL': 18.20, // Pegged to ZAR
      'SZL': 18.20, // Pegged to ZAR
      'KES': 130.55,
      'UGX': 3760.50,
      'TZS': 2520.75,
      'RWF': 1250.30,
    ...
    for (const fromCurrency of Object.keys(baseRates)) {
      for (const toCurrency of Object.keys(baseRates)) {
        if (fromCurrency !== toCurrency) {
          ...
          // Add to cache
          this.exchangeRateCache.set(`${fromCurrency}_${toCurrency}`, exchangeRate);
        }
      }
    }
  }
   ...
   */
  private fetchExchangeRate(fromCurrency: string, toCurrency: string): ExchangeRate | undefined {
    const cacheKey = `${fromCurrency}_${toCurrency}`;
    const cachedRate = this.exchangeRateCache.get(cacheKey);
   ...
   */
  private roundPrice(price: number, method: 'up' | 'down' | 'nearest', precision: number): number {
