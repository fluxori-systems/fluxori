...
export class ProductReviewService {
    ...
    try {
      ...
      const isFeatureEnabled =
        this.featureFlagService?.isEnabled(
          'pim.reviews.autoModeration',
          organizationId,
        ) ?? true;

      if (
        this.autoModerationEnabled &&
        isFeatureEnabled &&
        loadSheddingStatus.currentStage < 3
      ) {
        try {
          // Queue analysis as a non-critical operation during load shedding
          await this.loadSheddingService.queueOperation(
            async () => {
              const analysisResult = await this.analyzeReview(
                review.title,
                review.content,
              );
          ...
          this.logger.warn(
            `Auto-moderation failed, defaulting to PENDING: ${error.message}`,
          );
      ...
      if (this.aiEnabled && !sentiment && loadSheddingStatus.currentStage < 4) {
        this.loadSheddingService.queueOperation(
          async () => {
  ...
  async updateReview(
    ...
    try {
      ...
      if (review.organizationId !== organizationId) {
        throw new Error(
          `Review with ID ${reviewId} does not belong to this organization`,
        );
      }
  ...
  async getPendingReviews(
    ...
    try {
      return await this.productReviewRepository.findPendingReviews(
        organizationId,
        limit,
      );
    } catch (error) {
        ...
        action:
          status === ProductReviewStatus.APPROVED
            ...
            : status === ProductReviewStatus.REJECTED
              ? 'reject'
              : status === ProductReviewStatus.FLAGGED
                ? 'flag'
                : 'none',
      };
  ...
  async getReviewStatistics(
    ...
    try {
      ...
      return await this.productReviewRepository.calculateReviewStatistics(
        productId,
      );
    } catch (error) {
      this.logger.error(
   ...
   * @param organizationId Organization ID
   */
  async getReviewDashboardStatistics(organizationId: string): Promise<{
    ...
    try {
      ...
      const reviewCountByStatus =
        await this.productReviewRepository.countReviewsByStatus(organizationId);

      // Get pending moderation count
      const pendingModeration =
        reviewCountByStatus[ProductReviewStatus.PENDING];
      ...
      const recentReviewCount =
        await this.productReviewRepository.count(recentQuery);
      ...
      reviewsWithSentiment.forEach((review) => {
        ...
        if (sentiment.positiveAspects) {
          sentiment.positiveAspects.forEach((aspect) => {
            positiveAspectsMap.set(
              aspect,
              (positiveAspectsMap.get(aspect) || 0) + 1,
            );
          });
        }
        ...
        if (sentiment.negativeAspects) {
          sentiment.negativeAspects.forEach((aspect) => {
            negativeAspectsMap.set(
              aspect,
              (negativeAspectsMap.get(aspect) || 0) + 1,
            );
          });
        }
   ...
   * @private
   */
  private async processDeferredSentimentAnalysis(
    reviewId: string,
  ): Promise<void> {
    try {
      ...
      if (!this.aiEnabled || !this.agentService || !this.creditService) {
        this.logger.warn(
          'Skipping sentiment analysis - AI features not enabled',
        );
        return;
      }
      ...
      if (!hasCredits) {
        this.logger.warn(
          `Organization ${review.organizationId} has insufficient credits for sentiment analysis`,
        );
        return;
      }
      ...
      this.logger.error(
        `Error processing sentiment analysis: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  private async analyzeReview(
    ...
    try {
      ...
      this.logger.error(
        `Error analyzing review: ${error.message}`,
        error.stack,
      );
  ...
  async processPendingSentimentAnalysisBatch(
    ...
    try {
      ...
      if (!this.aiEnabled || !this.agentService || !this.creditService) {
        this.logger.warn(
          'Skipping batch sentiment analysis - AI features not enabled',
        );
        return 0;
      }
      ...
      if (loadSheddingStatus.currentStage > 4) {
        this.logger.warn(
          'Skipping batch sentiment analysis due to severe load shedding',
        );
        return 0;
      }
      ...
      const reviews =
        await this.productReviewRepository.findReviewsForSentimentAnalysis(
          organizationId,
          batchSize,
        );
      ...
      if (!hasCredits) {
        this.logger.warn(
          `Organization ${organizationId} has insufficient credits for batch sentiment analysis`,
        );
        return 0;
      }
      ...
      await this.loadSheddingService.executeBatchWithResilience(
        ...
        async (review) => {
          try {
            ...
            this.logger.error(
              `Error processing review ${review.id}: ${error.message}`,
            );
          }
        },
        {
