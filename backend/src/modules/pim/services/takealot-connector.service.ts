...
import { ProductMarketplaceMapping, MarketplaceSyncResult } from '../models/marketplace-mapping.model';
...
export class TakealotConnectorService {
  ...
  async transformToTakealotFormat(
    ...
    const takealotProduct: TakealotProduct = {
      ...
      shortDescription: product.shortDescription || product.description.substring(0, 255),
    ...
    if (networkInfo.connectionQuality !== 'high' && takealotProduct.images.length > 0) {
  ...
  async syncProductToTakealot(
    ...
    try {
      ...
      this.logger.error(`Error syncing product ${productId} to Takealot: ${error.message}`, error.stack);
  ...
  private async performTakealotSync(
    ...
    const variants = await this.variantService.findByParentId(productId, tenantId);
  ...
  private async createNewTakealotProduct(
    ...
    try {
      ...
      this.logger.error(`Error creating new product in Takealot: ${error.message}`, error.stack);
  ...
  async getTakealotCategories(
    ...
    try {
      ...
      if (!categoriesResult.success) {
        throw new Error(`Failed to get Takealot categories: ${categoriesResult.error?.message}`);
      }
      ...
      this.logger.error(`Error getting Takealot categories: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getCategoryAttributes(
    ...
    try {
      ...
      const attributesResult = await connector.getCategoryAttributes(categoryId);
      
      if (!attributesResult.success) {
        throw new Error(`Failed to get category attributes: ${attributesResult.error?.message}`);
      }
      
      return attributesResult.data;
    } catch (error) {
      this.logger.error(`Error getting category attributes: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async updateTakealotStock(
    ...
    try {
      ...
      if (result.success) {
        await this.mappingRepository.updateSyncStatus(mapping.id, 'active', tenantId);
      } else {
        await this.mappingRepository.updateSyncStatus(
          mapping.id, 
          'error', 
      ...
      this.logger.error(`Error updating Takealot stock: ${error.message}`, error.stack);
  ...
  async updateTakealotPrice(
    ...
    try {
      ...
      const rrp = mapping?.southAfrica?.takealot?.rrp || Math.round(price * 1.15 * 100) / 100;
      ...
      this.logger.error(`Error updating Takealot price: ${error.message}`, error.stack);
  ...
  private async getTakealotConnector(tenantId: string): Promise<any> {
    try {
      ...
      this.logger.error(`Error getting Takealot connector: ${error.message}`, error.stack);
      throw new Error(`Could not initialize Takealot connector: ${error.message}`);
    }
  }
