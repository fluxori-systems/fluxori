...
import { NetworkStatusService, NetworkStatus } from '../../../common/utils/network-status.service';
import { ProductImage, CompressionQuality, ResizeOption } from '../models/image.model';
...
export class PimStorageService {
  ...
  constructor(
    ...
    this.bucketName = this.configService.get<string>('GCS_BUCKET_NAME') || 'fluxori-uploads';
  }
  ...
  async getSignedUploadUrl(options: {
  ...
  }): Promise<{ url: string; fields: Record<string, string>; networkQuality?: string }> {
    const { fileName, contentType, expiresInMinutes, metadata = {}, organizationId } = options;

    // Get current network status to adapt to conditions
    const networkStatus = await this.networkStatusService.getNetworkStatus(organizationId);
    ...
    if (!networkStatus.isStable || !networkStatus.isSufficientBandwidth) {
      ...
      this.logger.log(`Extended URL expiration to ${adaptedExpiresInMinutes} minutes due to network conditions`);
    }
  ...
  async getSignedDownloadUrl(
    ...
    // Get the signed URL
    const url = await this.storageService.getSignedDownloadUrl(filePath, adaptedExpiresInMinutes);
  ...
  async uploadProductImage(
    ...
    const cdnDomain = this.configService.get<string>('CDN_DOMAIN') || 'cdn.fluxori.com';
    const cdnUrl = `https://${cdnDomain}/${storagePath}`;
    
    // Generate thumbnail URLs if requested
    const thumbnails: Record<string, string> = {};
    if (generateThumbnails) {
      const fileNameWithoutExt = storagePath.substring(0, storagePath.lastIndexOf('.'));
   ...
   * @returns Product image details
   */
  async getProductImage(imageId: string, organizationId?: string): Promise<ProductImage> {
    try {
      ...
      const imageFile = files.find(file => file.metadata?.imageId === imageId);
      ...
      this.logger.error(`Failed to get product image: ${errorMessage}`, errorStack);
  ...
  async getProductImages(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Failed to get product images: ${errorMessage}`, errorStack);
  ...
  async updateProductImage(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Failed to update product image: ${errorMessage}`, errorStack);
   ...
   * @param organizationId Organization ID
   */
  async deleteProductImage(imageId: string, organizationId?: string): Promise<void> {
    try {
      ...
      if (image.thumbnails) {
        const fileNameWithoutExt = image.storagePath.substring(0, image.storagePath.lastIndexOf('.'));
        const fileExt = image.storagePath.substring(image.storagePath.lastIndexOf('.'));
        
        // Try to delete all thumbnails (don't fail if some don't exist)
        try {
          await Promise.all([
            this.storageService.deleteFile(`${fileNameWithoutExt}_thumbnail${fileExt}`).catch(() => {}),
            this.storageService.deleteFile(`${fileNameWithoutExt}_small${fileExt}`).catch(() => {}),
            this.storageService.deleteFile(`${fileNameWithoutExt}_medium${fileExt}`).catch(() => {}),
            this.storageService.deleteFile(`${fileNameWithoutExt}_large${fileExt}`).catch(() => {}),
          ]);
        } catch (thumbnailError) {
          // Log but don't fail the operation
          const errorMessage = thumbnailError instanceof Error ? thumbnailError.message : String(thumbnailError);
          this.logger.warn(`Some thumbnails could not be deleted: ${errorMessage}`);
        }
      }
    } catch (error) {
      if (error instanceof NotFoundException) {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Failed to delete product image: ${errorMessage}`, errorStack);
  ...
  private getRecommendedConnectionType(
    ...
    if (resizeOption === ResizeOption.THUMBNAIL || 
    ...
    if (resizeOption === ResizeOption.SMALL || 
        (compressionQuality === CompressionQuality.MEDIUM && optimizeForLowBandwidth)) {
      return '3G or better';
    }
    
    // For medium images or adaptive compression
    if (resizeOption === ResizeOption.MEDIUM || compressionQuality === CompressionQuality.ADAPTIVE) {
      return '3G+ or better';
    }
    
    // For large or high quality images
    if (resizeOption === ResizeOption.LARGE || compressionQuality === CompressionQuality.HIGH) {
