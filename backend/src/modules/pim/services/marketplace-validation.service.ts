...
export class MarketplaceValidationService {
  ...
  private validateRequiredFields(
    ...
    if (!product.description || product.description.length < 20) {
      errors.description = [
        'Product description is required and should be at least 20 characters long',
      ];
  ...
  private validateSouthAfricanRequirements(
    ...
    if (!hasBarcode) {
      warnings.barcode = [
        'South African barcode is recommended for Takealot listings',
      ];
    }
    ...
    if (!product.dimensions || product.dimensions.length !== 3) {
      warnings.dimensions = [
        'Product dimensions are recommended for shipping calculations',
      ];
    ...
    if (!product.weight || product.weight <= 0) {
      warnings.weight = [
        'Product weight is recommended for shipping calculations',
      ];
      fields.weight = false;
    } else {
      fields.weight = true;
  ...
  private validateRegulatoryCompliance(
    ...
    if (category === 'electronics') {
      ...
      const hasIcasa =
        product.regional?.southAfrica?.icasaApproved ||
      ...
      if (!hasIcasa) {
        ...
        warnings.compliance.push(
          'ICASA approval is recommended for electronic products',
        );
      }
    } else if (category === 'appliances') {
      // Appliances may require SABS approval
      const hasSabs =
        product.regional?.southAfrica?.sabsApproved ||
      ...
      if (!hasSabs) {
        warnings.compliance = warnings.compliance || [];
        warnings.compliance.push(
          'SABS approval may be required for certain appliances',
        );
      }
    }
    ...
    if (this.requiresNrcs(product)) {
      const hasNrcs =
        product.regional?.southAfrica?.nrcsApproved ||
      ...
      if (!hasNrcs) {
        ...
        errors.compliance.push(
          'NRCS approval is required for this product category',
        );
      }
    }
  }
  ...
  private validateProductAttributes(
    ...
    if (!product.attributes || product.attributes.length < 3) {
      warnings.attributes = [
        'Product should have at least 3 attributes for good marketplace visibility',
      ];
    ...
    const hasColor = product.attributes.some(
      (attr) => attr.code === 'color' || attr.code === 'colour',
    );

    const hasBrand = product.attributes.some(
      (attr) => attr.code === 'brand' || attr.code === 'manufacturer',
    );

    const hasMaterial = product.attributes.some(
      (attr) => attr.code === 'material' || attr.code === 'fabric',
    );
    ...
    if (!hasBrand) {
      warnings.brand = [
        'Brand information is important for marketplace search',
      ];
    }
    ...
    if (!hasMaterial && this.isMaterialRelevant(product)) {
      warnings.material = [
        'Material information is helpful for this product type',
      ];
    }
  }
  ...
  private validateContentQuality(
    ...
    if (product.name.length < 20) {
      warnings.titleLength = [
        'Title is shorter than recommended (20+ characters)',
      ];
      fields.titleLength = false;
      recommendations.push(
        'Extend the product title to include more keywords and details',
      );
    } else if (product.name.length > 150) {
      warnings.titleLength = [
        'Title is longer than recommended (max 150 characters)',
      ];
      fields.titleLength = false;
    } else {
    ...
    if (product.description.length < 100) {
      warnings.descriptionLength = [
        'Description is shorter than recommended (100+ characters)',
      ];
      fields.descriptionLength = false;
      recommendations.push(
        'Expand the product description with more details and features',
      );
    ...
    if (imageCount < 3) {
      warnings.imageCount = [
        'Product should have at least 3 images for good presentation',
      ];
  ...
  private validateVariants(
    ...
    const hasAllPrices = variants.every(
      (v) => v.pricing && v.pricing.basePrice > 0,
    );
    ...
    const hasAllStocks = variants.every(
      (v) => v.stock && v.stock.quantity !== undefined,
    );
    ...
    const hasAllAttributes = variants.every(
    ...
    if (!hasAllAttributes) {
      errors.variantAttributes = [
        'All variants must have distinguishing attributes',
      ];
    }
  }
  ...
  private calculateValidationScore(
    ...
    const finalScore = Math.max(0, rawScore - errorPenalty - warningPenalty);
    ...
    switch (marketplaceId.toLowerCase()) {
      ...
      default:
        return {
          valid: false,
          errors: {
            marketplace: [`Validation for ${marketplaceId} not supported`],
          },
          warnings: {},
          recommendations: [],
          score: 0,
