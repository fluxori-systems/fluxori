...
export class RegionalConfigurationService {
  ...
  async getRegionById(
    ...
    // Get from repository
    const region = await this.regionalConfigRepository.findById(
      regionId,
      tenantId,
    );
  ...
  async createRegion(
    ...
    const created = await this.regionalConfigRepository.create(
      newRegion,
      tenantId,
    );
  ...
  async updateRegion(
    ...
    const result = await this.regionalConfigRepository.update(
      updated,
      tenantId,
    );
  ...
  async deleteRegion(regionId: string, tenantId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Failed to delete region ${regionId}: ${error.message}`,
      );
      return false;
    }
  }
   ...
   */
  async isFeatureEnabledForRegion(
    regionId: string,
    featureKey: string,
    tenantId: string,
  ): Promise<boolean> {
   ...
   */
  async checkCurrencyPriceUpdate(
    productId: string,
    regionId: string,
    tenantId: string,
  ): Promise<boolean> {
