...
export class MarketContextService implements IMarketContextProvider {
   ...
   * @returns The market context
   */
  async getMarketContextByRegion(
    region: string,
    country: string,
  ...
  async isFeatureAvailable(
    ...
    if (feature in context.features) {
      const featureEnabled =
        context.features[feature as keyof typeof context.features];
  ...
  async getVatRate(region: string, date: Date = new Date()): Promise<number> {
    switch (region) {
      ...
      case 'europe':
        ...
        return 0.2; // 20% as a common EU VAT rate
