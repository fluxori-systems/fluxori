...
export enum ComplianceAuthority {
  ...
  ICASA = 'icasa', // Independent Communications Authority of South Africa
  SAHPRA = 'sahpra', // South African Health Products Regulatory Authority
  SARS = 'sars', // South African Revenue Service
  
  // Continental African authorities
  ARSO = 'arso', // African Organisation for Standardisation
  AFSEC = 'afsec', // African Electrotechnical Standardization Commission
...
export interface ComplianceRule {
  ...
  regionCodes: string[]; // ISO country codes where rule applies
  productTypes: string[]; // Product types rule applies to
  requiredAttributes: string[]; // Attributes that must be present
...
export interface ComplianceValidationRule {
  id: string;
  type: 'attribute_presence' | 'attribute_value' | 'file_presence' | 'certificate_validity' | 'custom';
  attribute?: string; // Target attribute to validate
  condition?: string; // Validation condition in human-readable form
  validationExpression?: string; // Rule expression
...
export interface ComplianceRequirement {
  ...
  requiredBy: Date; // When compliance must be achieved
  documentationUrls?: string[]; // Supporting documentation
  certificateIds?: string[]; // Associated certificates
  notes?: string;
...
export class ComplianceFrameworkService {
  ...
  async checkProductCompliance(
    ...
    for (const rule of rules) {
      ...
      await this.updateComplianceRequirement(productId, rule.id, result, tenantId);
    }
    
    // Log compliance check
    this.logger.debug(`Completed compliance check for product ${productId} with ${results.length} rules`);
  ...
  async getProductComplianceStatus(
    ...
    const requirements = await this.complianceRequirementRepository.findByProduct(
      productId,
      tenantId,
    );
  ...
  async getApplicableRules(
    ...
    const now = new Date();
    if (now.getTime() - this.ruleCacheTimestamp.getTime() > this.RULE_CACHE_TTL) {
    ...
    const rules = await this.complianceRuleRepository.findByFilters(filters, tenantId);
  ...
  private async checkRuleCompliance(
    ...
    for (const attr of rule.requiredAttributes) {
      const hasAttribute = product.attributes && 
    ...
    for (const validationRule of rule.validationRules) {
      ...
      switch (validationRule.type) {
        case 'attribute_presence':
          ...
          if (validationRule.attribute) {
            passed = product.attributes && 
                    Object.prototype.hasOwnProperty.call(product.attributes, validationRule.attribute) &&
            ...
            message = passed ? 
              ...
              validationRule.errorMessage || `Missing required attribute: ${validationRule.attribute}`;
          }
          break;
          ...
          if (validationRule.attribute && validationRule.validationExpression) {
            ...
            if (attrValue !== undefined) {
              try {
                ...
                const expr = validationRule.validationExpression.replace('value', JSON.stringify(attrValue));
                passed = eval(expr); // eslint-disable-line no-eval
                message = passed ? 
                  `Value of ${validationRule.attribute} is valid` : 
                  validationRule.errorMessage || `Invalid value for ${validationRule.attribute}`;
              } catch (error) {
                this.logger.error(`Error evaluating validation expression: ${error.message}`);
                passed = false;
        ...
        case 'file_presence':
          ...
          if (validationRule.attribute) {
            ...
            passed = fileAttr && typeof fileAttr === 'string' && fileAttr.trim() !== '';
            message = passed ? 
              `File ${validationRule.attribute} is present` : 
              validationRule.errorMessage || `Missing required file: ${validationRule.attribute}`;
          }
          break;
          ...
          if (validationRule.attribute) {
            ...
            if (certId) {
              ...
              message = passed ? 
                ...
                validationRule.errorMessage || `Certificate ${certId} is invalid or expired`;
            } else {
              passed = false;
        ...
        case 'custom':
          ...
          this.logger.debug(`Custom validation not implemented for rule ${validationRule.id}`);
          passed = false;
      ...
      validationResults.push({
        ...
        rule: validationRule,
        message,
        attributes: validationRule.attribute ? { [validationRule.attribute]: product.attributes?.[validationRule.attribute] } : undefined,
      });
    ...
    if (missingRequirements.length > 0) {
      recommendations.push(`Add the following required attributes: ${missingRequirements.join(', ')}`);
    }
  ...
  private async updateComplianceRequirement(
    ...
    const existingReq = await this.complianceRequirementRepository.findByProductAndRule(
      productId,
      ruleId,
      tenantId,
    );
    ...
    if (existingReq) {
      ...
      await this.complianceRequirementRepository.create(newRequirement, tenantId);
    }
  }
   ...
   */
  async createRule(rule: Omit<ComplianceRule, 'id' | 'createdAt' | 'updatedAt'>, tenantId: string): Promise<ComplianceRule> {
    ...
    const createdRule = await this.complianceRuleRepository.create(newRule, tenantId);
   ...
   */
  async updateRule(ruleId: string, updates: Partial<ComplianceRule>, tenantId: string): Promise<ComplianceRule> {
    // Get existing rule
    ...
    const result = await this.complianceRuleRepository.update(updatedRule, tenantId);
  ...
  async updateComplianceStatus(
    ...
    if (!existingReq) {
      throw new Error(`Compliance requirement for product ${productId} and rule ${ruleId} not found`);
    }
   ...
   */
  async getRequirementsByProduct(productId: string, tenantId: string): Promise<ComplianceRequirement[]> {
    return this.complianceRequirementRepository.findByProduct(productId, tenantId);
  }
   ...
   */
  async getComplianceHistory(productId: string, ruleId: string, tenantId: string): Promise<ComplianceStatusChange[]> {
    const requirement = await this.complianceRequirementRepository.findByProductAndRule(
      productId,
      ruleId,
      tenantId,
    );
