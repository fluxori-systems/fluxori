...
export class ProductRepository extends TenantAwareRepository<Product> {
  ...
  async findByFilter(filter: ProductFilter): Promise<Product[]> {
    ...
    if (filter.skuPattern) {
      ...
      const upperBound =
        filter.skuPattern.slice(0, -1) +
        String.fromCharCode(
          filter.skuPattern.charCodeAt(filter.skuPattern.length - 1) + 1,
        );
      advancedFilters.push({
        field: 'sku',
        operator: '<',
    ...
    let products = await this.paginate(queryOptions).then(
      (result) => result.items,
    );
  ...
  async findByMarketplace(
    ...
    const filteredProducts = products.filter((product) =>
      product.marketplaceMappings?.some(
        (mapping) => mapping.marketplaceId === marketplaceId,
      ),
    );
