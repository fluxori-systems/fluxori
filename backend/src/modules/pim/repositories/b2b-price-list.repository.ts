...
import { FindByIdOptions, FindOptions, UpdateDocumentOptions, CreateDocumentOptions, DeleteDocumentOptions } from '../../../common/repositories/base/repository-types';

/**
 * Repository for B2B price lists
 */
@Injectable()
export class B2BPriceListRepository extends FirestoreBaseRepository<B2BPriceList, string> {
   ...
   */
  async findByCustomerTier(customerTierId: string, organizationId: string): Promise<B2BPriceList[]> {
    const query = this.collection
   ...
   */
  async findByCustomer(customerId: string, organizationId: string): Promise<B2BPriceList[]> {
    const query = this.collection
   ...
   */
  async findByProduct(productId: string, organizationId: string): Promise<B2BPriceList[]> {
    const query = this.collection
   ...
   */
  async findByRegion(region: string, organizationId: string): Promise<B2BPriceList[]> {
    const query = this.collection
   ...
   */
  async getProductPrice(priceListId: string, productId: string): Promise<any | null> {
  ...
  async updateProductPrice(
    ...
    return this.update(priceListId, {
      ...
      productIds: [...new Set([...priceList.productIds || [], productPrice.productId])]
    });
  }
   ...
   * @returns The updated price list
   */
  async removeProductPrice(priceListId: string, productId: string): Promise<B2BPriceList> {
    const priceList = await this.findById(priceListId);
    
    if (!priceList || !priceList.prices) {
      throw new Error(`Price list with ID ${priceListId} not found or has no prices`);
    }
    
    // Remove price
    const updatedPrices = priceList.prices.filter(p => p.productId !== productId);
    
    // Remove from product IDs as well if no other prices use this product
    const updatedProductIds = updatedPrices.some(p => p.productId === productId)
   ...
   */
  async findByDateRange(startDate: Date, endDate: Date | null, organizationId: string): Promise<B2BPriceList[]> {
    ...
    const priceLists = snapshot.docs.map(doc => this.mapSnapshotToEntity(doc));
  ...
  async findByIds(ids: string[], options?: FindByIdOptions): Promise<B2BPriceList[]> {
    // Implementation details would go here
    // This is just a stub to satisfy TypeScript
  ...
  async findBy(field: keyof B2BPriceList | string, value: any, options?: FindOptions<B2BPriceList>): Promise<B2BPriceList[]> {
    // Implementation details would go here
    // This is just a stub to satisfy TypeScript
  ...
  async findOneBy(field: keyof B2BPriceList | string, value: any, options?: FindOptions<B2BPriceList>): Promise<B2BPriceList | null> {
    // Implementation details would go here
    // This is just a stub to satisfy TypeScript
  ...
  async createMany(items: Array<Omit<B2BPriceList, 'id'>>, options?: CreateDocumentOptions): Promise<B2BPriceList[]> {
    // Implementation details would go here
    // This is just a stub to satisfy TypeScript
    const results: B2BPriceList[] = [];
  ...
  async updateMany(items: Array<{ id: string; data: Partial<B2BPriceList> }>, options?: UpdateDocumentOptions): Promise<B2BPriceList[]> {
    // Implementation details would go here
    // This is just a stub to satisfy TypeScript
    const results: B2BPriceList[] = [];
  ...
  async deleteMany(ids: string[], options?: DeleteDocumentOptions): Promise<void> {
    // Implementation details would go here
    // This is just a stub to satisfy TypeScript
    for (const id of ids) {
