...
export class TaxRateRepository extends TenantAwareRepository<TaxRateSchedule> {
  ...
  async findSpecialRate(
    ...
    if (!productType && (!productCategories || productCategories.length === 0)) {
      return null;
    }
    ...
    if (productType) {
      const productTypeMatch = validRates.find(rate => 
    ...
    if (productCategories && productCategories.length > 0) {
      const categoryMatch = validRates.find(rate => {
        if (!rate.metadata?.categories || !Array.isArray(rate.metadata.categories)) {
          return false;
        }
