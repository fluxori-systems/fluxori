...
describe("AttributeTemplateService", () => {
  ...
  beforeEach(async () => {
    ...
    attributeTemplateRepository = module.get<AttributeTemplateRepository>(AttributeTemplateRepository);
  });
  ...
  describe("createAttributeTemplate", () => {
    ...
    it("should validate regional templates have a region specified", async () => {
      ...
      // Execute and assert
      await expect(service.createAttributeTemplate(createTemplateDto))
        .rejects
        .toThrow("Region must be provided for regional attribute templates");
    });
  });
  ...
  describe("getAttributeTemplatesByCategory", () => {
    it("should return attribute templates for a category", async () => {
      ...
      // Execute
      const result = await service.getAttributeTemplatesByCategory("org-123", "cat-123");

      // Assert
      expect(result).toEqual(templates);
      expect(attributeTemplateRepository.findByCategory).toHaveBeenCalledWith("org-123", "cat-123");
    });
  });
  ...
  describe("getAttributeTemplateById", () => {
    it("should return a template when it exists and belongs to the organization", async () => {
      ...
      // Execute
      const result = await service.getAttributeTemplateById("template-123", "org-123");

      // Assert
      expect(result).toEqual(template);
      expect(attributeTemplateRepository.findById).toHaveBeenCalledWith("template-123");
    });
  ...
  describe("getApplicableAttributeTemplates", () => {
    it("should return applicable attribute templates for a product", async () => {
      ...
      expect(attributeTemplateRepository.findGlobalApplicable).toHaveBeenCalledWith("org-123");
      expect(attributeTemplateRepository.findByCategory).toHaveBeenCalledWith("org-123", "cat-123");
      expect(attributeTemplateRepository.findByRegion).toHaveBeenCalledWith("org-123", "south-africa");
      expect(attributeTemplateRepository.findByMarketplace).toHaveBeenCalledWith("org-123", "takealot");
    });
  });
});