...
export class InsightController {
   ...
   */
  @Post()
  async create(@Body() createInsightDto: CreateInsightDto): Promise<InsightResponse> {
   ...
   */
  @Get()
  async findWithFilters(@Query() queryDto: QueryInsightsDto): Promise<InsightResponse[]> {
  ...
  async deleteExpired(
    ...
    const count = await this.insightService.cleanupExpiredInsights(organizationId);
    return { count };
  }
  ...
  private mapToResponse(insight: Insight): InsightResponse {
    return {
      ...
      generatedAt: insight.generatedAt instanceof Date 
        ? insight.generatedAt 
        : new Date(insight.generatedAt),
      expiresAt: insight.expiresAt instanceof Date 
        ? insight.expiresAt 
        : insight.expiresAt ? new Date(insight.expiresAt) : undefined,
      acknowledgedAt: insight.acknowledgedAt instanceof Date 
        ? insight.acknowledgedAt 
        : insight.acknowledgedAt ? new Date(insight.acknowledgedAt) : undefined,
      acknowledgedBy: insight.acknowledgedBy,
      resolvedAt: insight.resolvedAt instanceof Date 
        ? insight.resolvedAt 
        : insight.resolvedAt ? new Date(insight.resolvedAt) : undefined,
