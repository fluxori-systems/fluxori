...
class CreditBalanceRepository extends FirestoreBaseRepository<CreditBalance> {
  ...
  async findByOrganization(
    ...
    const results = await this.find({
      filter: { organizationId } as Partial<CreditBalance>,
    });
...
export class CreditSystemService {
  ...
  constructor(private readonly firestoreConfigService: FirestoreConfigService) {
    ...
    this.balanceRepository = new CreditBalanceRepository(
      firestoreConfigService,
    );
  }
  ...
  async addCredits(addCreditsDto: AddCreditsDto): Promise<number> {
    this.logger.log(
      `Adding ${addCreditsDto.amount} credits for organization ${addCreditsDto.organizationId}`,
    );

    return this.firestoreConfigService
      .getFirestore()
      .runTransaction(async (transaction) => {
        // Get or create balance document
        let balance = await this.balanceRepository.findByOrganization(
          addCreditsDto.organizationId,
        );
  ...
  async useCredits(useCreditsDto: UseCreditsDto): Promise<number> {
    this.logger.log(
      `Using ${useCreditsDto.amount} credits for organization ${useCreditsDto.organizationId}`,
    );
    ...
    return this.firestoreConfigService
      ...
      .runTransaction(async (transaction) => {
        ...
        const balance = await this.balanceRepository.findByOrganization(
          useCreditsDto.organizationId,
        );

        if (!balance) {
          throw new Error(
            `No credit balance found for organization ${useCreditsDto.organizationId}`,
          );
        }
        ...
        if (balance.currentBalance < useCreditsDto.amount) {
          throw new Error(
            `Insufficient credits. Required: ${useCreditsDto.amount}, Available: ${balance.currentBalance}`,
          );
        }
  ...
  async getBalance(organizationId: string): Promise<number> {
    const balance =
      await this.balanceRepository.findByOrganization(organizationId);
  ...
  async getRecentTransactions(
    ...
    const transactions =
      await this.transactionRepository.findByOrganization(organizationId);
