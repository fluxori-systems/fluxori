...
export class InsightGenerationService {
   ...
   */
  async generateInsights(request: GenerateInsightsRequest): Promise<AIAnalysisResult> {
    this.logger.log(`Generating insights from ${request.dataType} data for organization ${request.organizationId}`);
    ...
    if (balance < estimatedCost) {
      throw new Error(`Insufficient credits. Required: ${estimatedCost}, Available: ${balance}`);
    }
    ...
    try {
      ...
      const result = await this.processWithAI(modelConfig, request.dataType, request.data);
  ...
  private async getModelConfig(
    ...
    if (providerName && modelName) {
      const configs = await this.modelConfigService.findByOrganization(organizationId);
      
      const matchingConfig = configs.find(
        config => config.modelProvider === providerName && 
    ...
    const defaultConfig = await this.modelConfigService.findDefaultConfig(organizationId);
    
    if (!defaultConfig) {
      throw new Error(`No AI model configuration found for organization ${organizationId}`);
    }
  ...
  private async processWithAI(
    ...
    this.logger.log(`Processing with ${modelConfig.modelProvider}/${modelConfig.modelName}`);
    ...
    if (dataType === 'inventory') {
      ...
      mockInsights.push({
        ...
        description: 'Several products with high sales velocity are approaching low stock levels.',
        severity: InsightSeverity.HIGH,
        confidence: 0.87,
        data: {
          affectedProducts: [
            { id: 'product-1', name: 'Product 1', stock: 5, salesVelocity: 2.3 },
            { id: 'product-2', name: 'Product 2', stock: 8, salesVelocity: 1.8 },
          ],
          recommendation: 'Restock these items within 3 days to avoid stockouts'
        }
      });
      ...
      mockInsights.push({
        ...
        description: 'Electronics category has shown 32% growth over the last 30 days.',
      ...
      mockInsights.push({
        ...
        description: 'Your top 5 products have high potential on Marketplace X based on competitive analysis.',
        severity: InsightSeverity.MEDIUM,
        confidence: 0.78,
        data: {
          ...
          recommendedProducts: ['product-3', 'product-7', 'product-9', 'product-11', 'product-15']
        }
      });
      ...
      mockInsights.push({
        ...
        description: 'Main competitor has reduced prices across 15 comparable products by an average of 12%.',
        severity: InsightSeverity.HIGH,
        confidence: 0.85,
        data: {
