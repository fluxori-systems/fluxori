...
export class SecurityController {
  ...
  async emergencyBlockIp(
    ...
    if (!request.user || request.user.role !== 'admin') {
      throw new UnauthorizedException(
        'Only administrators can block IP addresses',
      );
    }
  ...
  async grantEmergencyAccess(
    ...
    if (!request.user || request.user.role !== 'admin') {
      throw new UnauthorizedException(
        'Only administrators can grant emergency access',
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description: 'VPC Service Controls test results',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Cloud Armor security policy metrics',
  })
  ...
  async getCloudArmorRules(): Promise<any> {
    ...
    if (!request.user || request.user.role !== 'admin') {
      throw new UnauthorizedException(
        'Only administrators can view security policy rules',
      );
    }
