...
export class SecurityAuditService implements ISecurityAuditService {
  ...
  async recordAudit(
    ...
    try {
      ...
      this.logger.error(
        `Failed to record audit event: ${error.message}`,
        error.stack,
      );
  ...
  async queryAuditLogs(query: {
    ...
    try {
      ...
      let firestoreQuery: Query = this.firestore.collection(
        this.collectionName,
      );
  ...
  async exportAuditLogs(
    ...
    try {
      ...
      this.logger.log(
        `Would export ${records.length} audit records in ${format} format`,
      );
  ...
  async purgeAuditLogs(olderThan: Date): Promise<number> {
    ...
    try {
      ...
      const query: Query = this.firestore
        .collection(this.collectionName)
      ...
      this.logger.log(
        `Purged ${totalDeleted} audit records older than ${olderThan.toISOString()}`,
      );
      ...
      this.logger.error(
        `Failed to purge audit logs: ${error.message}`,
        error.stack,
      );
      this.observability.error(
        'Audit purge failed',
