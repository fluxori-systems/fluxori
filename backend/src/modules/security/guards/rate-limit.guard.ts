...
export class RateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const { allowed, current, limit, ttl } = await this.checkRateLimit(key, rateLimit);
    ...
    response.header('X-RateLimit-Reset', Math.ceil(Date.now() / 1000 + ttl).toString());
    ...
    if (!allowed) {
      ...
      this.logger.warn(`Rate limit exceeded: ${key}, current: ${current}, limit: ${limit}`);
      
      // Create a custom error message
      const errorMessage = rateLimit.errorMessage || 'Rate limit exceeded. Please try again later.';
  ...
  ): Promise<{ allowed: boolean; current: number; limit: number; ttl: number }> {
