...
export class FileScannerService {
  ...
  async scanFile(file: Buffer): Promise<{ clean: boolean; threats: string[] }> {
    ...
    try {
      ...
      if (!scanResult.clean) {
        this.observability.incrementCounter('security.fileScan.threats.detected');
      }
      ...
      this.observability.error('File scanning failed', error, FileScannerService.name);
   ...
   */
  private async uploadFileForScanning(file: Buffer, fileName: string): Promise<void> {
  ...
  private async triggerScannerFunction(fileName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to trigger scanner function: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  private async simulateScanResults(fileName: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to simulate scan results: ${error.message}`, error.stack);
    }
  }
   ...
   * Poll for scan results
   */
  private async pollForScanResults(scanId: string): Promise<{ clean: boolean; threats: string[] }> {
    const resultsBucket = this.storage.bucket(this.resultsBucket);
    const resultsFile = resultsBucket.file(`${scanId}/result.json`);
    ...
    while (Date.now() - startTime < this.scanTimeout) {
      try {
        ...
        this.logger.debug(`Error checking scan results (attempt ${attempt}): ${error.message}`);
      }
  ...
  private async cleanupScanFiles(scanId: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to clean up scan files: ${error.message}`, error.stack);
    }
  }
  ...
  async getServiceHealth(): Promise<{ status: string; error?: string }> {
    try {
      ...
      const [scanBucketExists] = await this.storage.bucket(this.scanBucket).exists();
      const [resultsBucketExists] = await this.storage.bucket(this.resultsBucket).exists();
      
      if (!scanBucketExists || !resultsBucketExists) {
        return {
          status: 'degraded',
          error: `Required buckets missing: ${!scanBucketExists ? this.scanBucket : ''} ${!resultsBucketExists ? this.resultsBucket : ''}`.trim(),
        };
      }
      ...
      this.logger.error(`File scanner health check failed: ${error.message}`, error.stack);
