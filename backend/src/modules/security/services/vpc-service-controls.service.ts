...
export class VpcServiceControlsService implements IVpcServiceControlsService {
  ...
  async configureServiceControls(config: VpcScConfiguration): Promise<void> {
    const span = this.observability.startTrace('security.configureServiceControls');
    ...
    try {
      ...
      this.logger.error(`Failed to configure VPC Service Controls: ${error.message}`, error.stack);
      this.observability.error('VPC Service Controls configuration failed', error, VpcServiceControlsService.name);
   ...
   */
  private async configureAccessLevels(accessLevels: VpcScConfiguration['accessLevels']): Promise<void> {
    if (!accessLevels || accessLevels.length === 0) {
      this.logger.warn('No access levels provided, skipping access level configuration');
      return;
    }
    ...
    for (const level of accessLevels) {
      try {
        ...
        try {
          const [existingLevel] = await this.accessContextManager.getAccessLevel({
        ...
        this.logger.error(`Error configuring access level ${level.name}: ${error.message}`, error.stack);
        throw error;
      }
    }
  }
   ...
   */
  private buildAccessConditions(level: VpcScConfiguration['accessLevels'][0]): Array<Record<string, any>> {
    const conditions = [];
   ...
   */
  private async configureServicePerimeters(perimeters: VpcScConfiguration['perimeters']): Promise<void> {
    if (!perimeters || perimeters.length === 0) {
      this.logger.warn('No perimeters provided, skipping perimeter configuration');
      return;
    }
    ...
    for (const perimeter of perimeters) {
      try {
        ...
        try {
          const [existingPerimeter] = await this.accessContextManager.getServicePerimeter({
            name: perimeterName,
          });
          
          if (existingPerimeter) {
            this.logger.log(`Updating existing service perimeter: ${perimeter.name}`);
            await this.updateServicePerimeter(perimeter, perimeterName);
          }
        } catch (error) {
          if (error.code === 5) { // NOT_FOUND
            this.logger.log(`Creating new service perimeter: ${perimeter.name}`);
        ...
        this.logger.error(`Error configuring service perimeter ${perimeter.name}: ${error.message}`, error.stack);
        throw error;
      }
    }
  }
   ...
   */
  private async createServicePerimeter(perimeter: VpcScConfiguration['perimeters'][0], parent: string): Promise<void> {
   ...
   */
  private async updateServicePerimeter(perimeter: VpcScConfiguration['perimeters'][0], name: string): Promise<void> {
    const projectResources = perimeter.projects.map(
      projectId => `projects/${projectId}`
    );
  ...
  async grantEmergencyAccess(
    ...
    const span = this.observability.startTrace('security.grantEmergencyAccess', {
      email,
      durationHours,
    });
    ...
    try {
      ...
      this.logger.warn(`Emergency access granted to ${email} for ${durationHours} hours. Reason: ${reason}`);
      ...
      this.logger.error(`Failed to grant emergency access: ${error.message}`, error.stack);
      this.observability.error('Emergency access grant failed', error, VpcServiceControlsService.name);
   ...
   */
  private scheduleEmergencyAccessRevocation(email: string, durationHours: number): void {
    // In a real implementation, this would create a Cloud Scheduler job
    // For this example, we'll just log the intent
    this.logger.log(`Scheduled emergency access revocation for ${email} in ${durationHours} hours`);
  }
  ...
  async testServiceControls(): Promise<{
    ...
    try {
      ...
      this.logger.error(`Failed to test VPC Service Controls: ${error.message}`, error.stack);
      this.observability.error('VPC Service Controls test failed', error, VpcServiceControlsService.name);
  ...
  async getServiceControlsStatus(): Promise<{
    ...
    const span = this.observability.startTrace('security.getServiceControlsStatus');
    ...
    try {
      ...
      this.logger.error(`Failed to get VPC Service Controls status: ${error.message}`, error.stack);
      this.observability.error('VPC Service Controls status check failed', error, VpcServiceControlsService.name);
