...
export class CredentialManagerService implements ICredentialManagerService {
  ...
  async getCredential(key: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error(`Failed to get credential: ${error.message}`, error.stack);
      this.observability.error('Credential access failed', error, CredentialManagerService.name);
  ...
  async storeCredential(
    ...
    try {
        ...
        await this.secretManager.createSecret({
          ...
          secret: {
            ...
            ...(options?.expireInDays ? {
              expireTime: {
                seconds: Math.floor(Date.now() / 1000) + (options.expireInDays * 86400),
              },
            } : {}),
          },
        });
      ...
      span.end();
      this.logger.log(`Credential stored successfully: ${this.maskSecretId(key)}`);
    } catch (error) {
      span.recordException(error);
      span.end();
      
      this.logger.error(`Failed to store credential: ${error.message}`, error.stack);
      this.observability.error('Credential storage failed', error, CredentialManagerService.name);
  ...
  async rotateCredential(key: string): Promise<string> {
    ...
    try {
      ...
      span.end();
      this.logger.log(`Credential rotated successfully: ${this.maskSecretId(key)}`);
      ...
      this.observability.error('Credential rotation failed', error, CredentialManagerService.name);
   ...
   */
  async listCredentials(): Promise<{ key: string; createdAt: Date; expiresAt?: Date }[]> {
    ...
    try {
      ...
      this.observability.error('Credential listing failed', error, CredentialManagerService.name);
  ...
  private generateCredential(currentValue: string): string {
    ...
    if (/^[A-Za-z0-9_-]+$/.test(currentValue)) {
      return crypto.randomBytes(Math.ceil(currentValue.length * 0.75))
        .toString('base64')
        .replace(/[+/=]/g, '')
