...
export class SecurityInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        this.logSecurityEvent(securityContext, 'request.success', durationMs, response.statusCode);
  ...
  private logSecurityEvent(
    ...
    if (!this.options.enableExtendedAuditLogging && event === 'request.success') {
      return;
    }
  ...
  private isSecurityError(error: any): boolean {
    ...
    const securityTerms = [
      'unauthorized', 'forbidden', 'permission', 'access denied',
      'csrf', 'xss', 'injection', 'token', 'security', 'authentication',
      'rate limit', 'permissions', 'firewall'
    ];
