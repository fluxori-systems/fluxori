...
export class SecurityHealthIndicator extends HealthIndicator {
  ...
  async isHealthy(key: string): Promise<HealthIndicatorResult> {
    ...
    try {
      ...
      const details = healthStatus.components.reduce(
        (acc, component) => {
  ...
  async checkSecurityComponents(): Promise<{
    ...
    try {
      ...
      const [
        ...
        this.checkComponentHealth('vpcServiceControls', () =>
          this.vpcServiceControls.getServiceControlsStatus().then((status) => ({
            status: status.enabled ? 'healthy' : 'degraded',
          })),
        ),
        this.checkComponentHealth('cloudArmor', () =>
          this.cloudArmor
            .getSecurityPolicyMetrics()
            .then(() => ({ status: 'healthy' })),
        ),
        this.checkComponentHealth('credentialManager', () =>
      ...
      if (securityHealth.status === 'fulfilled') {
        components.security = {
          ...
          components: securityHealth.value.components.reduce(
            (acc, component) => {
              acc[component.name] = {
      ...
      components.fileScanner =
        fileScannerHealth.status === 'fulfilled'
          ? { status: fileScannerHealth.value.status }
          : {
              status: 'unhealthy',
              error: fileScannerHealth.reason?.message || 'Unknown error',
            };

      // Process DLP health
      components.dlp =
        dlpHealth.status === 'fulfilled'
          ? { status: dlpHealth.value.status }
          : {
              status: 'unhealthy',
              error: dlpHealth.reason?.message || 'Unknown error',
            };

      // Process VPC Service Controls health
      components.vpcServiceControls =
        vpcHealth.status === 'fulfilled'
          ? { status: vpcHealth.value.status }
          : {
              status: 'unhealthy',
              error: vpcHealth.reason?.message || 'Unknown error',
            };

      // Process Cloud Armor health
      components.cloudArmor =
        cloudArmorHealth.status === 'fulfilled'
          ? { status: cloudArmorHealth.value.status }
          : {
              status: 'unhealthy',
              error: cloudArmorHealth.reason?.message || 'Unknown error',
            };

      // Process Credential Manager health
      components.credentialManager =
        credentialManagerHealth.status === 'fulfilled'
          ? { status: credentialManagerHealth.value.status }
          : {
              status: 'unhealthy',
              error: credentialManagerHealth.reason?.message || 'Unknown error',
            };
      ...
      this.logger.error(
        `Detailed security health check failed: ${error.message}`,
        error.stack,
      );
