  ...
  for (var _i = 0, _a = Object.entries(rules); _i < _a.length; _i++) {
    ...
    if (rule.required && (entity[key] === undefined || entity[key] === null)) {
      throw new RepositoryValidationError(
        'Missing required field: '.concat(key),
      );
    }
  }
  return true;
...
function validateRequiredFields(entity, requiredFields) {
  ...
  for (
    var _i = 0, requiredFields_1 = requiredFields;
    _i < requiredFields_1.length;
    _i++
  ) {
  ...
  if (missingFields.length > 0) {
    throw new common_1.BadRequestException(
      'Missing required fields: '.concat(missingFields.join(', ')),
    );
  }
}
/**
...
function validateEntityNotDeleted(entity, errorMessage) {
  if (errorMessage === void 0) {
    errorMessage = 'Entity is deleted';
  }
...
function validateBatchItems(items, minItems, maxItems) {
  ...
  if (items.length < minItems) {
    throw new common_1.BadRequestException(
      'Batch operation requires at least '.concat(minItems, ' item(s)'),
    );
  }
  if (items.length > maxItems) {
    throw new common_1.BadRequestException(
      'Batch operation cannot exceed '.concat(maxItems, ' items'),
    );
  }
}
