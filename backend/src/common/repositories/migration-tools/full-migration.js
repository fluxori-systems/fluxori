...
function updateImports(filePath) {
  ...
  const relPath = path.relative(
    fileDir,
    path.join(BACKEND_DIR, "src/common/repositories")
  ).replace(/\\/g, '/');
  ...
  if (content.includes("filters: [") && !content.includes("QueryFilter")) {
    ...
    if (!content.includes("QueryFilter")) {
      const typeImportPath = content.match(/from\s+['"]([^'"]*config\/firestore\.config)['"]/);
      if (typeImportPath) {
        const basePath = typeImportPath[1].replace(/config\/firestore\.config/, '');
        content = content.replace(
...
function fixQueryFilters(filePath, content) {
  ...
  content = content.replace(
    ...
    (match, options) => {
      ...
      const filterProperties = options.split(',').map(prop => prop.trim()).filter(prop => prop.length > 0);
...
async function main() {
  ...
  try {
    ...
    console.log("Please review the changes and run TypeScript compilation to verify.");
  } catch (err) {
