...
export class ServiceAuthUtils {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (this.isEnabled && !this.secret) {
      this.logger.warn('Service authentication is enabled but no secret is provided!');
    }
  }
  ...
  generateServiceToken(
    ...
    try {
      ...
      this.logger.error(`Error generating service token: ${error.message}`, error.stack);
      return '';
    }
  }
  ...
  validateServiceToken(
    ...
    try {
      ...
      this.logger.error(`Error validating service token: ${error.message}`, error.stack);
