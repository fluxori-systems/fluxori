...
var NetworkStatusService = (function () {
  ...
  var NetworkStatusService = (_classThis = /** @class */ (function () {
     ...
     */
    NetworkStatusService_1.prototype.getNetworkStatus = function (
      organizationId,
    ) {
     ...
     */
    NetworkStatusService_1.prototype.updateOrganizationNetworkStatus =
      function (organizationId, status) {
        var currentOrgStatus =
          this.recentTests.get(organizationId) ||
          __assign({}, this.currentStatus);
        var updatedStatus = __assign(
          __assign(__assign({}, currentOrgStatus), status),
          { lastUpdated: new Date() },
        );
        ...
        if (updatedStatus.downloadSpeed !== undefined) {
          ...
          updatedStatus.isSufficientBandwidth =
            updatedStatus.downloadSpeed >= 2;
        }
        ...
        if (
          ...
          var isGoodConnection = ['wifi', '4g'].includes(
        ...
        this.logger.debug(
          'Updated network status for organization '
            .concat(organizationId, ': ')
            .concat(JSON.stringify(updatedStatus)),
        );
     ...
     */
    NetworkStatusService_1.prototype.setGlobalNetworkIssue = function (
      isIssueActive,
    ) {
      this.globalNetworkIssue = isIssueActive;
      this.logger.log(
        'Global network issue flag set to: '.concat(isIssueActive),
      );
    };
    /**
    ...
    NetworkStatusService_1.prototype.updateDefaultStatus = function () {
      ...
      var maxCount = 0;
      for (
        var _b = 0, _c = Object.entries(connectionCounts);
        _b < _c.length;
        _b++
      ) {
        var _d = _c[_b],
      ...
      this.currentStatus = {
        ...
        uploadSpeed:
          uploadSpeedCount > 0
            ? totalUploadSpeed / uploadSpeedCount
            : undefined,
      ...
      this.logger.debug(
        'Updated default network status: '.concat(
