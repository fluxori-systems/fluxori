...
export class NetworkStatusService {
   ...
   */
  updateOrganizationNetworkStatus(organizationId: string, status: Partial<NetworkStatus>): void {
    const currentOrgStatus = this.recentTests.get(organizationId) || { ...this.currentStatus };
    ...
    if (updatedStatus.connectionType !== undefined || updatedStatus.latency !== undefined) {
      const isLowLatency = updatedStatus.latency === undefined || updatedStatus.latency < 200;
      const isGoodConnection = ['wifi', '4g'].includes(updatedStatus.connectionType);
      
      updatedStatus.isStable = isGoodConnection && isLowLatency;
    }
    
    this.recentTests.set(organizationId, updatedStatus);
    this.logger.debug(`Updated network status for organization ${organizationId}: ${JSON.stringify(updatedStatus)}`);
  ...
  private updateDefaultStatus(): void {
    ...
    this.logger.debug(`Updated default network status: ${JSON.stringify(this.currentStatus)}`);
  }
}