 ...
 */
class FirestoreConverter<T extends FirestoreEntity> implements FirestoreDataConverter<T> {
  /**
...
export class FirestoreConfigService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const databaseId = this.configService.get<string>('FIRESTORE_DATABASE_ID', 'fluxori-db');
    this.collectionPrefix = this.configService.get<string>('FIRESTORE_COLLECTION_PREFIX', '');
    ...
    this.logger.log(`Initialized Firestore connection to project ${this.projectId}, database ${databaseId}, region ${this.region}`);
  }
   ...
   */
  getCollection<T extends FirestoreEntity>(collectionName: string): TypedCollectionReference<T> {
    const fullCollectionName = this.collectionPrefix 
      ? `${this.collectionPrefix}_${collectionName}` 
      : collectionName;
    
    // Use the converter for type safety
    return this.firestore.collection(fullCollectionName).withConverter(
   ...
   */
  getDocument<T extends FirestoreEntity>(collectionName: string, documentId: string): DocumentReference<T> {
    return this.getCollection<T>(collectionName).doc(documentId) as DocumentReference<T>;
  }
