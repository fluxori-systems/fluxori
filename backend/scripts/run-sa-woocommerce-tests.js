...
const configDir = path.join(process.env.HOME || process.env.USERPROFILE, '.fluxori');
...
function initConfig() {
  if (fs.existsSync(configFile)) {
    ...
    rl.question('Configuration file already exists. Overwrite? (y/N): ', (answer) => {
  ...
  function writeConfig() {
    ...
    console.log('Please edit this file to add your South African WooCommerce store credentials.');
    process.exit(0);
  }
}
...
function runTests(args = []) {
  ...
  const jsRunnerPath = path.join(__dirname, '../dist/modules/connectors/test/woocommerce-sa-test-runner.js');
  ...
  function runNode() {
    ...
    const runner = path.join(__dirname, '../dist/modules/connectors/test/woocommerce-sa-test-runner.js');
...
function showReport() {
  ...
  if (!fs.existsSync(reportPath)) {
    console.error('No test report found. Run tests first to generate a report.');
    process.exit(1);
  }
