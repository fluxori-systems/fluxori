...
function fixConnectorFile(filePath: string, connectorName: string) {
  ...
  const className = path.basename(filePath).replace('.ts', '').split('-').map(
    part => part.charAt(0).toUpperCase() + part.slice(1)
  ).join('');

  // Check if the file already has the fixed methods
  if (content.includes('getProductByIdInternal') && content.includes('getOrdersInternal')) {
  ...
  protected async getOrdersInternal(options: PaginationOptions): Promise<PaginatedResponse<MarketplaceOrder>> {
    ...
    const productByIdRegex = /^\s*protected\s+async\s+getProductByIdInternal[\s\S]*?\}\s*$/m;
    const ordersRegex = /^\s*protected\s+async\s+getOrdersInternal[\s\S]*?\}\s*$/m;
