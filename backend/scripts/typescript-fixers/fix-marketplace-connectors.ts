...
function fixConnectorFile(filePath: string) {
  ...
  if (hasGetProductByIdInternal && hasGetOrdersInternal) {
    console.log(
      `  - ${path.basename(filePath)} already has the required methods, skipping...`,
    );
    return;
  }
  ...
  if (lastMethodMatch) {
    ...
    insertPosition =
      content.lastIndexOf(lastMethodMatch[0]) + lastMethodMatch[0].length;
  } else if (classEndMatch) {
    // Insert before the class closing bracket
    insertPosition = content.lastIndexOf(classEndMatch[0]);
  } else {
    console.error(
      `  - Could not find insertion point in ${path.basename(filePath)}, skipping...`,
    );
    return;
  }
  ...
  console.log(
    `  - Successfully added missing methods to ${path.basename(filePath)}`,
  );
}
