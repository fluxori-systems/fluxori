...
const BASE_PATH = path.resolve(
  __dirname,
  "../../src/modules/connectors/adapters/xero",
);
...
const BACKUP_DIR = path.resolve(
  __dirname,
  "../../backup/xero-connector-" + Date.now(),
);
...
function fixConnector() {
  ...
  content = content.replace(
    /import { IFinancialConnector } from '\.\.\/\.\.\/interfaces\/connector\.interface';/,
...
import { IFinancialConnector } from '../../interfaces/financial-connector.interface';`,
  ...
  // Fix Type errors
  content = content.replace(
    /Type: ('|")(ACCREC|ACCPAY)('|")/g,
    "type: XeroInvoiceType.$2",
  );
  content = content.replace(
    /type: ('|")(ACCREC|ACCPAY)('|")/g,
    "type: XeroInvoiceType.$2",
  ...
  content = content.replace(
    /Status: ('|")(DRAFT|SUBMITTED|AUTHORISED|DELETED|VOIDED|PAID)('|")/g,
  ...
  content = content.replace(
    /lineAmountTypes: ('|")(Exclusive|Inclusive|NoTax)('|")/g,
  ...
  content = content.replace(
    /this\.credentialManager/g,
    "this.credentialManagerService",
  );

  // Add credential manager service to the constructor
  content = content.replace(
    /constructor\(\s+private readonly apiClient: XeroApiClientService,/g,
    `constructor(
  ...
  content = content.replace(
    /addressType: this\.mapAddressType\(addr\.type\)/g,
  ...
  content = content.replace(
    /contactID: invoice\.contact\.contactID/g,
    'contactID: invoice.contact.contactID,\n          name: invoice.contact.name || ""',
  );

  // Fix account and invoice issues in payment creation
  content = content.replace(
    /account: {\s+accountID: payment\.account\.externalId\s+}/g,
    `account: {
  ...
  content = content.replace(
    /invoice: {\s+invoiceID: payment\.invoice\.externalId\s+}/g,
    `invoice: {
  ...
  content = content.replace(
    /status: ('|")ACTIVE('|")/g,
    "status: XeroTaxRateStatus.ACTIVE",
  );
...
function fixBankService() {
  ...
  if (!content.includes("XeroBankAccountsResponse")) {
    content = content.replace(
      /import {([^}]+)} from '\.\.\/interfaces\/xero-api-responses';/,
...
function fixContactsService() {
  ...
  // Fix imports
  content = content.replace(
    /import { ContactResponse, ContactsResponse } from/g,
...
function fixOAuthService() {
  ...
  content = content.replace(
    /this\.credentialManagerService\.deleteCredential/g,
...
function fixProductService() {
  ...
  // Fix parameter types
  content = content.replace(
    /this\.inventoryService\.getStockOnHand\({[\s\S]*?}\);/g,
    "this.inventoryService.getStockOnHand(itemId);",
  );

  content = content.replace(
    /this\.inventoryService\.getStockMovement\({[\s\S]*?}\);/g,
    "this.inventoryService.getStockMovement(itemId);",
  );

  content = content.replace(
    /this\.logger\.error\(`Error getting stock on hand: \${error\.message}\`, {[\s\S]*?}\);/g,
    "this.logger.error(`Error getting stock on hand: ${error.message}`, error);",
  );

  content = content.replace(
    /this\.logger\.error\(`Error creating product: \${error\.message}\`, {[\s\S]*?}\);/g,
...
function fixOrderIntegration() {
  ...
  content = content.replace(
    /type: address\.type/g,
    "addressType: this.mapAddressType(address.type)",
  );
...
function fixController() {
  const controllerPath = path.join(
    path.dirname(path.dirname(BASE_PATH)),
    "controllers",
    "xero.controller.ts",
  );
  ...
  content = content.replace(
    /constructor\(\s+private readonly xeroConnector: XeroConnector,/g,
    `constructor(
  ...
  if (!content.includes("async getAuthorizationUrl")) {
    const methodInsertPoint =
      content.indexOf("// OAuth and Connection Management") +
  ...
  async revokeToken(
    ...
    return this.oauthService.revokeToken(organizationId, tenantId);
      ...
      content =
        content.slice(0, methodInsertPoint) +
        methods +
  ...
  content = content.replace(
    /getOrganization\(\s+@Param\('organizationId'\) organizationId: string,\s+@Query\('tenantId'\) tenantId: string\s+\) {[\s\S]*?}/g,
    `getOrganization(
    @Param('organizationId') organizationId: string,
    @Query('tenantId') tenantId: string
  ) {
  ...
  content = content.replace(
    /getContacts\(\s+@Param\('organizationId'\) organizationId: string,\s+@Query\('tenantId'\) tenantId: string,\s+@Query\(\) queryParams: Record<string, any>\s+\) {[\s\S]*?}/g,
    `getContacts(
    @Param('organizationId') organizationId: string,
    @Query('tenantId') tenantId: string,
...
function fixIndex() {
  ...
  content = content.replace(
    /export \* from '\.\/interfaces\/xero-types';/g,
...
function addFinancialConnectorInterface() {
  const interfacePath = path.join(
    ...
    "../../interfaces/financial-connector.interface.ts",
  );
