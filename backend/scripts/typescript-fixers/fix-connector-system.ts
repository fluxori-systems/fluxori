...
function fixConnectorInterfaces(): void {
  ...
  const moduleInterfacePath = path.join(
    INTERFACE_PATH,
  ...
  const connectorInterfacePath = path.join(
    CONNECTORS_PATH,
    "interfaces",
    "connector.interface.ts",
  );
...
function fixBaseMarketplaceConnector(): void {
  ...
  const baseMarketplacePath = path.join(
    ADAPTERS_PATH,
    ...
    if (!content.includes("getProductByIdInternal")) {
      ...
      if (classEndMatch) {
        const insertPosition =
          content.indexOf(classEndMatch[0]) + classEndMatch[0].length;
        content =
...
function fixMarketplaceConnectors(): void {
  ...
  for (const filePath of connectorFiles) {
    if (fs.existsSync(filePath)) {
      ...
      if (!content.includes("getProductByIdInternal(")) {
        console.log(
          `Adding getProductByIdInternal to ${path.basename(filePath)}`,
        );
...
function fixConnectorFactory(): void {
  ...
  if (fs.existsSync(factoryPath)) {
    ...
    content = content.replace(
      /await connector.disconnect\(\);/g,
      "await connector.close();",
    );

    // Fix method name from getHealth to getHealthStatus
    content = content.replace(
      /await connector.getHealth\(\);/g,
      "await connector.getHealthStatus();",
    );
