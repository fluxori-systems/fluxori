...
> = {
  ...
  at: { standard: 0.2, reduced: 0.1 }, // Austria
  be: { standard: 0.21, reduced: 0.06, superReduced: 0.0 }, // Belgium
  bg: { standard: 0.2, reduced: 0.09 }, // Bulgaria
  cy: { standard: 0.19, reduced: 0.09, superReduced: 0.05 }, // Cyprus
  cz: { standard: 0.21, reduced: 0.15, superReduced: 0.1 }, // Czech Republic
  de: { standard: 0.19, reduced: 0.07 }, // Germany
  dk: { standard: 0.25 }, // Denmark
  ee: { standard: 0.22, reduced: 0.09 }, // Estonia
  es: { standard: 0.21, reduced: 0.1, superReduced: 0.04 }, // Spain
  fi: { standard: 0.24, reduced: 0.14, superReduced: 0.1 }, // Finland
  fr: { standard: 0.2, reduced: 0.1, superReduced: 0.055 }, // France
  ...
  it: { standard: 0.22, reduced: 0.1, superReduced: 0.04 }, // Italy
  ...
  sk: { standard: 0.2, reduced: 0.1 }, // Slovakia

  // Non-EU but following similar VAT rules
  gb: { standard: 0.2, reduced: 0.05, exempt: ["books", "children-clothing"] }, // UK
...
export class EuropeanVatService implements VatService {
  ...
  determineVatRate(country: string = "gb", productType?: string): number {
    ...
    const countryRates =
      EU_VAT_RATES[country.toLowerCase()] || EU_VAT_RATES["gb"];

    // Check if product is exempt
    if (
      productType &&
      countryRates.exempt &&
      countryRates.exempt.some((type) =>
    ...
    if (
      ...
      SUPER_REDUCED_RATE_PRODUCTS.some((type) =>
        productType.toLowerCase().includes(type),
      )
    ) {
    ...
    if (
      ...
      REDUCED_RATE_PRODUCTS.some((type) =>
        productType.toLowerCase().includes(type),
      )
    ) {
  ...
  getVatRateChangesBreakdown(
    ...
    if (includesVat) {
      ...
      result.push(
        this.calculateVat(price, "standard-goods", new Date(), country),
      );
    }
    ...
    if (countryRates.reduced) {
      if (includesVat) {
        ...
        result.push(
          this.calculateVat(price, "food-basic", new Date(), country),
        );
      }
    }
    ...
    if (countryRates.superReduced) {
      if (includesVat) {
        result.push(
          this.removeVat(price, "books-educational", new Date(), country),
        );
      } else {
        result.push(
          this.calculateVat(price, "books-educational", new Date(), country),
        );
      }
    }
    ...
    if (countryRates.exempt && countryRates.exempt.length > 0) {
      if (includesVat) {
        result.push(
          this.removeVat(price, countryRates.exempt[0], new Date(), country),
        );
      } else {
        result.push(
          this.calculateVat(price, countryRates.exempt[0], new Date(), country),
        );
      }
    }
