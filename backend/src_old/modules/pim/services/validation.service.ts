...
export class ValidationService {
  ...
  private async performValidation(
    ...
    if (
      validationOptions.includeCategoryRules &&
    ...
    if (
      validationOptions.includeVariantValidation &&
    ...
    if (validationOptions.includeCategoryRules && context.category) {
      ...
      rulesToRun = rulesToRun.filter(
        (rule) => !rule.categoryIds || rule.categoryIds.length === 0,
      );
    }
  ...
  private initializeValidationRules(): void {
      ...
      validator: (product: Product) => {
        ...
        if (product.sku && !/^[a-zA-Z0-9_-]+$/.test(product.sku)) {
          issues.push({
            ...
            field: "sku",
            message:
              "SKU should only contain letters, numbers, underscores, and hyphens",
            severity: ValidationSeverity.ERROR,
          });
        }
        ...
        if (
          ...
          (isNaN(product.pricing.basePrice) || product.pricing.basePrice <= 0)
        ) {
          issues.push({
            ruleId: "field-formats",
        ...
        if (
          ...
          (isNaN(product.weight) || product.weight <= 0)
        ) {
          issues.push({
            ruleId: "field-formats",
            field: "weight",
        ...
        if (product.dimensions) {
          if (
            ...
            issues.push({
              ...
              field: "dimensions",
              message:
                "Dimensions must be an array with exactly 3 values [length, width, height]",
              severity: ValidationSeverity.ERROR,
            });
    ...
    this.rules.push({
      ...
      validator: (product: Product) => {
        ...
        if (!product.categories || product.categories.length === 0) {
          ...
          const primaryCategories = product.categories.filter(
            (c) => c.isPrimary,
          );
    ...
    this.rules.push({
      ...
      validator: (product: Product) => {
        ...
        if (
          product.images?.main &&
          !this.isValidImageUrl(product.images.main)
        ) {
          issues.push({
            ruleId: "image-validation",
            field: "images.main",
    ...
    this.rules.push({
      ...
      validator: (product: Product) => {
        ...
        if (
          product.name &&
          (product.name.length < 5 || product.name.length > 70)
        ) {
          issues.push({
        ...
        if (product.description && product.description.length < 100) {
          issues.push({
            ...
            field: "description",
            message:
              "Description is too short for good SEO. Aim for at least 100 characters.",
            severity: ValidationSeverity.INFO,
          });
        }
        ...
        if (!product.seo) {
          issues.push({
            ...
            field: "seo",
            message:
              "SEO information is recommended for better search visibility",
            severity: ValidationSeverity.INFO,
          });
        } else {
          // Check meta title
      ...
      validator: (product: Product) => {
        ...
        if (!product.attributes || product.attributes.length === 0) {
          issues.push({
            ...
            field: "attributes",
            message:
              "Product should have attributes for better filtering and search",
            severity: ValidationSeverity.WARNING,
          });
      ...
      validator: (product: Product, context: ValidationContext) => {
        ...
        if (this.isElectronicsProduct(product, context)) {
          ...
          if (!hasIcasa) {
            issues.push({
              ...
              message:
                "ICASA approval is required for electronic products in South Africa",
              severity: ValidationSeverity.WARNING,
            });
          }
        }
        ...
        if (!hasBarcode) {
          issues.push({
            ...
            message:
              "South African barcode is recommended for South African market",
            severity: ValidationSeverity.INFO,
          });
        }
        ...
        if (!product.dimensions || !product.weight) {
          issues.push({
            ...
            field: "dimensions",
            message:
              "Dimensions and weight are important for shipping in South Africa",
            severity: ValidationSeverity.WARNING,
          });
        }
