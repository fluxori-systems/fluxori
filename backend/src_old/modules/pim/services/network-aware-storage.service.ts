...
export class NetworkAwareStorageService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const isLoadSheddingResilience = await this.featureFlagService.isEnabled(
    ...
    if (isLoadSheddingResilience) {
      ...
      const processingIntervalMs =
        this.configService.get<number>("QUEUE_PROCESSING_INTERVAL_MS") || 30000; // 30 seconds default
      ...
      const networkCheckIntervalMs =
        this.configService.get<number>("NETWORK_CHECK_INTERVAL_MS") || 60000; // 1 minute default
  ...
  private async updateNetworkStatus(): Promise<void> {
    try {
      ...
      this.logger.debug(
        `Network status: ${JSON.stringify(this.networkStatus)}`,
      );
      ...
      this.logger.error(
        `Error updating network status: ${errorMessage}`,
        errorStack,
      );
    }
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        if (result.status === "fulfilled" && result.value) {
          this.operationQueue = this.operationQueue.filter(
            (entry) => entry.id !== batch[i].id,
          );
        }
      }

      this.logger.log(
        `Processed ${batch.length} queued operations, ${this.operationQueue.length} remaining`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error processing operation queue: ${errorMessage}`,
        errorStack,
      );
    } finally {
  ...
  private async processQueueEntry(entry: QueuedOperation): Promise<boolean> {
    try {
      ...
      this.logger.log(
        `Successfully processed queued operation ${entry.id} (${entry.operation})`,
      );
      return true;
    } catch (error) {
      ...
      if (entry.retries < maxRetries) {
        this.logger.warn(
          `Failed to process operation ${entry.id}, will retry (${entry.retries}/${maxRetries}): ${errorMessage}`,
        );
        return false;
      } else {
        this.logger.error(
          `Failed to process operation ${entry.id} after ${entry.retries} retries: ${errorMessage}`,
        );
  ...
  private queueOperation(
    ...
    this.logger.log(
      `Queued ${operation} operation ${id} with ${priority} priority`,
    );
   ...
   */
  async getNetworkOptimizedImageOptions(
    originalOptions: ImageUploadOptions,
  ): Promise<ImageUploadOptions> {
    ...
    if (connectionQuality === "low" || this.loadSheddingActive) {
      ...
      optimizedOptions.compressionQuality =
        optimizedOptions.compressionQuality || CompressionQuality.ADAPTIVE;
