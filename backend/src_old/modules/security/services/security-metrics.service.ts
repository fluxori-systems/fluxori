...
export class SecurityMetricsService {
  ...
  async collectSecurityMetrics(): Promise<Record<string, number>> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to collect security metrics: ${error.message}`,
        error.stack,
      );
      this.observability.error(
        "Security metrics collection failed",
        error,
        SecurityMetricsService.name,
      );
  ...
  private async collectMonitoringMetrics(): Promise<Record<string, number>> {
    try {
      ...
      this.logger.error(
        `Failed to collect monitoring metrics: ${error.message}`,
        error.stack,
      );
      return {};
    }
  ...
  private async collectIncidentMetrics(): Promise<Record<string, number>> {
    try {
      ...
      this.logger.error(
        `Failed to collect incident metrics: ${error.message}`,
        error.stack,
      );
      return {};
    }
  ...
  private async collectAuthMetrics(): Promise<Record<string, number>> {
    try {
      ...
      this.logger.error(
        `Failed to collect auth metrics: ${error.message}`,
        error.stack,
      );
      return {};
    }
  ...
  async recordMetric(
    ...
    try {
      ...
      this.logger.error(
        `Failed to record security metric: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async incrementMetric(
    ...
    try {
      ...
      this.logger.debug(
        `Incremented security metric: ${metricName} by ${incrementBy}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to increment security metric: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async createAlertPolicy(
    ...
    try {
      ...
      this.logger.log(
        `Would create alert policy for metric: ${metricName}, threshold: ${threshold}, duration: ${duration.seconds}s`,
      );

      return `projects/${this.projectId}/alertPolicies/security-${metricName}-${Date.now()}`;
    } catch (error) {
      this.logger.error(
        `Failed to create alert policy: ${error.message}`,
        error.stack,
      );
      throw error;
    }
