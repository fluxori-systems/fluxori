  ...
  if (errorCount > 0) {
    ...
    topFiles.forEach(([file, fileErrors]) => {
      ...
      Object.entries(byCode).forEach(([code, codeErrors]) => {
        ...
        console.log(
          `  ${colors.yellow}${code}${colors.reset} (${codeErrors.length}): ${example.message}`,
        );
      });
    });
  }
...
function checkESLint() {
  ...
  const output = runCommand(
    `npx eslint --config ${eslintConfig} "src/**/*.{ts,tsx}" --format stylish`,
    FRONTEND_DIR,
  );
...
function checkMantineProps() {
    ...
    Object.entries({
      ...
      try {
        const grepOutput = runCommand(
          `grep -r "${pattern}" --include="*.tsx" src/`,
          FRONTEND_DIR,
        );
  ...
  if (fileCount > 0) {
    console.log(
      `Found ${colors.yellow}${fileCount} files${colors.reset} with deprecated Mantine props:\n`,
    );
    ...
    Object.entries(results).forEach(([file, props]) => {
      ...
      Object.entries(props).forEach(([prop, components]) => {
        ...
        console.log(
          `  ${colors.red}${prop}${colors.reset} → ${colors.green}${PROP_MAPPING[prop]}${colors.reset} in ${uniqueComponents.join(", ")}`,
        );
      });
    ...
    console.log(
      `${colors.yellow}Note:${colors.reset} Use modern Mantine props instead. See TYPESCRIPT_GUIDE.md for details.`,
    );
  } else {
    console.log(
      `${colors.green}No deprecated Mantine props found.${colors.reset}`,
    );
  }
...
function checkUseClientDirectives() {
  ...
  try {
    const hookMatches = runCommand(
      `grep -l -E "(${clientHookPatterns})" --include="*.tsx" src/`,
      FRONTEND_DIR,
    )
      .split("\n")
      .filter(Boolean);
    const eventMatches = runCommand(
      `grep -l -E "(${eventHandlerPatterns})" --include="*.tsx" src/`,
      FRONTEND_DIR,
    )
      .split("\n")
      .filter(Boolean);
    ...
    const clientComponentFiles = [
      ...new Set([...hookMatches, ...eventMatches]),
    ];
  ...
  if (results.length > 0) {
    console.log(
      `Found ${colors.yellow}${results.length} files${colors.reset} missing 'use client' directive:\n`,
    );
    ...
    console.log(
      `${colors.green}No files missing 'use client' directive.${colors.reset}`,
    );
  }
...
function main() {
  console.log(
    `${colors.bold}${colors.magenta}TypeScript Linting Report${colors.reset}\n`,
  );
  ...
  const totalIssues =
    tsErrorCount +
    eslintErrorCount +
    deprecatedPropsCount +
    missingClientDirectivesCount;
  ...
  console.log(
    `Files with deprecated props: ${formatErrorCount(deprecatedPropsCount)}`,
  );
  console.log(
    `Files missing 'use client': ${formatErrorCount(missingClientDirectivesCount)}`,
  );
