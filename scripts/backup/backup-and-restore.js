...
program
  .version('1.0.0')
  .requiredOption('--action <action>', 'Action to perform: backup, restore, list-backups, cleanup')
  .requiredOption('--target <target>', 'Resource to backup/restore: firestore, storage, all')
  .requiredOption('--project <project>', 'GCP project ID')
  .option('--backupId <backupId>', 'Backup ID for restore (required for restore)')
  .option('--dryRun', 'Show what would be done without making changes')
  .option('--label <label>', 'Additional label for the backup (e.g. "pre-deployment")')
  .option('--destination <destination>', 'Destination for restore (different project or location)')
  .option('--collections <collections>', 'Comma-separated list of Firestore collections to backup')
  .option('--buckets <buckets>', 'Comma-separated list of storage buckets to backup')
  .parse(process.argv);
...
const backupId = options.backupId || `${timestamp}${options.label ? `-${options.label}` : ''}`;
...
async function backupFirestore() {
  ...
  for (const collection of collections) {
    ...
    logger.info(`Backing up Firestore collection '${collection}' to ${collectionPath}`);
...
async function restoreFirestore() {
  ...
  try {
    ...
    logger.warn('Could not load backup metadata, using default collection list');
  }
  ...
  for (const collection of collections) {
    ...
    logger.info(`Restoring Firestore collection '${collection}' from ${collectionPath} to project ${destProject}`);
    executeCommand(command, options.dryRun);
  }
...
async function restoreStorage() {
  ...
  try {
    if (!options.dryRun) {
      const tempFile = path.join('/tmp', `metadata-storage-${options.backupId}.json`);
      executeCommand(`gsutil cp ${metadataPath} ${tempFile}`, options.dryRun);
      const metadata = JSON.parse(fs.readFileSync(tempFile, 'utf8'));
  ...
  for (const bucket of buckets) {
    ...
    logger.info(`Restoring Cloud Storage bucket from '${bucketBackup}' to ${destBucket}`);
    executeCommand(command, options.dryRun);
  }
...
async function cleanupBackups() {
  ...
  logger.info(`Deleting backups older than ${CONFIG.backupRetentionDays} days (before ${cutoffDateString})`);
    ...
    try {
      ...
      if (!options.dryRun) {
        ...
        for (const backup of backups) {
          const metadataPath = `${backupPath}${backup}/metadata.json`;
          try {
            const tempFile = path.join('/tmp', `metadata-cleanup-${backup}.json`);
            executeCommand(`gsutil cp ${metadataPath} ${tempFile}`, options.dryRun);
            const metadata = JSON.parse(fs.readFileSync(tempFile, 'utf8'));
            
            const backupDate = new Date(metadata.timestamp);
            if (backupDate < cutoffDate) {
              logger.info(`  Deleting backup ${backup} from ${metadata.timestamp} (older than retention period)`);
              
              // Delete the backup
              executeCommand(`gsutil -m rm -r ${backupPath}${backup}`, options.dryRun);
            } else {
              logger.debug(`  Keeping backup ${backup} from ${metadata.timestamp} (within retention period)`);
            }
