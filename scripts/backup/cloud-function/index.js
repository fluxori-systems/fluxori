...
exports.backupHandler = async (req, res) => {
  ...
  logMessage(
    logStream,
    "INFO",
    `Starting backup operation: action=${action}, target=${target}, project=${project}, backupId=${backupId}`,
  );
...
async function uploadLogFile(logFile, backupId, action, target) {
  ...
  try {
    ...
    console.log(
      `Log file uploaded to gs://${CONFIG.logsBucket}/${destination}`,
    );
  } catch (error) {
  ...
  for (const collection of collections) {
    ...
    try {
      ...
      logMessage(
        logStream,
        "INFO",
        `Collection ${collection} exported successfully`,
      );
    } catch (error) {
      logMessage(
        logStream,
        "ERROR",
        `Failed to export collection ${collection}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  for (const bucketName of buckets) {
    ...
    try {
      ...
      logMessage(
        logStream,
        "INFO",
        `Bucket ${bucketName} backed up successfully`,
      );
    } catch (error) {
      logMessage(
        logStream,
        "ERROR",
        `Failed to backup bucket ${bucketName}: ${error.message}`,
      );
      throw error;
    }
  }
...
async function cleanupOldBackups(target, project, logStream) {
  ...
  const services = target === "all" ? Object.keys(CONFIG.services) : [target];
  ...
  for (const service of services) {
    ...
    try {
      ...
      for (const [backupId, backupFiles] of Object.entries(backupGroups)) {
        ...
        const metadataFile = backupFiles.find((file) =>
          ...
          try {
            ...
            if (backupDate < cutoffDate) {
              logMessage(
                logStream,
                "INFO",
                `Deleting old backup: ${backupId} (created ${backupDate.toISOString()})`,
              );
              ...
              logMessage(
                logStream,
                "DEBUG",
                `Keeping backup: ${backupId} (within retention period)`,
              );
            }
          } catch (error) {
            logMessage(
              logStream,
              "WARN",
              `Failed to process metadata for backup ${backupId}: ${error.message}`,
            );
          }
        } else {
          logMessage(
            logStream,
            "WARN",
            `No metadata found for backup ${backupId}, skipping`,
          );
        }
      }
    } catch (error) {
      logMessage(
        logStream,
        "ERROR",
        `Failed to list backups for ${service}: ${error.message}`,
      );
      throw error;
    }
  }
