...
async function validateCloudRun() {
  ...
  try {
    ...
    const { stdout } = await execAsync(`gcloud run services list --region=${region} --format=json`);
    ...
    if (missingServices.length > 0) {
      ...
      results.services.cloudRun.details.push(`Missing expected services: ${missingServices.join(', ')}`);
      results.overall.issues.push(`Missing Cloud Run services: ${missingServices.join(', ')}`);
    }
    ...
    for (const service of services) {
      ...
      if (service.metadata.labels.cloud_run_location !== region) {
        ...
        serviceResults.issues.push(`Service is in ${service.metadata.labels.cloud_run_location} instead of ${region}`);
        results.overall.issues.push(`Service ${serviceName} is in ${service.metadata.labels.cloud_run_location} instead of ${region}`);
      }
      ...
      if (!service.status.url.startsWith('https://')) {
        ...
        results.overall.pass = false;
        results.overall.issues.push(`Service ${serviceName} is not using HTTPS`);
      }
      ...
      try {
        ...
        const response = await axios.get(`${service.status.url}${healthEndpoint}`, {
          timeout: 5000,
          validateStatus: null,
        });
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          serviceResults.issues.push(`Health check failed with status ${response.status}`);
          results.overall.issues.push(`Service ${serviceName} health check failed with status ${response.status}`);
        }
      } catch (error) {
        serviceResults.health = 'fail';
        serviceResults.issues.push(`Health check failed: ${error.message}`);
        results.overall.issues.push(`Service ${serviceName} health check failed: ${error.message}`);
      }
    ...
    results.services.cloudRun.status = missingServices.length > 0 || 
      Object.values(results.services.cloudRun.services).some(service => service.status === 'fail') 
      ? 'fail' : 'pass';
    ...
    console.error(chalk.red(`Error validating Cloud Run services: ${error.message}`));
    results.services.cloudRun.status = 'fail';
...
async function validateFirestore() {
  ...
  try {
    ...
    const { stdout } = await execAsync('gcloud firestore databases list --format=json');
    ...
    if (database.type !== 'FIRESTORE_NATIVE') {
      results.services.firestore.status = 'fail';
      results.services.firestore.details.push('Firestore database is not in Native mode');
      results.overall.pass = false;
    ...
    if (!database.locationId.includes('nam5') && !database.locationId.includes('africa-south1')) {
      results.services.firestore.status = 'warn';
      results.services.firestore.details.push(`Firestore database is in ${database.locationId}, which may not be optimal for South Africa`);
      results.overall.issues.push(`Firestore database is in ${database.locationId}, which may not be optimal for South Africa`);
    }
    
    // Overall status for Firestore
    results.services.firestore.status = results.services.firestore.details.length > 0 ? 'fail' : 'pass';
...
async function validateCloudStorage() {
  ...
  try {
    ...
    const missingBuckets = expectedBuckets.filter(bucket => !bucketNames.some(name => name.includes(bucket)));
    
    if (missingBuckets.length > 0) {
      results.services.cloudStorage.status = 'warn';
      results.services.cloudStorage.details.push(`Missing expected buckets: ${missingBuckets.join(', ')}`);
      results.overall.issues.push(`Missing expected storage buckets: ${missingBuckets.join(', ')}`);
    }
    ...
    for (const bucketUrl of buckets) {
      ...
      // Get bucket details
      const { stdout: detailsJson } = await execAsync(`gcloud storage buckets describe ${bucketUrl} --format=json`);
      ...
      if (bucketDetails.location !== 'AFRICA-SOUTH1') {
        ...
        bucketResults.issues.push(`Bucket is in ${bucketDetails.location} instead of AFRICA-SOUTH1`);
        results.overall.issues.push(`Bucket ${bucketName} is in ${bucketDetails.location} instead of AFRICA-SOUTH1`);
      }
    ...
    results.services.cloudStorage.status = missingBuckets.length > 0 ? 'warn' : 'pass';
    
    console.log(chalk.green('Cloud Storage validation completed'));
  } catch (error) {
    console.error(chalk.red(`Error validating Cloud Storage: ${error.message}`));
    results.services.cloudStorage.status = 'fail';
    results.services.cloudStorage.details.push(`Error: ${error.message}`);
    results.overall.pass = false;
    results.overall.issues.push(`Cloud Storage validation error: ${error.message}`);
  }
}
...
async function validateMonitoring() {
  ...
  try {
    ...
    const { stdout: policiesJson } = await execAsync('gcloud alpha monitoring policies list --format=json');
    ...
    // Check dashboards
    const { stdout: dashboardsJson } = await execAsync('gcloud monitoring dashboards list --format=json');
    ...
    const { stdout: channelsJson } = await execAsync('gcloud alpha monitoring channels list --format=json');
    const channels = JSON.parse(channelsJson);
    
    if (channels.length === 0) {
      results.services.monitoring.status = 'warn';
      results.services.monitoring.details.push('No notification channels found');
    ...
    results.services.monitoring.status = results.services.monitoring.details.length > 0 ? 'warn' : 'pass';
    ...
    results.overall.pass = false;
    results.overall.issues.push(`Monitoring validation error: ${error.message}`);
  }
}
...
async function validateNetworking() {
  ...
  try {
    ...
    const { stdout: lbJson } = await execAsync('gcloud compute forwarding-rules list --format=json');
    ...
    if (loadBalancers.length === 0) {
      ...
      const premiumTier = loadBalancers.some(lb => lb.networkTier === 'PREMIUM');
      if (!premiumTier) {
        results.services.networking.status = 'warn';
        results.services.networking.details.push('Not using Premium Network Tier, which may impact performance in South Africa');
        results.overall.issues.push('Not using Premium Network Tier, which may impact performance in South Africa');
      }
    }
    
    // Check if security policies are set up
    const { stdout: securityJson } = await execAsync('gcloud compute security-policies list --format=json');
    ...
    results.services.networking.status = results.services.networking.details.length > 0 ? 'warn' : 'pass';
    ...
    results.overall.pass = false;
    results.overall.issues.push(`Networking validation error: ${error.message}`);
  }
}
...
function printSummary() {
  ...
  console.log(overallColor(`Overall: ${results.overall.pass ? 'PASS' : 'FAIL'}`));
