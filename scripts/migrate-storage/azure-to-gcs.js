...
async function updateFirestoreReferences(migrationResults, firestore) {
  console.log(
    `Updating ${migrationResults.length} file references in Firestore...`,
  );
...
async function migrateContainer(containerName, azureClient, gcsClient) {
  ...
  try {
    ...
    for await (const blobItem of blobsIterator) {
      count++;
      const result = await migrateBlob(
        blobItem,
        containerClient,
        bucket,
        containerName,
    ...
    console.log(
      `=== Completed migration of ${count} files from ${containerName} to ${bucketName} ===`,
    );
