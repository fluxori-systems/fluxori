...
async function validateBlob(blobItem, containerClient, bucket, containerName) {
  ...
  try {
    ...
    const azureTempPath = path.join(TEMP_DIR, `azure-${path.basename(blobName)}`);
    ...
    const azureTempPath = path.join(TEMP_DIR, `azure-${path.basename(blobItem.name)}`);
    const gcsTempPath = path.join(TEMP_DIR, `gcs-${path.basename(blobItem.name)}`);
...
async function checkDatabaseReferences(validationResults, firestore) {
  console.log(`Checking ${validationResults.length} file references in Firestore...`);
...
async function validateContainer(containerName, azureClient, gcsClient) {
  ...
  try {
    ...
    for await (const blobItem of blobsIterator) {
      count++;
      const result = await validateBlob(blobItem, containerClient, bucket, containerName);
    ...
    console.log(`=== Completed validation of ${count} files from ${containerName} ===`);
...
async function validateMigration() {
  ...
  try {
    ...
    for (const container of AZURE_CONTAINERS) {
      const results = await validateContainer(container, azureClient, gcsClient);
    ...
    if (refCheckResults) {
      ...
      console.log(`Total references checked: ${refCheckResults.totalReferences}`);
      ...
      Object.keys(refCheckResults.documentsByCollection).forEach(collection => {
        const collStats = refCheckResults.documentsByCollection[collection];
        console.log(`- ${collection}: ${collStats.withOldReferences}/${collStats.checked}`);
        ...
        if (collStats.withOldReferences > 0) {
          ...
          if (collStats.oldReferencesList.length > 10) {
            console.log(`  - ...and ${collStats.oldReferencesList.length - 10} more`);
          }
        }
      });
    }
    ...
    const resultsFile = path.join(process.cwd(), 'migration-validation-results.json');
    const resultsData = {
      stats,
      refCheckResults,
