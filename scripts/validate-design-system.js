...
// Define paths
const designSystemPath = path.join(__dirname, '../frontend/src/lib/design-system');
...
try {
  ...
  const contrastIssues = colorsContent.match(/\/\/ \[CONTRAST ISSUE\].*$/mg);
...
try {
  const globalCssPath = path.join(__dirname, '../frontend/src/styles/globals.css');
  ...
  if (!rootVars) {
    ...
    if (!darkVars) {
      ...
      console.log(`✅ Found ${darkVarCount} CSS variables in dark mode selector`);
    }
  }
} catch (error) {
...
try {
  ...
  if (!fontFace) {
    ...
    if (!cssContent.includes('font-family: \'Space Grotesk\'')) {
      console.warn('⚠️ Space Grotesk font not found in @font-face declarations');
    } else {
...
try {
  ...
  if (missingFonts.length > 0) {
    ...
    console.warn('These fonts need to be downloaded and placed in the /frontend/public/fonts directory');
  } else {
...
try {
  ...
  const showcasePath = path.join(designSystemPath, 'components/ThemeShowcase.tsx');
  const docsPath = path.join(designSystemPath, 'components/DesignSystemDocs.tsx');
  
  if (fs.existsSync(showcasePath) && fs.existsSync(docsPath)) {
    console.log('✅ Documentation components: Present');
  } else {
    console.warn('⚠️ Documentation components: Missing showcase or docs components');
  }
} catch (error) {
...
console.log('Note: For a complete validation, download required fonts and run the TypeScript compiler on the entire project.');
