...
function testAdrCreation() {
  ...
  const sanitizedTitle = TEST_ADR_TITLE.toLowerCase().replace(
    /[^a-z0-9]+/g,
    "-",
  );
...
function testVisualizationGeneration(adrNumber) {
  ...
  const visualizationPath = path.join(
    ADR_DIR,
    "visualizations",
    `adr-${adrNumber.toString().padStart(3, "0")}-test-visualization.svg`,
  );
...
function testMermaidGeneration(adrNumber) {
  ...
  const mermaidPath = path.join(
    ADR_DIR,
    "visualizations",
    `adr-${adrNumber.toString().padStart(3, "0")}-test-flowchart-mermaid.md`,
  );
...
function cleanUp(adrPath) {
  ...
  const visualizationPath = path.join(
    ADR_DIR,
    "visualizations",
    `adr-${adrNumber}-test-visualization.svg`,
  );
  if (fileExists(visualizationPath)) {
  ...
  const mermaidPath = path.join(
    ADR_DIR,
    "visualizations",
    `adr-${adrNumber}-test-flowchart-mermaid.md`,
  );
  if (fileExists(mermaidPath)) {
...
function runTests() {
  ...
  try {
    ...
    runCommand(
      "chmod +x adr.sh extract-subgraph.sh update-adr-visualizations.sh",
    );
