...
function renderADR(options: RenderOptions): string {
  ...
  if (interactive) {
    ...
    html +=
      '  const vizElements = document.querySelectorAll(".dependency-visualization.interactive svg");\n';
...
function renderADRDirectory(inputDir: string, outputDir: string): void {
  ...
  if (fs.existsSync(vizDir)) {
    ...
    console.log(
      `Copied ${vizFiles.length} visualization files to ${outputVizDir}`,
    );
  }
    ...
    switch (command) {
      case "render-file":
        if (!options.input || !options.output) {
          console.error(
            "Error: --input and --output are required for render-file command",
          );
          printUsage();
          break;
        }
      ...
      case "render-directory":
        if (!options.input || !options.output) {
          console.error(
            "Error: --input and --output are required for render-directory command",
          );
          printUsage();
          break;
        }
...
export { renderADR, renderADRDirectory, generateStyles };
