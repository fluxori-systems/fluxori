...
function createAdr(options: ADRCreateOptions): string {
  const { title, modules = [], includeCommon = true, status = 'Proposed' } = options;
  ...
  templateContent = templateContent.replace(/# ADR-000: Title of Architecture Decision/, `# ADR-${adrNumber.toString().padStart(3, '0')}: ${title}`);
  ...
  if (modules.length > 0) {
    ...
    try {
      ...
      const placeholder = '<!-- Include the current dependency visualization, focused on the modules relevant to this decision. -->';
      ...
      console.warn('Warning: Failed to generate dependency visualization.', error);
    }
  }
...
function updateAdrIndex(): void {
  ...
  for (const adr of adrs) {
    ...
    const vizLink = hasViz 
      ? `[View](${adr.visualizations[0]})` 
      : 'N/A';
...
function updateAdrStatus(adrNumber: number, newStatus: string): boolean {
  // Find the ADR file
  const files = fs.readdirSync(ADR_DIR);
  const adrFile = files.find(file => file.startsWith(`ADR-${adrNumber.toString().padStart(3, '0')}`));
...
function regenerateVisualizations(): void {
  ...
  for (const adr of adrs) {
    ...
    if (!moduleMatches) {
      console.warn(`Warning: Could not extract module information from ADR-${adr.number}`);
      continue;
    }
    ...
    try {
      ...
      console.error(`Error regenerating visualization for ADR-${adr.number}:`, error);
    }
  }
    ...
    switch (command) {
      ...
      case 'update-status':
        if (!options.adr || !options.status) {
          console.error('Error: Missing required options --adr and/or --status');
          printUsage();
          break;
        }
