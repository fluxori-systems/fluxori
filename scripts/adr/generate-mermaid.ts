 ...
 */
function generateModuleDependencyFlowchart(modules: string[], adrNumber: number, title: string): string {
 ...
 */
function generateSequenceDiagram(adrNumber: number, title: string, actors: string[]): string {
    ...
    switch (command) {
      case 'flowchart':
        if (!options.adr || !options.modules) {
          console.error('Error: Missing required options --adr and/or --modules');
          printUsage();
          break;
        }
        const title = options.title || 'Module Dependencies';
        const outputPath = generateModuleDependencyFlowchart(options.modules, options.adr, title);
        console.log(`Generated flowchart: ${outputPath}`);
        break;
        
      case 'class-diagram':
        if (!options.adr || !options.module) {
          console.error('Error: Missing required options --adr and/or --module');
          printUsage();
          break;
        }
      ...
      case 'sequence-diagram':
        if (!options.adr || !options.actors) {
          console.error('Error: Missing required options --adr and/or --actors');
          printUsage();
          break;
        }
        const seqTitle = options.title || 'Sequence Flow';
        const seqOutput = generateSequenceDiagram(options.adr, seqTitle, options.actors);
