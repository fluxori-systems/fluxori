 ...
 */
function generateAdrDependencyVisualization(
  ...
  const depCruiserConfigPath = path.join(
    __dirname,
    "..",
    "..",
    "backend",
  ...
  try {
    console.log(
      `Generating dependency visualization for ADR-${adrNumber.toString().padStart(3, "0")}: ${title}`,
    );
...
function buildDepCruiseOptions(
  ...
  if (includeCommonUtilities) {
    options +=
      ' --include-only "^src/common/|^src/modules/(' +
      focusModules.join("|") +
      ')"';
  }
...
function parseArgs(): VisualizationOptions | null {
  ...
  if (!options.adrNumber || !options.title || !options.focusModules) {
    console.error(
      "Error: Missing required options. Use --help for usage information.",
    );
    return null;
  }
