...
function updateRepositoryFile(filePath: string) {
  ...
  let updatedContent = content.replace(
    new RegExp(`export class (${entityName}Repository|.*Repository)\\s+extends\\s+FirestoreBaseRepository<([^>]*)>`, 'g'),
...
function updateServiceFile(filePath: string) {
  ...
  if (content.includes('productService') && !content.includes('findAll') && content.includes('find(')) {
    updatedContent = updatedContent.replace(
  ...
  if (fileName === 'product-ai.service' || fileName === 'image-analysis.service' || fileName === 'product-review.service') {
...
function updateAllRepositories() {
  ...
  const repositoryFiles = glob.sync(path.join(BASE_PATH, 'repositories', '**', '*.repository.ts'));
...
function updateAllServices() {
  ...
  const serviceFiles = glob.sync(path.join(BASE_PATH, 'services', '**', '*.service.ts'));
...
function updateAllControllers() {
  ...
  const controllerFiles = glob.sync(path.join(BASE_PATH, 'controllers', '**', '*.controller.ts'));
