...
describe("AI Insights Module", () => {
  ...
  describe("AI Model Configurations", () => {
    ...
    it("should retrieve a specific model by ID", async () => {
      ...
      const response = await testUtils.api.get(
        `/ai-model-configs/${firstModel.id}`,
      );
  ...
  describe("Insight Generation", () => {
    ...
    const testOrSkip = config.environment === "production" ? it.skip : it;
    ...
    testOrSkip(
      ...
      async () => {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          const checkResponse = await testUtils.api.get(
            `/insights/${insightId}`,
          );
          ...
          if (checkResponse.data.status === "FAILED") {
            throw new Error(
              `Insight generation failed: ${checkResponse.data.error}`,
            );
          }
        }
        ...
        const creditResponse = await testUtils.api.get(
