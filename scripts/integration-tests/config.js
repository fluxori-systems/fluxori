...
const config = {
  ...
  environments: {
    dev: {
      ...
      storage: {
        bucketName: process.env.DEV_STORAGE_BUCKET_NAME || 'fluxori-dev-storage',
      },
      firestore: {
    ...
    staging: {
      api: {
        baseUrl: process.env.STAGING_API_BASE_URL || 'https://api.staging.fluxori.com',
      },
      storage: {
        bucketName: process.env.STAGING_STORAGE_BUCKET_NAME || 'fluxori-staging-storage',
      },
      firestore: {
