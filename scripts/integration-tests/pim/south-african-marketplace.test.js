...
const generateSATestProduct = (marketplace = null) => {
  const baseProduct = {
    ...
    description:
      "This is a test product for South African marketplaces integration testing with VAT compliance.",
...
describe("South African Marketplace Integration", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (credentialsResponse.status !== 200) {
        console.warn(
          "Failed to fetch marketplace credentials - tests may be skipped",
        );
      ...
      if (Object.keys(marketplaceCredentials).length === 0) {
        console.warn(
          "No South African marketplace credentials available - tests may be skipped",
        );
        shouldRunTests = false;
        return;
      }
      ...
      console.log(
        "Available SA marketplace credentials:",
  ...
  describe("VAT Handling", () => {
    it("should correctly calculate and display South African VAT (15%)", async () => {
      ...
      const vatCalcResponse = await testUtils.api.post(
        `/pim/products/${productId}/calculate-vat`,
        {
          price: 230,
          vatIncluded: true,
        },
      ...
      const vatCalcResponse2 = await testUtils.api.post(
        `/pim/products/${productId}/calculate-vat`,
        {
          price: 200,
          vatIncluded: false,
  ...
  describe("South African Shipping Features", () => {
    it("should support SA-specific shipping regions", async () => {
      if (!shouldRunTests) {
        console.log(
          "Skipping shipping regions test - credentials or setup issue",
        );
        return;
      }
      ...
      const response = await testUtils.api.get(
        `/pim/products/${testProductId}/shipping-options`,
      );
    ...
    it("should calculate accurate shipping costs for SA regions", async () => {
      if (!shouldRunTests) {
        console.log(
          "Skipping shipping costs test - credentials or setup issue",
        );
        return;
      }

      // Test shipping cost calculation
      const shippingResponse = await testUtils.api.post(
        `/pim/products/${testProductId}/calculate-shipping`,
        {
          destination: {
            region: "gauteng",
  ...
  describe("Load Shedding Resilience", () => {
    ...
    const shouldRunLoadSheddingTests =
      !process.env.CI || process.env.RUN_LOAD_SHEDDING_TESTS === "true";
    ...
    it("should queue operations during load shedding", async () => {
      ...
      if (availableMarketplaces.length === 0) {
        console.log(
          "Skipping load shedding test - no marketplace credentials available",
        );
        return;
      }
      ...
      expect(["COMPLETED", "PARTIAL_SUCCESS"]).toContain(
        syncStatus.data.lastSyncStatus,
      );
    });
    ...
    it("should provide low-bandwidth UI during load shedding", async () => {
      if (!shouldRunTests || !shouldRunLoadSheddingTests) {
        console.log(
          "Skipping low-bandwidth UI test - environment or setup issue",
        );
        return;
      }
      ...
      expect(uiConfigResponse.data).toHaveProperty(
        "loadSheddingDetected",
        true,
      );
      ...
      expect(uiConfigAfterResponse.data).toHaveProperty(
        "lowBandwidthMode",
        false,
      );
      expect(uiConfigAfterResponse.data).toHaveProperty(
  ...
  describe("Takealot-Specific Features", () => {
    it("should validate Takealot-specific requirements", async () => {
      if (
        !shouldRunTests ||
        !marketplaceCredentials[SA_MARKETPLACES.TAKEALOT]
      ) {
        console.log(
          "Skipping Takealot validation test - no credentials available",
        );
        return;
      }
      ...
      const validationResponse = await testUtils.api.post(
        `/pim/products/${takealotProductId}/validate`,
        {
      ...
      expect(validationResponse.data.validationRules).toContain(
        "takealotBarcode",
      );
      expect(validationResponse.data.validationRules).toContain(
        "takealotOfferSystem",
    ...
    it("should format product data correctly for Takealot", async () => {
      ...
      const formatResponse = await testUtils.api.get(
        `/pim/products/${takealotProductId}/format`,
        {
          params: {
            marketplaceType: SA_MARKETPLACES.TAKEALOT,
          },
  ...
  describe("Multi-Warehouse Support", () => {
    ...
    beforeAll(async () => {
      ...
      for (const region of regions) {
        ...
        console.log(
          `Created test warehouse: ${warehouseResponse.data.id} (${region.name})`,
        );
      }
    ...
    it("should distribute stock across multiple South African warehouses", async () => {
      if (!shouldRunTests) {
        console.log(
          "Skipping multi-warehouse test - credentials or setup issue",
        );
        return;
      }

      // Get stock across all warehouses
      const stockResponse = await testUtils.api.get(
        `/inventory/products/${testProductId}/stock`,
      );
      ...
      const totalStock = stockResponse.data.reduce(
    ...
    it("should select optimal warehouse for order fulfillment based on region", async () => {
      if (!shouldRunTests) {
        console.log(
          "Skipping warehouse optimization test - credentials or setup issue",
        );
        return;
      }

      // Test warehouse selection algorithm
      const optimizationResponse = await testUtils.api.post(
      ...
      const selectedWarehouse = await testUtils.api.get(
        `/inventory/warehouses/${optimizationResponse.data.selectedWarehouseId}`,
      );
      expect(selectedWarehouse.data.location.state.toLowerCase()).toBe(
        "gauteng",
      );
    });
    ...
    afterAll(async () => {
      ...
      for (const warehouseId of testWarehouseIds) {
        try {
          ...
          console.error(
            `Error deleting warehouse ${warehouseId}:`,
            error.message,
          );
        }
      }
