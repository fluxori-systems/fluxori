...
console.log('========================================');
console.log('Options:');
console.log(`  Marketplace Tests:      ${options.marketplace ? 'Enabled' : 'Disabled'}`);
...
console.log(`  Load Shedding Tests:    ${options.loadShedding ? 'Enabled' : 'Disabled'}`);
console.log(`  Network Condition Tests: ${options.network ? 'Enabled' : 'Disabled'}`);
console.log(`  Skip Cleanup:           ${options.skipCleanup ? 'Enabled' : 'Disabled'}`);
console.log('========================================');
...
try {
  if (options.marketplace) {
    ...
    execSync(`npx jest ${TEST_FILES.marketplace} --forceExit --detectOpenHandles --json`, {
      stdio: 'inherit',
      env,
    });
  ...
  if (options.sa) {
    ...
    execSync(`npx jest ${TEST_FILES.sa} --forceExit --detectOpenHandles --json`, {
      stdio: 'inherit',
      env,
    });
  ...
  if (options.performance) {
    ...
    execSync(`npx jest ${TEST_FILES.performance} --forceExit --detectOpenHandles --testTimeout=300000 --json`, {
      stdio: 'inherit',
      env,
    });
