...
const generateTestProduct = (marketplaceSpecific = {}) => ({
  ...
  description:
    "This is a test product for integration testing the PIM module with marketplace connectors. It includes South African specific attributes such as VAT compliance information.",
...
describe("PIM - Marketplace Integration", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const productResponse = await testUtils.api.post(
        "/pim/products",
        generateTestProduct(),
      );
      ...
      for (const marketplace of Object.values(MARKETPLACES)) {
        if (!marketplaceCredentials[marketplace]) {
          console.log(
            `Skipping ${marketplace} tests - no credentials available`,
          );
          continue;
        }

        const marketplaceProduct = generateTestProduct(
          MARKETPLACE_SPECIFIC_DATA[marketplace],
        );
        ...
        console.log(
          `Created ${marketplace} test product: ${testProductIds[marketplace]}`,
        );
      }
    } catch (error) {
    ...
    const testMarketplaceSync = (marketplace) => {
      describe(`${marketplace} Integration`, () => {
        ...
        it(`should validate product for ${marketplace} marketplace`, async () => {
          ...
          const response = await testUtils.api.post(
            `/pim/products/${testProductIds[marketplace]}/validate`,
            {
          ...
          if (!response.data.valid) {
            console.log(
              `Validation issues for ${marketplace}:`,
        ...
        it(`should sync product to ${marketplace} marketplace`, async () => {
          ...
          const response = await testUtils.api.post(
            `/pim/products/${testProductIds[marketplace]}/sync`,
            {
              marketplaceType: marketplace,
              marketplaceCredentialId: marketplaceCredentials[marketplace],
            },
          ...
          expect(
            ["COMPLETED", "PARTIAL_SUCCESS"].includes(
          ...
          const productResponse = await testUtils.api.get(
            `/pim/products/${testProductIds[marketplace]}`,
          );
          expect(productResponse.data.marketplaceIds).toHaveProperty(
            marketplace,
          );
        });
        ...
        it(`should fetch product status from ${marketplace}`, async () => {
          ...
          const response = await testUtils.api.get(
            `/pim/products/${testProductIds[marketplace]}/marketplace-status`,
            {
              params: {
                marketplaceType: marketplace,
                marketplaceCredentialId: marketplaceCredentials[marketplace],
          ...
          expect(
            ["ACTIVE", "PENDING", "DRAFT"].includes(response.data.status),
          ).toBe(true);
        });
        ...
        it(`should update product on ${marketplace}`, async () => {
          ...
          const updateResponse = await testUtils.api.patch(
            `/pim/products/${testProductIds[marketplace]}`,
            {
          ...
          const syncResponse = await testUtils.api.post(
            `/pim/products/${testProductIds[marketplace]}/sync`,
            {
              marketplaceType: marketplace,
              marketplaceCredentialId: marketplaceCredentials[marketplace],
          ...
          expect(statusResponse.data).toHaveProperty(
            "lastSyncResult",
            "SUCCESS",
          );
        });
  ...
  describe("Batch Product Operations", () => {
    it("should sync multiple products to marketplaces", async () => {
      ...
      if (availableMarketplaces.length === 0) {
        console.log(
          "Skipping batch sync test - no marketplace credentials available",
        );
        return;
      }
      ...
      // Batch sync request
      const batchSyncResponse = await testUtils.api.post(
        "/pim/products/batch-sync",
        {
      ...
      expect(batchSyncResponse.data).toHaveProperty(
        "totalProducts",
      ...
      do {
        ...
        ["QUEUED", "PROCESSING"].includes(batchStatus) &&
  ...
  describe("South African Market Requirements", () => {
    it("should handle VAT calculation correctly (15%)", async () => {
      ...
      const productWithVAT = await testUtils.api.post(
        "/pim/products",
        generateTestProduct({
          pricing: {
      ...
      // Get VAT calculation
      const vatResponse = await testUtils.api.get(
        `/pim/products/${productId}/vat-calculation`,
      );
    ...
    it("should generate Takealot-specific attributes", async () => {
      if (!marketplaceCredentials[MARKETPLACES.TAKEALOT]) {
        console.log(
          "Skipping Takealot attributes test - no credentials available",
        );
        return;
      }
      ...
      const attributesResponse = await testUtils.api.get(
        `/pim/products/${takealotProductId}/marketplace-attributes`,
        {
          params: {
  ...
  describe("Network Resilience", () => {
    ...
    NETWORK_CONDITIONS.forEach((condition) => {
      it(`should sync product with ${condition} network condition`, async () => {
        ...
        if (availableMarketplaces.length === 0) {
          console.log(
            "Skipping network test - no marketplace credentials available",
          );
          return;
        }
        ...
        if (condition === "offline") {
          ...
          const syncStatus = await testUtils.api.get(
            `/pim/products/${productId}/sync-status`,
          );
          ...
          const waitTime =
            condition === "poor" || condition === "critical" ? 10000 : 5000;
  ...
  describe("Product Media Management", () => {
    ...
    it("should upload and attach media to a product", async () => {
      ...
      const attachResponse = await testUtils.api.post(
        `/pim/products/${testProductId}/media`,
        {
          url: fileUrl,
          type: "IMAGE",
    ...
    it("should sync product with media to marketplaces", async () => {
      ...
      if (availableMarketplaces.length === 0) {
        console.log(
          "Skipping media sync test - no marketplace credentials available",
        );
        return;
      }
      ...
      const mediaStatusResponse = await testUtils.api.get(
        `/pim/products/${testProductId}/marketplace-media-status`,
        {
          params: {
            marketplaceType: testMarketplace,
            marketplaceCredentialId: marketplaceCredentials[testMarketplace],
