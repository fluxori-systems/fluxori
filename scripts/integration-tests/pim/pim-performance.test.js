...
describe("PIM Performance Tests", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      const credentialsResponse = await testUtils.api.get(
      ...
      if (credentialsResponse.status === 200) {
        ...
        console.log(
          "Available marketplace credentials:",
          Object.keys(marketplaceCredentials),
        );
      }
      ...
      // Small catalog
      console.log(
        `Creating SMALL catalog (${CATALOG_SIZES.SMALL} products)...`,
      );
      productIds.SMALL = await createTestCatalog("SMALL", CATALOG_SIZES.SMALL);

      // Medium catalog
      console.log(
        `Creating MEDIUM catalog (${CATALOG_SIZES.MEDIUM} products)...`,
      );
      productIds.MEDIUM = await createTestCatalog(
        "MEDIUM",
        CATALOG_SIZES.MEDIUM,
      );

      // Large catalog (create fewer products in test environment)
      console.log(
        `Creating LARGE catalog (${CATALOG_SIZES.LARGE} products)...`,
      );
  ...
  async function createTestCatalog(sizeKey, size) {
    ...
    for (let batch = 0; batch < size / batchSize; batch++) {
      ...
      for (let i = 0; i < batchSize; i++) {
        ...
        batchPromises.push(
          testUtils.api.post("/pim/products", product).then((response) => {
      ...
      try {
        ...
        console.log(
          `Created batch ${batch + 1}/${Math.ceil(size / batchSize)}: ${batchResults.length} products`,
        );
      } catch (error) {
  ...
  describe("Search Performance", () => {
    const testSearch = (sizeKey, expectedMaxTime) => {
      it(`should search ${sizeKey} catalog within performance threshold`, async () => {
        ...
        console.log(
          `Running search performance test for ${sizeKey} catalog...`,
        );
        ...
        console.log(
          `${sizeKey} catalog search took ${duration}ms for ${response.data.total} total matches`,
        );
    ...
    it("should filter products with complex criteria efficiently", async () => {
      ...
      console.log(
        `Complex filtering took ${duration}ms for ${response.data.total} matches`,
      );

      // Assert filtering performance
      expect(duration).toBeLessThan(
        PERFORMANCE_THRESHOLDS.LARGE_CATALOG.filter,
      );
    });
  });
  ...
  describe("Batch Operations Performance", () => {
    const testBatchUpdate = (sizeKey, expectedMaxTime) => {
      it(`should perform batch updates on ${sizeKey} catalog within performance threshold`, async () => {
        ...
        const productBatch = productIds[sizeKey].slice(
          0,
          Math.min(50, productIds[sizeKey].length),
        );

        console.log(
          `Running batch update for ${productBatch.length} products from ${sizeKey} catalog...`,
        );
        ...
        console.log(
          `Batch update initiated: ${batchId}, duration: ${duration}ms`,
        );
        ...
        do {
          ...
          const statusResponse = await testUtils.api.get(
            `/pim/batch/${batchId}/status`,
          );
        ...
        // Final status check
        const finalStatusResponse = await testUtils.api.get(
          `/pim/batch/${batchId}/status`,
        );
        expect(finalStatusResponse.status).toBe(200);
        expect(["COMPLETED", "PARTIAL_SUCCESS"]).toContain(
          finalStatusResponse.data.status,
        );
        ...
        const updatedProduct = await testUtils.api.get(
          `/pim/products/${productBatch[0]}`,
        );
        expect(updatedProduct.data.attributes).toHaveProperty(
          "batchUpdated",
          "true",
        );
      });
    ...
    testBatchUpdate(
      "MEDIUM",
      PERFORMANCE_THRESHOLDS.MEDIUM_CATALOG.batchUpdate,
    );
    ...
    it("should handle large catalog export efficiently", async () => {
      ...
      console.log(
        `Export initiated: ${exportId}, request duration: ${duration}ms`,
      );
      ...
      const fileResponse = await testUtils.api.get(
        `/pim/export/${exportId}/download`,
      );
  ...
  describe("Marketplace Integration Performance", () => {
    it("should efficiently sync products to marketplaces", async () => {
      ...
      const productBatch = productIds.MEDIUM.slice(
        0,
        Math.min(20, productIds.MEDIUM.length),
      );

      console.log(
        `Testing marketplace sync with ${productBatch.length} products...`,
      );
      ...
      const batchId = response.data.batchId;
      console.log(
        `Batch sync initiated: ${batchId}, request duration: ${duration}ms`,
      );
      ...
      expect(completionTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.MEDIUM_CATALOG.batchSync,
      );

      // Check sync results
      const resultsResponse = await testUtils.api.get(
        `/pim/batch/${batchId}/results`,
      );
    ...
    it("should handle marketplace sync timeout gracefully", async () => {
      ...
      const response = await testUtils.api.post(
        `/pim/products/${testProductId}/sync`,
        {
          marketplaceType: testMarketplace,
          marketplaceCredentialId: marketplaceCredentials[testMarketplace],
      ...
      const productResponse = await testUtils.api.get(
        `/pim/products/${testProductId}`,
      );
      expect(productResponse.status).toBe(200);
      expect(productResponse.data).toHaveProperty("syncStatus");
      expect(productResponse.data.syncStatus[testMarketplace]).not.toBe(
        "SYNCING",
      );
    });
  });
  ...
  describe("Network Resilience Performance", () => {
    ...
    const shouldRunNetworkTests =
      !process.env.CI || process.env.RUN_NETWORK_TESTS === "true";
    ...
    it("should adapt performance for poor network conditions", async () => {
      if (!shouldRunTests || !shouldRunNetworkTests) {
        console.log(
          "Skipping network performance test - environment or setup issue",
        );
        return;
      }
      ...
      expect(poorNetworkConfig.paginationLimit).toBeLessThan(
        normalConfig.paginationLimit,
      );
    ...
    it("should use progressive loading for large result sets", async () => {
      if (!shouldRunTests) {
        console.log(
          "Skipping progressive loading test - environment or setup issue",
        );
        return;
      }
      ...
      expect(firstPageDuration).toBeLessThan(
        PERFORMANCE_THRESHOLDS.SMALL_CATALOG.search,
      );
      ...
      expect(secondPageResponse.data.total).toBeGreaterThanOrEqual(
        secondPageResponse.data.estimatedTotal,
      );
    });
  });
  ...
  afterAll(async () => {
    ...
    console.log(
      "Cleaning up performance test data - this may take some time...",
    );

    const cleanupCatalog = async (sizeKey) => {
      console.log(
        `Cleaning up ${sizeKey} catalog (${productIds[sizeKey].length} products)...`,
      );
      ...
      for (let i = 0; i < productIds[sizeKey].length; i += batchSize) {
        ...
        console.log(
          `Deleted batch ${Math.floor(i / batchSize) + 1}: ${batchSuccessCount}/${batch.length} products`,
        );
      ...
      console.log(
        `Cleanup complete for ${sizeKey} catalog: ${successCount}/${productIds[sizeKey].length} products deleted`,
      );
    };
