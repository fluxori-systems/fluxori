...
program
  ...
  .option('-e, --env <environment>', 'Environment to test (dev, staging, production)', 'dev')
  ...
  .option('-d, --duration <duration>', 'Test duration (short, medium, long)', 'medium')
  ...
  .option('--parallel <count>', 'Run tests in parallel with specified number of workers')
...
if (!testPath && options.scenario) {
  ...
  if (!testPath) {
    ...
    console.log(chalk.yellow('Available scenarios: auth, storage, inventory, orders, ai, e2e'));
    process.exit(1);
  }
}
...
console.log(chalk.blue(`Skip slow tests: ${chalk.yellow(process.env.SKIP_SLOW_TESTS === 'true')}`));
console.log(chalk.blue(`Skip non-critical tests: ${chalk.yellow(process.env.SKIP_NON_CRITICAL_TESTS === 'true')}`));
