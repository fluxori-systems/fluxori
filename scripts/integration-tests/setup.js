...
global.testUtils = {
  ...
  cleanupTestFiles: async () => {
    ...
    for (const file of files) {
      if (file.metadata && file.metadata.metadata && file.metadata.metadata._testData === 'true') {
...
beforeAll(async () => {
  ...
  if (config.auth.email && config.auth.password) {
    try {
      ...
      console.warn(chalk.yellow('Pre-test authentication failed, tests may fail if they require auth'));
    }
  } else {
    console.warn(chalk.yellow('No authentication credentials provided, tests requiring auth may fail'));
  }
});
