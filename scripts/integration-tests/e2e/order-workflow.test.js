...
describe('Order Processing Workflow', () => {
  ...
  describe('Order Ingestion', () => {
    it('should ingest a new order from JSON payload', async () => {
      ...
      expect(response.data).toHaveProperty('externalId', orderPayload.externalId);
  ...
  describe('Order Fulfillment', () => {
    it('should create a fulfillment for the order', async () => {
      ...
      const response = await testUtils.api.post('/orders/fulfillments', fulfillmentPayload);
      ...
      const orderResponse = await testUtils.api.get(`/orders/${testOrderId}`);
      expect(orderResponse.data).toHaveProperty('fulfillmentStatus', 'FULFILLED');
    });
    ...
    it('should mark fulfillment as shipped', async () => {
      ...
      // Mark as shipped
      const response = await testUtils.api.patch(`/orders/fulfillments/${fulfillmentId}/ship`);
      
      expect(response.status).toBe(200);
      expect(response.data).toHaveProperty('status', 'SHIPPED');
      
      // Verify the order was updated
      const updatedOrderResponse = await testUtils.api.get(`/orders/${testOrderId}`);
      expect(updatedOrderResponse.data).toHaveProperty('fulfillmentStatus', 'SHIPPED');
  ...
  describe('Order Delivery', () => {
    it('should mark the order as delivered', async () => {
      ...
      // Mark as delivered
      const response = await testUtils.api.patch(`/orders/fulfillments/${fulfillmentId}/deliver`);
      ...
      // Verify the order was updated
      const updatedOrderResponse = await testUtils.api.get(`/orders/${testOrderId}`);
      expect(updatedOrderResponse.data).toHaveProperty('fulfillmentStatus', 'DELIVERED');
