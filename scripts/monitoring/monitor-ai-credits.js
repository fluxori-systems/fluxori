...
async function writeCustomMetric(metricType, value, labels = {}) {
  if (dryRun) {
    console.log(chalk.blue(`[DRY RUN] Would write metric ${metricType} = ${value} with labels:`, labels));
    return;
  }
  ...
  try {
    ...
    console.log(chalk.green(`Successfully wrote metric ${metricType} = ${value}`));
  } catch (error) {
    console.error(chalk.red(`Error writing custom metric ${metricType}:`), error.message);
...
async function getOrganizationCreditData() {
  try {
    ...
    const allotmentsSnapshot = await firestore.collection('ai_credit_allotments').get();
    ...
    const usageSnapshot = await firestore.collection('ai_credit_usage')
    ...
    const orgData = orgsSnapshot.docs.map(doc => {
      ...
      const allotment = allotments.find(a => a.organizationId === org.id) || { monthlyLimit: 0 };
      ...
      return {
        ...
        currentUsage,
        usagePercentage: allotment.monthlyLimit ? (currentUsage / allotment.monthlyLimit) * 100 : 0,
      };
    });
    
    return orgData;
  } catch (error) {
    console.error(chalk.red('Error getting organization credit data:'), error.message);
...
async function reportCreditMetrics() {
  ...
  for (const org of orgData) {
    console.log(chalk.blue(`Reporting metrics for organization: ${org.name} (${org.id})`));
    ...
    await writeCustomMetric('ai_credits_current_usage', Math.round(org.currentUsage), {
      organization_id: org.id,
      organization_name: org.name,
    });
    
    // Report usage percentage
    await writeCustomMetric('ai_credit_usage_percentage', Math.round(org.usagePercentage), {
      organization_id: org.id,
      organization_name: org.name,
    });
    ...
    if (org.usagePercentage >= 80 && !dryRun) {
      console.log(chalk.yellow(`Organization ${org.name} is approaching credit limit (${Math.round(org.usagePercentage)}%)`));
  ...
  console.log(chalk.green('Successfully reported credit metrics for all organizations.'));
}
