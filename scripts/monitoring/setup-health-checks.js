...
async function createUptimeCheck(displayName, url, resource) {
  try {
    ...
    console.error(
      chalk.red(`Error creating uptime check ${displayName}:`),
      error.message,
    );
    return null;
  }
...
async function setupUptimeChecks() {
  ...
  try {
    ...
    for (const endpoint of apiEndpoints) {
      ...
      await createUptimeCheck(`Fluxori ${endpoint.name} (${env})`, url, {
        type: "uptime_url",
        labels: {
...
async function createUptimeAlertPolicy() {
  ...
  try {
    ...
    const notificationChannels =
      await monitoringAdmin.projects.notificationChannels.list({
    ...
    if (channelIds.length === 0) {
      console.warn(
        chalk.yellow(
          "Warning: No notification channels found. Alerts will not be sent.",
        ),
      );
    }
      ...
      conditions: [
        {
          ...
          conditionThreshold: {
            filter:
              'metric.type="monitoring.googleapis.com/uptime_check/check_passed" AND resource.type="uptime_url" AND metric.label.check_id=~"fluxori.*"',
...
`,
    ...
    console.log(
      chalk.green(`Created uptime check alert policy: ${response.data.name}`),
    );
  } catch (error) {
