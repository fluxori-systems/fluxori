...
const collectionNameMap = {
  ...
  organizations: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_organizations`
    : "organizations",
  products: COLLECTION_PREFIX ? `${COLLECTION_PREFIX}_products` : "products",
  warehouses: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_warehouses`
    : "warehouses",
  stockLevels: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_stockLevels`
    : "stockLevels",
  orders: COLLECTION_PREFIX ? `${COLLECTION_PREFIX}_orders` : "orders",
  marketplaceCredentials: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_marketplaceCredentials`
    : "marketplaceCredentials",
  buyboxStatuses: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_buyboxStatuses`
    : "buyboxStatuses",
  buyboxHistories: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_buyboxHistories`
    : "buyboxHistories",
  repricingRules: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_repricingRules`
    : "repricingRules",
  insights: COLLECTION_PREFIX ? `${COLLECTION_PREFIX}_insights` : "insights",
  aiModelConfigs: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_aiModelConfigs`
    : "aiModelConfigs",
  notifications: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_notifications`
    : "notifications",
  notificationSettings: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_notificationSettings`
    : "notificationSettings",
  internationalShipments: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_internationalShipments`
    : "internationalShipments",
  hsCodes: COLLECTION_PREFIX ? `${COLLECTION_PREFIX}_hsCodes` : "hsCodes",
  tradeRestrictions: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_tradeRestrictions`
    : "tradeRestrictions",
  complianceRequirements: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_complianceRequirements`
    : "complianceRequirements",
  documents: COLLECTION_PREFIX ? `${COLLECTION_PREFIX}_documents` : "documents",
  embeddingProviders: COLLECTION_PREFIX
    ? `${COLLECTION_PREFIX}_embeddingProviders`
    : "embeddingProviders",
};
...
async function importToFirestore(firestore, collectionName, documents) {
  const firestoreCollectionName =
    collectionNameMap[collectionName] || collectionName;
  ...
  console.log(
    `Imported ${documents.length} documents to ${firestoreCollectionName}`,
  );
}
  ...
  try {
    ...
    for (const collectionName of collections) {
      try {
        ...
        console.error(
          `Error migrating collection ${collectionName}. Continuing with next collection.`,
        );
      }
    }
