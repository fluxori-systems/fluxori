...
function analyzeBackendModules() {
  ...
  modules.forEach(module => {
    try {
      ...
      if (fs.existsSync(indexPath)) {
        ...
        const importRegex = /import\s+(?:(?:[{}\s\w*,]+from\s+)?["']\.\.\/([^"'\/]+))/g;
        let match;
  ...
  modules.forEach(module => {
    console.log(`\n${module}:`);
    console.log(`  Depends on: ${dependencies[module].dependsOn.join(', ') || 'none'}`);
    console.log(`  Imported by: ${dependencies[module].importedBy.join(', ') || 'none'}`);
  });
...
function generateDotGraph(modules, dependencies) {
  ...
  // Save to file
  const outputPath = path.resolve(__dirname, '../backend/module-dependencies.dot');
...
function analyzeFrontendModules() {
  ...
  if (fs.existsSync(libDir)) {
    const libs = fs.readdirSync(libDir).filter(file => 
      fs.statSync(path.join(libDir, file)).isDirectory()
    );
...
function generateFrontendDot(frontendReport) {
  ...
  if (frontendReport.libs.includes('ui') && frontendReport.libs.includes('shared')) {
    dot += '  "ui" -> "shared";\n';
  }
  
  if (frontendReport.libs.includes('motion') && frontendReport.libs.includes('shared')) {
    dot += '  "motion" -> "shared";\n';
  }
  ...
  // Save to file
  const outputPath = path.resolve(__dirname, '../frontend/module-dependencies.dot');
  fs.writeFileSync(outputPath, dot);
...
function convertDotToSvg() {
  ...
  const backendDot = path.resolve(__dirname, '../backend/module-dependencies.dot');
  const frontendDot = path.resolve(__dirname, '../frontend/module-dependencies.dot');
