...
function analyzeModules() {
  ...
  Object.keys(MODULE_PATHS).forEach((moduleName) => {
    ...
    console.log(
      `Found ${moduleFiles[moduleName].length} files in ${moduleName} module`,
    );
  });
  ...
  Object.keys(moduleFiles).forEach((moduleName) => {
    ...
    console.log(
      `Found ${moduleImports[moduleName].length} imports in ${moduleName} module`,
    );
  });
  ...
  if (
    moduleDependencies["ui→motion"] > 0 &&
    moduleDependencies["motion→ui"] > 0
  ) {
    console.log(
    ...
    crossModuleImports
      .filter(
        (imp) => imp.source.module === "ui" && imp.target.module === "motion",
      )
      .forEach((imp) => {
    ...
    crossModuleImports
      .filter(
        (imp) => imp.source.module === "motion" && imp.target.module === "ui",
      )
      .forEach((imp) => {
        console.log(`  ${imp.source.file} imports from ${imp.target.path}`);
      });
  ...
  if (
    ...
    console.log(
      "  1. Resolve circular dependencies between UI and Motion modules:",
    );
    console.log("     - Move shared functionality to the Shared module");
    console.log(
      "     - Create interfaces in the Shared module that both UI and Motion can implement",
    );
    console.log("     - Use dependency injection to avoid direct imports");
  }

  if (
    moduleDependencies["ui→shared"] === 0 ||
    moduleDependencies["motion→shared"] === 0
  ) {
    console.log("  2. Make better use of the Shared module:");
    console.log(
      "     - Extract common types and interfaces to the Shared module",
    );
  ...
  console.log(
    "     - Consider making UI and Motion completely independent of each other",
  );
}
