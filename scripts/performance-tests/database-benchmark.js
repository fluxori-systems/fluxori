...
if (!keyFilePath) {
  keyFilePath = path.join(__dirname, `../../credentials/${env}-service-account.json`);
  if (!fs.existsSync(keyFilePath)) {
    console.error(chalk.red(`Error: Service account key file not found at ${keyFilePath}`));
    console.error(chalk.yellow('Please set GOOGLE_APPLICATION_CREDENTIALS environment variable or provide credentials file'));
    process.exit(1);
  }
}
...
async function runReadBenchmark(collection, samples = 100, useCache = true) {
  ...
  console.log(chalk.green(`Read benchmark completed: ${result.operationsPerSecond} ops/sec, avg latency: ${result.averageLatencyMs}ms`));
...
async function runWriteBenchmark(collection, samples = 100) {
  ...
  console.log(chalk.green(`Write benchmark completed: ${result.operationsPerSecond} ops/sec, avg latency: ${result.averageLatencyMs}ms`));
...
async function runQueryBenchmark(collection, samples = 50) {
  ...
  for (let i = 0; i < queriesPerType; i++) {
    await firestore.collection(collection)
      .where('isActive', '==', true)
      .where('timestamp', '>', Firestore.Timestamp.fromMillis(Date.now() - 24 * 60 * 60 * 1000))
      .limit(20)
      .get();
  }
  ...
  try {
    ...
    console.log(chalk.yellow(`Skipping complex query due to error: ${error.message}`));
  }
  ...
  console.log(chalk.green(`Query benchmark completed: ${result.operationsPerSecond} ops/sec, avg latency: ${result.averageLatencyMs}ms`));
...
async function runTransactionBenchmark(collection, samples = 50) {
  ...
  performance.measure('Transaction Operations', 'transaction-start', 'transaction-end');
  
  const duration = performance.getEntriesByName('Transaction Operations')[0].duration;
  const result = {
  ...
  console.log(chalk.green(`Transaction benchmark completed: ${result.operationsPerSecond} ops/sec, avg latency: ${result.averageLatencyMs}ms`));
...
async function runBenchmarks() {
  ...
  console.log(chalk.blue(`Project ID: ${process.env.GCP_PROJECT_ID || `fluxori-${env}`}`));
  ...
  for (const collection of collections) {
    try {
      ...
      if (!snapshot.empty) {
        ...
        console.log(chalk.yellow(`Collection ${collection} is empty, skipping...`));
      }
    } catch (error) {
      console.log(chalk.yellow(`Error accessing collection ${collection}, skipping: ${error.message}`));
    }
  }
  
  if (validCollections.length === 0) {
    console.error(chalk.red('No valid collections found with data. Please populate the database first.'));
    process.exit(1);
  }
  ...
  for (const collection of validCollections) {
    ...
    // Run read benchmarks
    results.collections[collection].read = await runReadBenchmark(collection, readSamples, true);
    results.collections[collection].readUncached = await runReadBenchmark(collection, readSamples, false);
    
    // Run write benchmarks
    results.collections[collection].write = await runWriteBenchmark(collection, writeSamples);
    
    // Run query benchmarks
    results.collections[collection].query = await runQueryBenchmark(collection, querySamples);
    
    // Run transaction benchmarks
    results.collections[collection].transaction = await runTransactionBenchmark(collection, transactionSamples);
  }
  ...
  const overallStats = {
    read: {
      ...
      averageOpsPerSecond: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].read.operationsPerSecond), 0) / validCollections.length,
      averageLatencyMs: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].read.averageLatencyMs), 0) / validCollections.length,
    },
    write: {
      totalOperations: validCollections.length * writeSamples,
      averageOpsPerSecond: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].write.operationsPerSecond), 0) / validCollections.length,
      averageLatencyMs: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].write.averageLatencyMs), 0) / validCollections.length,
    },
    query: {
      totalOperations: validCollections.length * querySamples,
      averageOpsPerSecond: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].query.operationsPerSecond), 0) / validCollections.length,
      averageLatencyMs: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].query.averageLatencyMs), 0) / validCollections.length,
    },
    transaction: {
      totalOperations: validCollections.length * transactionSamples,
      averageOpsPerSecond: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].transaction.operationsPerSecond), 0) / validCollections.length,
      averageLatencyMs: validCollections.reduce((acc, col) => 
        acc + parseFloat(results.collections[col].transaction.averageLatencyMs), 0) / validCollections.length,
    },
  };
  ...
  const resultsFile = path.join(resultsDir, `firestore-benchmark-${env}-${timestamp}.json`);
  ...
  console.log(chalk.bold.green('\nOverall Performance:'));
  console.log(chalk.cyan(`Read operations: ${overallStats.read.averageOpsPerSecond.toFixed(2)} ops/sec (${overallStats.read.averageLatencyMs.toFixed(2)}ms avg latency)`));
  console.log(chalk.cyan(`Write operations: ${overallStats.write.averageOpsPerSecond.toFixed(2)} ops/sec (${overallStats.write.averageLatencyMs.toFixed(2)}ms avg latency)`));
  console.log(chalk.cyan(`Query operations: ${overallStats.query.averageOpsPerSecond.toFixed(2)} ops/sec (${overallStats.query.averageLatencyMs.toFixed(2)}ms avg latency)`));
  console.log(chalk.cyan(`Transaction operations: ${overallStats.transaction.averageOpsPerSecond.toFixed(2)} ops/sec (${overallStats.transaction.averageLatencyMs.toFixed(2)}ms avg latency)`));
  ...
  try {
    ...
    console.log(chalk.green('Benchmark results saved to Firestore collection "benchmark_results"'));
  } catch (error) {
    console.error(chalk.yellow('Failed to save benchmark results to Firestore:'), error.message);
  }
}
