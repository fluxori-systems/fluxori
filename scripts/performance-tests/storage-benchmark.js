...
async function runUploadBenchmark(fileSizeBytes, iterations = 5) {
  ...
  try {
    ...
    if (!exists) {
      console.log(chalk.yellow(`Bucket ${bucketName} doesn't exist. Creating it...`));
    ...
    console.error(chalk.red(`Error creating/accessing bucket: ${error.message}`));
    return null;
  }
  ...
  console.log(chalk.blue(`Creating test file of ${fileSizeMB.toFixed(2)}MB...`));
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    try {
      ...
      console.log(chalk.green(`Upload complete in ${durationMs.toFixed(2)}ms (${throughputMBps} MB/s)`));
  ...
  console.log(chalk.bold.green(`Upload benchmark complete for ${fileSizeMB.toFixed(2)}MB file:`));
  console.log(chalk.bold.green(`Average throughput: ${avgThroughput.toFixed(2)} MB/s`));
  console.log(chalk.bold.green(`Average duration: ${avgDuration.toFixed(2)}ms`));
...
async function runDownloadBenchmark(fileSizeBytes, iterations = 5) {
  ...
  try {
    ...
    files = files.filter(file => {
      ...
      return metadata?.benchmark === 'true' && parseInt(metadata?.fileSize) === fileSizeBytes;
    });
    
    if (files.length === 0) {
      console.error(chalk.yellow(`No test files found for size ${fileSizeMB}MB. Run upload benchmark first.`));
      return null;
    }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      console.log(chalk.green(`Download complete in ${durationMs.toFixed(2)}ms (${throughputMBps} MB/s)`));
  ...
  console.log(chalk.bold.green(`Download benchmark complete for ${fileSizeMB.toFixed(2)}MB file:`));
  console.log(chalk.bold.green(`Average throughput: ${avgThroughput.toFixed(2)} MB/s`));
  console.log(chalk.bold.green(`Average duration: ${avgDuration.toFixed(2)}ms`));
...
async function runMetadataBenchmark(iterations = 50) {
  ...
  try {
    ...
    if (files.length === 0) {
      console.error(chalk.yellow(`No test files found. Run upload benchmark first.`));
      return null;
    }
    
    // Limit to the number of iterations
    if (files.length < iterations) {
      console.log(chalk.yellow(`Only ${files.length} files available for testing.`));
    }
  ...
  try {
    ...
    console.error(chalk.red(`listFiles (prefix+maxResults) failed: ${error.message}`));
  }
  
  // Calculate averages
  const getMetadataAvg = results.getMetadata.reduce((sum, r) => sum + r.durationMs, 0) / results.getMetadata.length;
  const updateMetadataAvg = results.updateMetadata.reduce((sum, r) => sum + r.durationMs, 0) / results.updateMetadata.length;
  
  console.log(chalk.bold.green(`Metadata benchmark complete:`));
  console.log(chalk.bold.green(`Average getMetadata duration: ${getMetadataAvg.toFixed(2)}ms`));
  console.log(chalk.bold.green(`Average updateMetadata duration: ${updateMetadataAvg.toFixed(2)}ms`));
...
async function cleanupBenchmarkFiles() {
  ...
  try {
    ...
    console.log(chalk.green(`Cleanup complete. Deleted ${files.length} files.`));
  } catch (error) {
  ...
  try {
    ...
    console.error(chalk.red(`Error cleaning up temp directory: ${error.message}`));
  }
}
...
async function runAllBenchmarks() {
  console.log(chalk.bold.green('Starting Google Cloud Storage Performance Benchmarks'));
  ...
  if (!process.env.KEEP_BENCHMARK_FILES) {
    ...
    console.log(chalk.yellow(`Keeping benchmark files as KEEP_BENCHMARK_FILES is set.`));
  }
  
  // Save results
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const resultsFile = path.join(resultsDir, `gcs-benchmark-${env}-${timestamp}.json`);
  ...
  Object.entries(results.upload).forEach(([size, data]) => {
    if (data) {
      console.log(chalk.cyan(`${size}: ${data.averageThroughputMBps} MB/s (${data.averageDurationMs.toFixed(2)}ms)`));
    }
  });
  ...
  if (results.metadata) {
    console.log(chalk.bold.cyan('\nMetadata Operations:'));
    console.log(chalk.cyan(`getMetadata: ${results.metadata.results.getMetadataAvgMs}ms average`));
    console.log(chalk.cyan(`updateMetadata: ${results.metadata.results.updateMetadataAvgMs}ms average`));
    
    results.metadata.results.listFiles.forEach((data) => {
      console.log(chalk.cyan(`${data.description}: ${data.durationMs.toFixed(2)}ms`));
    });
  }
}
