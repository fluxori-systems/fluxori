...
const {
  NetworkQuality,
} = require("../../backend/src/modules/pim/interfaces/types");
...
program
  ...
  .option(
    "--catalog-size <size>",
    "Catalog size to simulate (small, medium, large, xlarge)",
    "medium",
  )
  .option(
    "--network <network>",
    "Network condition (excellent, good, fair, poor, critical, offline)",
    "good",
  )
...
const CONFIG = {
  ...
  testQueries: [
    ...
    { name: "With status filter", options: { status: ["active"], limit: 20 } },
    {
      ...
      options: { categories: ["category-1"], limit: 20 },
    },
    {
      name: "Price range",
      options: { priceRange: { min: 100, max: 500 }, limit: 20 },
    },
    ...
    { name: "Pagination - Regular", options: { page: 2, limit: 20 } },
    {
      ...
      options: { cursor: "cursor-placeholder", limit: 20 },
    },
    { name: "Minimal fields", options: { limit: 20, minimalFields: true } },
    {
      name: "Progressive loading",
      options: { limit: 20, progressiveLoading: true },
    },
    {
      name: "Sort by price",
      options: { sortBy: "price", sortDirection: "asc", limit: 20 },
    },
...
async function generateTestData(catalogSize) {
  console.log(
    chalk.cyan(`Generating test data for catalog size: ${catalogSize}`),
  );
...
function createRepositories(
    ...
    async search(organizationId, options = {}) {
      ...
      if (options.priceRange) {
        if (options.priceRange.min !== undefined) {
          filtered = filtered.filter(
            (p) => p.pricing.basePrice >= options.priceRange.min,
          );
        }
        if (options.priceRange.max !== undefined) {
          filtered = filtered.filter(
            (p) => p.pricing.basePrice <= options.priceRange.max,
          );
        }
      }
      ...
      if (options.stockRange) {
        if (options.stockRange.min !== undefined) {
          filtered = filtered.filter(
            (p) => p.stockQuantity >= options.stockRange.min,
          );
        }
        if (options.stockRange.max !== undefined) {
          filtered = filtered.filter(
            (p) => p.stockQuantity <= options.stockRange.max,
          );
        }
      }
  ...
  const optimizedRepository = {
    ...
    async searchOptimized(organizationId, options = {}) {
      ...
      if (options.cacheResults !== false) {
        ...
        if (cached) {
          ...
          performance.measure(
            "Optimized Repository Search (Cached)",
            "optimized-start",
            "optimized-end",
          );
          return cached;
      ...
      if (options.cursor) {
        ...
        if (cursorIndex >= 0) {
          result = filtered.slice(
            cursorIndex + 1,
            cursorIndex + 1 + (options.limit || 20),
          );
        } else {
    ...
    generateCacheKey(organizationId, options) {
      ...
      const sortedOptions = Object.keys(cacheableOptions)
        .sort()
        .reduce((obj, key) => {
  ...
  const catalogOptimizationService = {
    ...
    async searchProducts(organizationId, options) {
      ...
      if (options.adaptToNetworkQuality) {
        this.adaptOptionsToNetworkQuality(
          options,
          networkQuality,
          loadSheddingStatus.isActive,
        );
      }
      ...
      try {
        ...
        const searchResult =
          await this.loadSheddingService.executeWithResilience(
            () =>
              this.optimizedRepository.searchOptimized(organizationId, {
                ...options,
                minimalFields:
                  options.minimalFields ||
...
async function runTest(
  ...
  const { regularRepository, optimizedRepository, catalogOptimizationService } =
    repositories;
  ...
  if (testCase.options.cursor === "cursor-placeholder") {
    ...
    if (firstPage.products.length > 0) {
      ...
      console.warn(
        chalk.yellow("Skipping cursor-based test due to no products found"),
      );
      return null;
    }
  }
  ...
  for (let i = 0; i < numRuns; i++) {
    ...
    try {
      ...
      console.error(
        chalk.red(
          `Regular repository error (${testCase.name}): ${error.message}`,
        ),
      );
    ...
    try {
      ...
      console.error(
        chalk.red(
          `Optimized repository error (${testCase.name}): ${error.message}`,
        ),
      );
    ...
    try {
      ...
      console.error(
        chalk.red(`Service error (${testCase.name}): ${error.message}`),
      );
  ...
  if (serviceValid.length > 0) {
    results.service.avg =
      serviceValid.reduce((a, b) => a + b, 0) / serviceValid.length;
  ...
  if (results.optimized.avg > 0 && results.service.avg > 0) {
    results.serviceOverhead =
      ((results.service.avg - results.optimized.avg) / results.optimized.avg) *
      100;
  }
...
async function runBenchmarks() {
  ...
  // Generate test data
  const catalogSize =
    CONFIG.catalogSizes[options.catalogSize] || CONFIG.catalogSizes.medium;
...
async function generateReport(results) {
  ...
  const outputFile = path.join(
    resultsDir,
    `pim-benchmark-${options.catalogSize}-${options.network}-ls${options.loadShedding}-${timestamp}.json`,
  );
  ...
  if (improvements.length > 0) {
    summary.summary.averageImprovement =
      improvements.reduce((a, b) => a + b, 0) / improvements.length;

    // Sort for median and min/max
    improvements.sort((a, b) => a - b);

    summary.summary.medianImprovement =
      improvements.length % 2 === 0
        ? (improvements[improvements.length / 2 - 1] +
    ...
    summary.summary.queriesWithImprovementOver50Percent = improvements.filter(
      (i) => i >= 50,
    ).length;
    summary.summary.queriesWithImprovementOver20Percent = improvements.filter(
      (i) => i >= 20,
    ).length;
    summary.summary.queriesWithImprovementUnder10Percent = improvements.filter(
      (i) => i < 10,
    ).length;
  }
  ...
  const overheads = results
    .map((r) => r.serviceOverhead)
    .filter((i) => !isNaN(i));

  if (overheads.length > 0) {
    summary.summary.averageServiceOverhead =
...
${results
  ...
  .map(
    (r, i) =>
      `${i + 1}. ${r.name}: ${r.improvement.toFixed(2)}% improvement (${r.regular.avg.toFixed(2)}ms → ${r.optimized.avg.toFixed(2)}ms)`,
  )
  .join("\n")}
...
Service Overhead: ${r.serviceOverhead.toFixed(2)}%`,
  )
  .join("\n")}
...
${
  ...
  const summaryFile = path.join(
    resultsDir,
    `pim-benchmark-summary-${options.catalogSize}-${options.network}-ls${options.loadShedding}-${timestamp}.txt`,
  );
  ...
  console.log(
    chalk.cyan(
      `Average Performance Improvement: ${summary.summary.averageImprovement.toFixed(2)}%`,
    ),
  );
  console.log(
    chalk.cyan(
  ...
  top3.forEach((r, i) => {
    console.log(
      chalk.cyan(
        `${i + 1}. ${r.name}: ${r.improvement.toFixed(2)}% improvement`,
      ),
    );
  });
