...
// Initialize log file
fs.writeFileSync(logFile, `CDN Performance Test - ${new Date().toISOString()}\n\n`);
...
const makeRequest = async (url, expectedCacheStatus = null) => {
  try {
    ...
    const serverLocation = response.headers['server-location'] || 
...
const testAsset = async (assetPath, location, isCDN = true) => {
  ...
  successfulResults.forEach(result => {
    ...
    statistics.cacheStatuses[status] = (statistics.cacheStatuses[status] || 0) + 1;
  });
...
const runCdnTests = async () => {
  ...
  for (const fileType of config.fileTypes) {
    ...
    for (const location of config.locations) {
      ...
      if (cdnResult) {
        ...
        log(`CDN test complete. Average response time: ${cdnResult.statistics.averageResponseTime?.toFixed(2)}ms`);
        log(`Cache statuses: ${JSON.stringify(cdnResult.statistics.cacheStatuses)}`);
      }
      ...
      if (config.directStorageUrl) {
        ...
        if (directResult) {
          ...
          log(`Direct storage test complete. Average response time: ${directResult.statistics.averageResponseTime?.toFixed(2)}ms`);
        }
      }
    }
  }
  ...
  for (const fileType of config.fileTypes) {
    ...
    const cdnAvg = cdnResults.reduce((sum, r) => sum + (r.statistics.averageResponseTime || 0), 0) / cdnResults.length;
    const directAvg = directResults.length > 0
      ? directResults.reduce((sum, r) => sum + (r.statistics.averageResponseTime || 0), 0) / directResults.length
      : null;
    
    const improvement = directAvg ? ((directAvg - cdnAvg) / directAvg) * 100 : null;
    
    resultsByFileType[fileType.name] = {
      cdnAverageResponseTime: cdnAvg,
      directAverageResponseTime: directAvg,
      improvementPercentage: improvement,
      cacheHitRate: cdnResults.reduce((sum, r) => {
        const hitCount = r.statistics.cacheStatuses['HIT'] || 0;
        return sum + (hitCount / r.statistics.totalRequests);
      }, 0) / cdnResults.length * 100,
    };
  }
  ...
  if (cdnResults.length > 0 && directResults.length > 0) {
    const cdnAvg = cdnResults.reduce((sum, r) => sum + (r.statistics.averageResponseTime || 0), 0) / cdnResults.length;
    const directAvg = directResults.reduce((sum, r) => sum + (r.statistics.averageResponseTime || 0), 0) / directResults.length;
    
    report.summary.overallCdnImprovement = ((directAvg - cdnAvg) / directAvg) * 100;
    
    log(`Overall CDN performance improvement: ${report.summary.overallCdnImprovement.toFixed(2)}%`);
  } else {
  ...
  for (const [fileType, results] of Object.entries(resultsByFileType)) {
    log(`${fileType}:`);
    log(`  CDN average response time: ${results.cdnAverageResponseTime.toFixed(2)}ms`);
    
    if (results.directAverageResponseTime !== null) {
      log(`  Direct average response time: ${results.directAverageResponseTime.toFixed(2)}ms`);
  ...
  // Save full report
  report.testDuration = new Date().getTime() - new Date(report.timestamp).getTime();
  
  const reportFile = path.join(config.outputDir, `cdn_report_${getTimestamp()}.json`);
