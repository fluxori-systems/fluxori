...
const duration =
  parseInt(args.find((arg) => arg.startsWith("--duration="))?.split("=")[1]) ||
  10; // in seconds
const connections =
  parseInt(
    args.find((arg) => arg.startsWith("--connections="))?.split("=")[1],
  ) || 10;
const pipelining =
  parseInt(
    args.find((arg) => arg.startsWith("--pipelining="))?.split("=")[1],
  ) || 1;
const specificEndpoint = args
...
async function saveResults(results) {
  ...
  const outputFile = path.join(
    resultsDir,
    `api-benchmark-${env}-${timestamp}.json`,
  );
  ...
  const summaryFile = path.join(
    resultsDir,
    `api-benchmark-summary-${env}-${timestamp}.txt`,
  );
...
async function runAllBenchmarks() {
  ...
  console.log(
    chalk.blue(`Environment: ${env} (${config.environments[env].apiBaseUrl})`),
  );
  ...
  if (!loggedIn && !specificEndpoint) {
    console.error(
      chalk.red("Failed to log in. Only testing public endpoints."),
    );
  }
  ...
  if (specificEndpoint) {
    ...
    if (!endpoint) {
      console.error(
        chalk.red(
          'Invalid endpoint format. Use name:endpoint, e.g., "Health Check:/health"',
        ),
      );
      process.exit(1);
    }
    ...
    console.log(
      chalk.magenta(`Testing specific endpoint: ${name} (${endpoint})`),
    );
  ...
  Object.entries(results.endpoints).forEach(([name, result]) => {
    ...
    console.log(
      chalk.cyan(`  Average Latency: ${result.latency.average.toFixed(2)}ms`),
    );
    console.log(
      chalk.cyan(`  p99 Latency: ${result.latency.p99.toFixed(2)}ms`),
    );
    console.log(
      chalk.cyan(`  Requests/sec: ${result.requests.average.toFixed(2)}`),
    );
    console.log(chalk.cyan(`  Errors: ${result.errors || 0}`));
    console.log(
      chalk.cyan(
        `  Status Codes: ${JSON.stringify(result.statusCodeStats || {})}`,
      ),
    );
    console.log("");
  });
