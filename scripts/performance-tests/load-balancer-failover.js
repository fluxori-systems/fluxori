...
const logFile = path.join(
  config.logDir,
  `failover_test_${getDateFormatted()}.log`,
);
const log = (message) => {
...
fs.writeFileSync(
  logFile,
  `Load Balancer Failover Test - ${new Date().toISOString()}\n\n`,
);
...
const disableServiceInstance = async (service) => {
  ...
  try {
    ...
    const currentInstances =
      serviceConfig.status?.conditions?.find((c) => c.type === "Ready")
        ?.status === "True"
        ? serviceConfig.status?.address?.url || "Unknown"
        : "Not Ready";
...
const simulateLoad = async (duration, requestsPerSecond, url) => {
  log(
    `Simulating load on ${url} for ${duration} seconds at ${requestsPerSecond} requests per second...`,
  );
  ...
  results.medianResponseTime = results.responseTimes.sort((a, b) => a - b)[
    Math.floor(results.responseTimes.length / 2)
  ];
  results.successRate = (results.successCount / results.totalRequests) * 100;

  log(
    `Load test completed. ${results.successCount}/${results.totalRequests} requests successful (${results.successRate.toFixed(2)}%)`,
  );
...
const runFailoverTest = async () => {
  ...
  const analysis = {
    ...
    responseTimeIncrease:
      failoverResults.averageResponseTime - baselineResults.averageResponseTime,
    recoveryResponseTimeDiff:
      recoveryResults.averageResponseTime - baselineResults.averageResponseTime,
  };
  ...
  const testPassed =
    analysis.failoverSuccessRate > 95 && analysis.responseTimeIncrease < 1000;
  ...
  const reportFile = path.join(
    config.logDir,
    `failover_report_${getDateFormatted()}.json`,
  );
