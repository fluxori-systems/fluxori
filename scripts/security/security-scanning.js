...
async function scanIamSecurity() {
  ...
  try {
    ...
    for (const binding of iamPolicy.bindings) {
      if (binding.members.some(m => m === 'allUsers' || m === 'allAuthenticatedUsers')) {
        iamIssues.push({
          severity: 'CRITICAL',
          resource: binding.role,
          issue: 'Public access granted',
          remediation: 'Remove public access (allUsers, allAuthenticatedUsers) from IAM policies'
        });
      }
    }
      ...
      if (keys.length > 0) {
        ...
        if (oldKeys.length > 0) {
          iamIssues.push({
            ...
            remediation: 'Rotate service account keys regularly and remove unused keys'
          });
        }
      }
    }
    ...
    log(`IAM security scan completed. Found ${iamIssues.length} potential issues.`, 
  ...
  try {
    ...
    for (const firewall of firewalls) {
      ...
      if (firewall.direction === 'INGRESS' && 
        ...
        const allowedTraffic = firewall.allowed.map(allow => {
          const protocol = allow.IPProtocol;
          const ports = allow.ports || ['all'];
          return `${protocol}:${ports.join(',')}`;
        }).join('; ');
        
        if (allowedTraffic.includes('all') || allowedTraffic.includes('tcp:all')) {
          networkIssues.push({
            severity: 'CRITICAL',
            resource: firewall.name,
            issue: `Overly permissive firewall rule allows ${allowedTraffic} from anywhere (0.0.0.0/0)`,
            remediation: 'Restrict firewall rules to specific IP ranges and necessary ports only'
          });
        } else if (allowedTraffic.includes('tcp:22') || allowedTraffic.includes('tcp:3389')) {
          networkIssues.push({
            severity: 'HIGH',
            resource: firewall.name,
            issue: `Firewall rule exposes SSH (22) or RDP (3389) to the internet: ${allowedTraffic}`,
            remediation: 'Restrict SSH/RDP access to specific IP ranges or use IAP for secure access'
          });
        }
      }
    }
    ...
    try {
      ...
      if (perimeters.length === 0) {
        networkIssues.push({
          ...
          remediation: 'Consider implementing VPC Service Controls to restrict data exfiltration'
        });
      }
    } catch (error) {
      ...
      networkIssues.push({
        ...
        remediation: 'Ensure VPC Service Controls are properly configured if handling sensitive data'
      });
    }
        ...
        try {
          ...
          if (!lbDetails.sslPolicy) {
            networkIssues.push({
              ...
              remediation: 'Configure a custom SSL policy with modern and secure settings'
            });
          }
        } catch (error) {
          log(`Could not check SSL policy for ${lb.name}: ${error.message}`, 'warning');
        }
      }
    } catch (error) {
      log(`Could not check load balancer configurations: ${error.message}`, 'warning');
    }
    ...
    try {
      ...
      if (securityPolicies.length === 0) {
        networkIssues.push({
          ...
          remediation: 'Implement Cloud Armor security policies to protect against common web attacks'
        });
      } else {
        ...
        for (const policy of securityPolicies) {
          ...
          const hasXssProtection = policyDetails.rules.some(rule => 
            rule.match && rule.match.expr && rule.match.expr.expression && 
            rule.match.expr.expression.includes('xss')
          );
          
          const hasSqliProtection = policyDetails.rules.some(rule => 
            rule.match && rule.match.expr && rule.match.expr.expression && 
          ...
          if (!hasXssProtection) {
            networkIssues.push({
              ...
              remediation: 'Add XSS protection rule using evaluatePreconfiguredExpr("xss-stable")'
            });
          }
          ...
          if (!hasSqliProtection) {
            networkIssues.push({
              ...
              remediation: 'Add SQL injection protection rule using evaluatePreconfiguredExpr("sqli-stable")'
            });
          }
        }
      }
    ...
    // Save report
    const networkReportPath = path.join(outputDir, 'network-security-report.json');
    fs.writeFileSync(networkReportPath, JSON.stringify(networkReport, null, 2));
    
    log(`Network security scan completed. Found ${networkIssues.length} potential issues.`, 
...
async function scanApplicationSecurity() {
  ...
  try {
    ...
    try {
      ...
      if (sslOutput.includes('SSLv3') || sslOutput.includes('TLSv1.0') || sslOutput.includes('TLSv1.1')) {
        appIssues.push({
          severity: 'HIGH',
          resource: options.apiUrl,
          issue: 'Outdated SSL/TLS protocols in use',
          remediation: 'Disable SSLv3, TLSv1.0, and TLSv1.1, use only TLSv1.2+ with strong ciphers'
        });
      }
      ...
      if (sslOutput.includes('weak cipher')) {
        appIssues.push({
          ...
          remediation: 'Disable weak cipher suites, use only strong ciphers with perfect forward secrecy'
        });
      }
    } catch (error) {
    ...
    if (corsOutput.includes('*')) {
      appIssues.push({
        severity: 'HIGH',
        resource: options.apiUrl,
        issue: 'CORS allows requests from any origin (Access-Control-Allow-Origin: *)',
    ...
    log(`Application security scan completed. Found ${appIssues.length} potential issues.`, 
...
function generateConsolidatedReport(reports) {
  ...
  log(`Consolidated security report generated with ${consolidated.summary.total} issues:`, 
